...
class ItemViewTest extends EntityViewTestCase {
	...
	public function testTermsViewPlaceholdersArePropageted() {
		$placeholders = [ 'a' => 'b' ];
		$itemView = $this->newItemView( $placeholders );

		$view = $itemView->getHtml( $this->makeEntity( $this->makeEntityId( 42 ) ) );
	...
	private function newItemView( $placeholders = [] ) {
		...
		$termsView->method( 'getPlaceholderMapping' )->willReturn( $placeholders );
