...
$.widget( 'wikibase.sitelinkview', PARENT, {
	...
	_shortenPageName: function() {
		if ( this._isInEditMode || !this.options.value ) {
			return;
		}

		var text = this.options.value.getPageName();
		// 15 characters cover ~57% of the article titles in the German Wikipedia.
		if ( text.length <= 15 ) {
			return;
		}
		...
		while ( true ) {
			var length = !lowerBreakPoint ? 10
				: !upperBreakPoint ? text.length
			...
			$a.text( length < text.length ? text.slice( 0, length ) + '…' : text );
