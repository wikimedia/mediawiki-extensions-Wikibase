...
$.widget( 'wikibase.statementview', PARENT, {
	...
	_create: function() {
		...
		var wasEmpty = this.element.is( ':empty' );
		PARENT.prototype._create.call( this );

		if (wasEmpty && this.options.value === null ) {
			this.draw();
		} else {
	...
	_createReferencesToggler: function() {
		...
		if ( this._referencesListview ) {
			visible = this._referencesListview.items().length === 0;
			this.$references.toggleClass( 'wikibase-initially-collapsed', !visible );
		} else {
			visible = !this.$references.hasClass( 'wikibase-initially-collapsed' );
		}
	...
	isInitialValue: function() {
		if ( !this.isInEditMode() ) {
			console.warn( "statementview::isInitialValue should only be called in edit mode" );
			return true;
		}
		var i;
	...
	_instantiateStatement: function( guid ) {
		if ( !this.isInEditMode() ) {
			console.warn( "statementview::_instantiateStatement should only be called in edit mode" );
			return null;
		}
	...
	isValid: function() {
		if ( !this.isInEditMode() ) {
			console.warn( "statementview::isValid should only be called in edit mode" );
			return true;
		}
