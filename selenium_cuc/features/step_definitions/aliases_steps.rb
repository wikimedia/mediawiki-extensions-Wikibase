...
When /^I enter (.+) as new aliases$/ do |values|
  on(ItemPage).insert_aliases(values.split(','))
end
...
Then /^There should be (\d+) aliases in the list$/ do |num|
  on(ItemPage).count_existing_aliases.should == Integer(num)
end

Then /^List of aliases should contain (.+)$/ do |values|
  values.split(',').each do |value|
    on(ItemPage).alias_exists(value).should be_true
  end
end
