...
class TermSqlIndexTest extends TermIndexTestCase {
	...
	public function testInsertTerms_NoUseSearchFields() {
		$item = new Item( new ItemId( 'Q1112362' ) );
		$termDe = new TermIndexEntry( [
			'entityId' => $item->getId(),
			'termText' => 'German',
		...
		$this->assertSame( '', $result->fetchRow()['term_search_key'] );
	}

	public function testInsertTerms_NoUseSearchFieldsButForceWriteSearchFields() {
		$item = new Item( new ItemId( 'Q1112362' ) );
		$termDe = new TermIndexEntry( [
			'entityId' => $item->getId(),
			'termText' => 'German',
		...
		$termIndex = $this->getTermIndex();
		$termIndex->setUseSearchFields( false );
		$termIndex->setForceWriteSearchFields( true );
		/** @var TermSqlIndex $termIndex */
		$termIndex = TestingAccessWrapper::newFromObject( $termIndex );
		...
		$this->assertSame( 'german', $result->fetchRow()['term_search_key'] );
	}
