...
use Wikibase\Lib\Store\Sql\MediaWikiTermStore\MediaWikiNormalizedTermCleaner;
use Wikimedia\Rdbms\IMaintainableDatabase;

/**
 * @covers \Wikibase\Lib\Store\Sql\MediaWikiTermStore\MediaWikiNormalizedTermCleaner
 *
 * @group Wikibase
 * @group WikibaseStore TODO?
 * @group Database
 *
...
class MediaWikiNormalizedTermCleanerTest extends MediaWikiTestCase {
	...
	protected function getSchemaOverrides( IMaintainableDatabase $db ) {
		return [
			// TODO lib->repo dependency? perhaps the class under test also belongs in repo?
			'scripts' => [ __DIR__ . '/../../../../../../repo/sql/AddNormalizedTermsTablesDDL.sql' ],
			'create' => [
				'wbt_type',
				'wbt_text',
				'wbt_text_in_lang',
	...
	public function testCleanupEverything() {
		...
		$cleaner = new MediaWikiNormalizedTermCleaner( $this->db, $this->db );
		$cleaner->cleanTermInLangIds( [ $termInLang1Id, $termInLang2Id ] );
	...
	public function testCleanupTermInLangButNoTextInLang() {
		...
		$cleaner = new MediaWikiNormalizedTermCleaner( $this->db, $this->db );
		$cleaner->cleanTermInLangIds( [ $termInLang1Id, $termInLang4Id ] );
	...
	public function testCleanupOneTextInLangButNoText() {
		...
		$cleaner = new MediaWikiNormalizedTermCleaner( $this->db, $this->db );
		$cleaner->cleanTermInLangIds( [ $termInLang1Id ] );
