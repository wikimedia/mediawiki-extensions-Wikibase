...
class PropertyTermStoreWriterTest extends \MediaWikiTestCase {
	...
	public function testSavesPropertyTermsInStore() {
		$property = $this->buildPropertyWithTerms();
		$writer = $this->getPropertyTermStoreWriter();

		$this->assertNoPropertyTermsExistInStore( $property );

		$writer->saveTerms( $property );

		$this->assertPropertyTermsExistInStore( $property );
	}
	 ...
	 * @return Property
	 */
	private function buildPropertyWithTerms( $id = 'P123' ) {
