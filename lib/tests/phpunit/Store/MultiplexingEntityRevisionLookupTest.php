...
class MultiplexingEntityRevisionLookupTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGivenExistingRevision_getEntityRevisionReturnsIt() {
		...
		$multiplexingLookup = new MultiplexingEntityRevisionLookup( array( '' => $localLookup, 'foo' => $fooLookup, ) );

		$revision = $multiplexingLookup->getEntityRevision( new ItemId( 'Q123' ) );
		$foreignRevision = $multiplexingLookup->getEntityRevision( new ItemId( 'foo:Q303' ) );

		$this->assertEquals( $item, $revision->getEntity() );
		$this->assertEquals( 123, $revision->getRevisionId() );

		$this->assertEquals( $foreignItem, $foreignRevision->getEntity() );
	...
	public function testGivenNotExistingEntityIdFromKnownRepository_getEntityRevisionReturnsNull() {
		...
		$fooLookup->expects( $this->any() )
			->method( 'getEntityRevision' )
			->with( new ItemId( 'foo:Q808' ) )
			->willReturn( null );

		$multiplexingLookup = new MultiplexingEntityRevisionLookup( array( '' => $localLookup, 'foo' => $fooLookup, ) );

		$this->assertNull( $multiplexingLookup->getEntityRevision( new ItemId( 'Q124' ) ) );
		$this->assertNull( $multiplexingLookup->getEntityRevision( new ItemId( 'foo:Q808' ) ) );
	}
	...
	public function testGivenExistingEntityId_getLatestRevisionIdReturnsTheId() {
		...
		$this->assertEquals( 100, $multiplexingLookup->getLatestRevisionId( new ItemId( 'foo:Q303' ) ) );
	}
	...
	public function testGivenNotExistingEntityIdFromKnownRepository_getLatestRevisionIdReturnsFalse() {
		...
		$this->assertFalse( $multiplexingLookup->getLatestRevisionId( new ItemId( 'foo:Q303' ) ) );
	}
