...
class ChunkCacheTest extends \MediaWikiTestCase {
	 ...
	 *
	 * @param $start int The start address of the chunk to load.
	 * @param $length int The length of the chunk to load.
	 * @param $expectedAccess array List of [ $start, $length ] accesses to the backing store.
	 * @param $info string Description.
	 * @return array
	 */
	protected static function makeCacheAction( $start, $length, $expectedAccess, $info ) {
	...
	public function testLoadChunk( $chunkSize, $maxSize, $sequence ) {
		...
		$store->method( 'loadChunk' )
			->withConsecutive( ...array_merge( ...array_filter( array_map(
				function ( $action ) {
