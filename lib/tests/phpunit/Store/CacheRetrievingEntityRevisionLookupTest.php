...
class CacheRetrievingEntityRevisionLookupTest extends EntityRevisionLookupTest {
	...
	/**
	 * Make sure we don't write to the cache.
	 */
	public function testNotSettingCache() {
		$mock = new MockRepository();
		...
		// Retrieve
		$this->assertEquals( $item, $lookup->getEntityRevision( $id )->getEntity() );

		// Should still not be cached
		$this->assertNull( $entityRevisionCache->get( $id ) );
	}
