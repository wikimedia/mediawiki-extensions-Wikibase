...
class SitesBuilderTest extends PHPUnit_Framework_TestCase {
	 ...
	 * @dataProvider sitesProvider
	 */
	public function testFilterSites( $sites, $group, $wikiId, $expected ) {
		$store = new MockSiteStore();

		$sitesBuilder = new SitesBuilder( $store );
		$filteredSites = $sitesBuilder->filterSites( $sites, $group, $wikiId );

		$this->assertEquals( $expected, $filteredSites );
	}
