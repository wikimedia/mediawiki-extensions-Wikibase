	...
	SiteLinkLookup
{
	...
	public function getLinks( array $itemIds, array $siteIds = array(), array $pageNames = array() ) {
		...
		foreach ( array_keys( $this->entities ) as $idString ) {
			...
			if ( !empty( $itemIds ) && !in_array( $entityId->getNumericId(), $itemIds ) ) {
				continue;
			}
			...
			foreach ( $item->getSiteLinks() as $siteLink ) {
				if ( $this->linkMatches( $item, $siteLink, $itemIds, $siteIds, $pageNames ) ) {
					$links[] = array(
						$siteLink->getSiteId(),
	...
	private function linkMatches(
		...
		return ( empty( $itemIds ) || in_array( $item->getId()->getNumericId(), $itemIds ) )
	...
	public function getLatestRevisionId( EntityId $entityId ) {
		...
		return $revision === null ? false : $revision->getRevision();
	}
	...
	public function saveEntity( Entity $entity, $summary, User $user, $flags = 0, $baseRevisionId = false ) {
		...
		if ( $baseRevisionId !== false && $this->getEntityRevision( $entityId )->getRevision() !== $baseRevisionId ) {
		...
		$this->putLog( $revision->getRevision(), $entity->getId(), $summary, $user->getName() );
		return $revision;
	}
	...
	public function userWasLastToEdit( User $user, EntityId $entityId, $lastRevisionId ) {
		...
		foreach ( $this->entities[$key] as $revision ) {
			if ( $revision->getRevision() >= $lastRevisionId ) {
