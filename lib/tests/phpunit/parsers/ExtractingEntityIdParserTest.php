...
class ExtractingEntityIdParserTest extends \PHPUnit_Framework_TestCase {
	...
	public function provideParse_bad_regex() {
		return array(
			'prefix with no capture group' => array( '!^https?://acme.test/entity/.*$!' ),
			'ID pattern with no capture group' => array( '!^[QP]\d+$!' ),
		);
	}

	/**
	 * @dataProvider provideParse_bad_regex
	 */
	public function testParse_bad_regex( $regexp ) {
		$parser = new ExtractingEntityIdParser( $regexp, new BasicEntityIdParser() );
