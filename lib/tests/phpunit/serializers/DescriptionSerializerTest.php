...
class DescriptionSerializerTest extends \PHPUnit_Framework_TestCase {
	 ...
	 */
	public function testNewFromSerialization( $descriptions, $options, $message ) {
		$descriptionSerializer = new DescriptionSerializer( $options );
		$serializedDescriptions = $descriptionSerializer->getSerialized( $descriptions );

		$deserializedDescriptions = $descriptionSerializer->newFromSerialization( $serializedDescriptions );
		$this->assertEquals( $descriptions, $deserializedDescriptions, $message );
	}
	...
	public function newFromSerializationProvider() {
		...
		return array(
			array( $descriptions, $options, 'serialization with index tags' ),
			array( $descriptions, $options2, 'serialization without index tags' )
		);
	}
}
