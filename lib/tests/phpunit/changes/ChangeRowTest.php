...
class ChangeRowTest extends \ORMRowTest {
     ...
     */
    public function testConstructor( $data, $loadDefaults ) {
		$row = $this->getRowInstance( $data, $loadDefaults );
		$this->verifyFields( $this->getRowInstance( $data, $loadDefaults ), $data );
		return $row;
    }

    public function instanceProvider() {
		$entities = $this->constructorTestProvider();
		$instances = array();
		foreach( $entities as $entity ) {
			$instances[] = array( $this->getRowInstance( $entity[0], $entity[1] ) );
		}
		return $instances;
    }
