...
final class TestItems {
	...
	public static function getItems() {
		...
		if ( $links->count() > 1 ) {
			$item->addSiteLink( new SiteLink( $links->getIterator()->current()->getGlobalId(), 'spam' ) );
		}
		...
		if ( $links->count() > 1 ) {
			...
			$item->addSiteLink( new SiteLink( $linksIterator->current()->getGlobalId(), 'spam' ) );
			$linksIterator->next();
			$item->addSiteLink( new SiteLink( $linksIterator->current()->getGlobalId(), 'foobar' ) );
		}
