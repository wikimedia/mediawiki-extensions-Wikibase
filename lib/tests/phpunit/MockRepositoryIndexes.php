 ...
 */
class MockRepositoryIndexes implements EntityStore, SiteLinkLookup {
	...
	public function getConflictsForItem( Item $item, DatabaseBase $db = null ) {
		...
		$conflicts = array();
		$ids = $this->repository->getEntityIds();
	...
	/**
	 * Unregisters the sitelinsk of the given entity so they are no longer found with getLinks, etc
	 *
	 * @param EntityId $id
	 */
	protected function unregisterSiteLinks( EntityId $id ) {
		// clean up old sitelinks

		$numId = $id->getNumericId();
	...
	public function removeEntity( EntityId $id ) {
		try {
			...
			if ( $oldEntity && ( $oldEntity instanceof Item ) ) {
				// clean up old sitelinks
				$this->unregisterSiteLinks( $oldEntity->getId() );
			}
	...
	public function getSiteLinksForItem( ItemId $itemId ) {
		$entity = $this->repository->getEntity( $itemId );
	...
	public function saveRedirect( EntityRedirect $redirect, $summary, User $user, $flags = 0, $baseRevId = false ) {
		$this->unregisterSiteLinks( $redirect->getEntityId() );
