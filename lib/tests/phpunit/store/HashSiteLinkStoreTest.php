...
class HashSiteLinkStoreTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetItemIdForLink() {
		...
		$this->assertNull(  $siteLinkStore->getItemIdForLink( 'enwiki', 'Foo' ) );
		$this->assertEquals( $itemId, $siteLinkStore->getItemIdForLink( 'enwiki', 'Bar' ) );

		// test lookup after item deletion
		$siteLinkStore->deleteLinksOfItem( $itemId );

		$this->assertNull( $siteLinkStore->getItemIdForLink( 'enwiki', 'Foo' ) );
		$this->assertNull( $siteLinkStore->getItemIdForLink( 'enwiki', 'Bar' ) );
	}
	...
	public function testGetLinks( array $items, array $itemIds, array $sites, array $pages, array $expectedLinks ) {
		...
		$links = $siteLinkStore->getLinks( $itemIds, $sites, $pages );
