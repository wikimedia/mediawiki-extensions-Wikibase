...
class PropertyInfoStoreTestHelper {
	...
	public function testGetPropertyInfoForDataType() {
		...
		$this->test->assertCount( 0, $table->getPropertyInfoForDataType( 'commonsMedia' ), "should initially be empty" );

		$table->setPropertyInfo( $p23, $info23 );
		$this->test->assertCount( 0, $table->getPropertyInfoForDataType( 'commonsMedia' ), "after adding one property" );

		$table->setPropertyInfo( $p42, $info42 );
		$this->test->assertCount( 1, $table->getPropertyInfoForDataType( 'commonsMedia' ), "after adding the second property" );

		$table->removePropertyInfo( $p23 );
		$this->test->assertCount( 1, $table->getPropertyInfoForDataType( 'commonsMedia' ), "after removing one property" );

		$table->removePropertyInfo( $p42 );
		$this->test->assertCount( 0, $table->getPropertyInfoForDataType( 'commonsMedia' ), "after removing the second property" );
	}
