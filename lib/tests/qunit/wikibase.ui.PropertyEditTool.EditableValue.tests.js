...
var
	...
	QUnit.test( 'dis- and enabling', function( assert ) {
		var ev = newTestEV( $( '<div><div class="wb-value"/></div>' ) ),
			editGroup = ev.getToolbar().editGroup;
		...
		assert.ok(
			ev.getToolbar().editGroup.btnSave.data( 'toolbarbutton' ).isDisabled(),
			'Save button is disabled.'
		);

		assert.ok(
			ev.getToolbar().editGroup.btnCancel.data( 'toolbarbutton' ).isDisabled(),
			'Cancel button is disabled.'
		);

		assert.ok(
			ev.getToolbar().editGroup.btnRemove.data( 'toolbarbutton' ).isEnabled(),
	...
	QUnit.test( 'error handling', function( assert ) {
		...
		assert.ok(
			ev._toolbar.editGroup.btnSave.data( 'toolbarbutton' ).getTooltip() instanceof wb.ui.Tooltip,
		...
		assert.ok(
			ev._toolbar.editGroup.btnRemove.data( 'toolbarbutton' ).getTooltip() instanceof wb.ui.Tooltip,
		...
		ev.stopEditing( true );
		assert.equal(
			ev._toolbar.editGroup.btnSave.data( 'toolbarbutton' ).getTooltip()._error.message,
