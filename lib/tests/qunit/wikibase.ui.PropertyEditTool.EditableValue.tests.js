...
var
	...
	QUnit.test( 'dis- and enabling', function( assert ) {
		...
		assert.ok(
			ev.getToolbar().editGroup.btnSave.isDisabled(),
			'Save button is disabled.'
		);

		assert.ok(
			ev.getToolbar().editGroup.btnCancel.isDisabled(),
			'Cancel button is disabled.'
		);

		assert.ok(
			ev.getToolbar().editGroup.btnRemove.isEnabled(),
	...
	QUnit.test( 'error handling', function( assert ) {
		...
		ev.stopEditing( true );
		assert.equal(
			ev._toolbar.editGroup.btnSave.getTooltip()._error.message,
