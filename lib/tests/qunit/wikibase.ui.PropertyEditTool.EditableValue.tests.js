...
var
	...
	QUnit.test( 'error handling', function( assert ) {
		...
		assert.ok(
			editGroup.$btnSave.data( 'wbtooltip' ) instanceof $.wikibase.wbtooltip,
		...
		assert.ok(
			editGroup.$btnRemove.data( 'wbtooltip' ) instanceof $.wikibase.wbtooltip,
		...
		ev.stopEditing( true );
		assert.equal(
			editGroup.$btnSave.data( 'wbtooltip' ).options.content.message,
