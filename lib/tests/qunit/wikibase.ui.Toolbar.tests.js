...
( function( mw, wb, $, QUnit ) {
	...
	QUnit.test( 'fill and remove', function( assert ) {
		...
		var label = $( '<span/>' ).toolbarlabel( { content: 'label text' } );
		...
		var second_label = $( '<span/>' ).toolbarlabel( { content: 'second label text' } );
	...
	QUnit.test( 'dis- and enabling', function( assert ) {
		var label = $( '<span/>' ).toolbarlabel( { content: 'label text', stateChangeable: false } );
		this.toolbar.addElement( label );
