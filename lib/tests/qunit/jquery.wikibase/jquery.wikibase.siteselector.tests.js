...
( function( wb, $, QUnit ) {
	...
	QUnit.test( 'getSelectedSite()', 36, function( assert ) {
		...
		for ( var i = 0; i < testStrings.length; i++ ) {
			$queue = $queue.queue( 'tests', function( next ) {
				testString( testStrings[i][0], testStrings[i][1], next );
			} );
		}
