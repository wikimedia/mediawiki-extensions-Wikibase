...
( function( mw, wb, $, QUnit, Site, GenericSet ) {
	'use strict';
	...
	var newTestSiteSelector = function( options ) {
		...
		options = $.merge( { resultSet: sites }, options );
	...
	QUnit.test( 'Update result set', function( assert ) {
		...
		siteselector.setResultSet( new GenericSet( Site ) );
		...
		siteselector.setResultSet( wb.getSites() );
...
}( mediaWiki, wikibase, jQuery, QUnit, wikibase.Site, GenericSet ) );
