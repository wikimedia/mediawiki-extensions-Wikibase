...
QUnit.test( 'remove superflous empty sitelinkview', function( assert ) {
	var $sitelinklistview = createSitelinklistview( {
			allowedSiteIds: [ 'aawiki', 'dewiki', 'enwiki' ]
		} ),
		sitelinklistview = $sitelinklistview.data( 'sitelinklistview' ),
		listview = sitelinklistview.$listview.data( 'listview' );

	sitelinklistview.enterNewItem();
	listview.items().first().find( '.wikibase-sitelinkview-siteid input' ).val( 'enwiki' );
	listview.items().first().find( '.wikibase-sitelinkview-siteid input' ).trigger( 'input' );
	listview.items().first().find( '.wikibase-sitelinkview-page input' ).val( 'testpage' );
	listview.items().first().find( '.wikibase-sitelinkview-page input' ).trigger( 'input' );

	assert.equal( listview.items().length, 2 );

	listview.items().eq(1).find( '.wikibase-sitelinkview-siteid input' ).val( 'aawiki' );
	listview.items().eq(1).find( '.wikibase-sitelinkview-siteid input' ).trigger( 'input' );
	listview.items().eq(1).find( '.wikibase-sitelinkview-page input' ).val( 'testpage' );
	listview.items().eq(1).find( '.wikibase-sitelinkview-page input' ).trigger( 'input' );

	assert.equal( listview.items().length, 3 );

	listview.items().first().find( '.wikibase-sitelinkview-page input' ).val( '' );
	listview.items().first().find( '.wikibase-sitelinkview-page input' ).trigger( 'input' );
	listview.items().first().find( '.wikibase-sitelinkview-siteid input' ).val( '' );
	listview.items().first().find( '.wikibase-sitelinkview-siteid input' ).trigger( 'input' );
