...
( function( $, mw, wb ) {
	...
	function createValueWidget( value ) {
		return $( '<div/>' ).addClass( 'test_valuewidget' ).valuewidget( ( value || null ) );
	}
	...
	QUnit.test( 'Initialize and destroy', function( assert ) {
		...
		$node = createListview( valueWidgets );
		listview = $node.data( 'listview' );
		...
		valueWidgets.push( createValueWidget( 'b' ) );
	...
	QUnit.test( 'Add and remove items', function( assert ) {
		...
		for( var i = 0; i < valueWidgets.length; i++ ) {
			listview.addItem( valueWidgets[i] );
			...
			assert.equal(
				listview.listItemAdapter().liValue( listview.items().eq( i ) ).get( 0 ),
				valueWidgets[i].get( 0 ),
	...
	QUnit.test( 'Enter new items', function( assert ) {
		...
		listview.addItem( valueWidgets[0] );
