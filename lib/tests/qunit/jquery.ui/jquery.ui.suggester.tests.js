...
( function( mw, wb, $, QUnit, undefined ) {
	...
	QUnit.module( 'jquery.ui.suggester', QUnit.newWbEnvironment( {
		setup: function() {
			this.subject = $( '<input/>' ).suggester( {
				source: [
					'a',
					'ab',
					'abc',
			...
			this.autocomplete = this.subject.data( 'suggester' );
		},
		teardown: function() {
			this.subject.remove();
	...
	QUnit.test( 'basic tests', function( assert ) {
		...
		assert.equal(
			this.autocomplete.menu.element.children().length,
			3,
