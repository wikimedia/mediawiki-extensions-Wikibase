...
( function( mw, wb, $, QUnit, undefined ) {
	...
	QUnit.test( 'valid input resulting into some site being selected', function( assert ) {
		...
		$.each( siteIds, function( i, siteId ) {
			var testSite = wb.sites.getSite( siteId );
			var inputStrings = {
	...
	QUnit.test( 'init input with blacklist', function( assert ) {
		...
		siteIdInterface.setValue( wb.sites.getSite( siteIds[0] ).getId() );
		...
		siteIdInterface._inputElem.val( wb.sites.getSite( siteIds[1] ).getId() );
		siteIdInterface._inputElem.data( 'siteselector' ).setSelectedSite( null );

		QUnit.stop();

		// let site selector widget search for any result
		siteIdInterface._inputElem.data( 'siteselector' ).search()
		.done( function() {
			...
			assert.equal(
				siteIdInterface.setValue( wb.sites.getSite( siteIds[0] ).getId() ),
				wb.sites.getSite( siteIds[0] ).getId(),
				'set value to valid site id'
			);

			assert.equal(
				siteIdInterface.getSelectedSite(),
				wb.sites.getSite( siteIds[0] ),
