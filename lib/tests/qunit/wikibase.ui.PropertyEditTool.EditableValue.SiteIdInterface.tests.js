...
( function() {
	...
	test( 'init input', function() {
		...
		equal(
			this.subject._inputElem.data( 'siteselector' ).getSelectedSiteId(),
			null,
			'No site id selected.'
		);
		...
		$.each( testStrings, function( index, val ) {
			equal(
				self.subject.setValue( val ),
				testSite.getId(),
				'Set value to "' + val + '"'
			);

			equal(
				self.subject._inputElem.data( 'siteselector' ).getSelectedSiteId(),
				testSite.getId(),
	...
	test( 'init input with blacklist', function() {
		...
		this.subject.init( this.node, {
			ignoredSiteLinks: [ this.siteIds[1] ]
		} );
