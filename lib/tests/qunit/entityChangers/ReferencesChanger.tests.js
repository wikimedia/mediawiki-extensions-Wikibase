...
( function( sinon, wb, $ ) {
	...
	QUnit.module( 'wikibase.entityChangers.ReferencesChanger', QUnit.newMwEnvironment() );
	...
	QUnit.test( 'removeReference correctly handles API failures', function( assert ) {
	  ...
	  .fail( function( error ) {
	    QUnit.start();
			assert.ok( error instanceof wb.RepoApiError, 'removeReference did not fail with a RepoApiError' );
	...
	QUnit.test( 'setReference correctly handles API failures', function( assert ) {
	  ...
	  .fail( function( error ) {
	    QUnit.start();
			assert.ok( error instanceof wb.RepoApiError, 'setReference did not fail with a RepoApiError' );
			assert.equal( error.code, 'errorCode' );
	  } );
	} );
