...
( function( sinon, wb, $ ) {
	...
	QUnit.test( 'setReferences correctly handles API failures', function( assert ) {
	  var api = {
	    setReference: sinon.spy( function() {
	  ...
	  .done( function() {
	    assert.ok( false, 'setReferences should have failed' );
	  } )
	  .fail( function( error ) {
	    QUnit.start();
	    assert.equal( error.error.code, 'errorCode' );
	  } );
	} );
