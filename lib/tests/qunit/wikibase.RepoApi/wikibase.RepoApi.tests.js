...
( function( wb, $, QUnit, undefined ) {
	...
	/**
	 * Creates a property via the API.
	 *
	 * @param {Object} [data] Stringified JSON representing the property content
	 */
	var createProperty = function( data ) {
		data = data || { datatype: 'string' };

		api.createEntity( 'property', data ).done( function( response ) {
			QUnit.assert.equal(
				response.success,
				1,
				'Created property.'
	...
	QUnit.test( 'Create a claim (string value)', function( assert ) {
		...
		testrun.queue( qkey, function() { createItem(); } );
		testrun.queue( qkey, function() { createProperty(); } );

		testrun.queue( qkey, function() {

			api.createClaim(
					entity.id, entity.lastrevid, 'value',
					property.id, 'This claim is true'
				).done( function( response ) {
	...
	QUnit.test( 'Get claim (string value)', function( assert ) {
		...
		testrun.queue( qkey, function() {
			...
			api.getClaim(entity.id, property.id).done( function( response ) {

				assert.ok(
					property.id in response.claims,
					'Claim data for this property found'
				);
				assert.equal(
	...
	function testSetBasicMultiLingualValue( assert, type, apiFnName ) {
		...
		testrun.queue( qkey, function() {
			abstractedApi.getEntities( entity.id, typesApiPropName ).done( function( entities ) {
				...
				assert.ok(
					entities[ entity.id ] instanceof wb.Entity,
