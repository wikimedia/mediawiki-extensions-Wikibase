...
( function() {
	...
	test( 'basic check', function() {
		...
		equal(
			typeof this.autocomplete._inputElem.data( 'suggester' ),
			'object',
			'initialized autocomplete widget'
		);

		equal(
			this.autocomplete._inputElem.data( 'suggester' ).menu.element.css( 'display' ),
			'none',
			'menu is hidden'
		);
	...
	test( 'simulate AJAX', function() {
		...
		this.autocomplete._inputElem.data( 'suggester' ).source = $.proxy(
		...
		this.autocomplete._inputElem.data( 'suggester' ).search( 'y' );
