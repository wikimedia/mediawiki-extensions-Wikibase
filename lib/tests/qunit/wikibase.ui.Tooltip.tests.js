...
( function() {
	module( 'wikibase.ui.Tooltip', window.QUnit.newWbEnvironment( {
		setup: function() {
			this.node = $( '<div/>' );
			this.label = $( '<span/>' ).toolbarlabel( { content: 'Text' } );
		},
		teardown: function() {
			$( window ).off( 'click' );
			$( window ).off( 'resize' );
			this.tooltip = null;
			this.label.data( 'toolbarlabel' ).destroy();
			this.node = null;
		}
	...
	test( 'show and hide by triggering events', function() {
		this.label.data( 'toolbarlabel' ).setTooltip( 'Text' );

		equal(
			this.label.data( 'toolbarlabel' )._tooltip._isVisible,
			false,
			'tooltip is hidden'
		);
	...
	test( 'show and hide error tooltip', function() {
		...
		this.label.data( 'toolbarlabel' ).setTooltip( this.tooltip );
		...
		this.label.data( 'toolbarlabel' ).removeTooltip();
