...
( function( mw, wb, $, QUnit ) {
	...
	/**
	 * Executes assertions testing how many values are currently in a certain section of the
	 * SiteLinksEditTool's subject table.
	 *
	 * @param {QUnit.assert} assert
	 * @param {wb.ui.SiteLinksEditTool} subject
	 * @param {Object} definition The table section (e.g. "tbody" or "tfoot") as key, the expected
	...
	function assertSitesSetWithSites( assert, sitesSet, sites, description ) {
		var sitesSetsSiteIds = [];
		sitesSet.each( function( site ) {
			sitesSetsSiteIds.push( site.getId() );
		} );
		assert.deepEqual(
			sitesSetsSiteIds,
			sites,
	...
	QUnit.test( 'getUnusedAllowedSites()', function( assert ) {
		var subject = this.subject;

		assertSitesSetWithSites( assert,
			subject.getUnusedAllowedSites(),
			[ 'enwiki', 'dewiki' ],
	...
	QUnit.test( 'getRepresentedSites()', function( assert ) {
		var subject = this.subject;

		assertSitesSetWithSites( assert,
			subject.getRepresentedSites(),
			[],
