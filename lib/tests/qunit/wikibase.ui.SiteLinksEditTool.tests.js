...
( function( wb, $, QUnit ) {
	...
	function hackyValueInsertion( editTool, siteLink, siteLinkGlobalId ) {
		...
		newValue.sitePageInterface._inputElem.data( 'suggester' ).option( 'source', siteLink[1] );
	...
	QUnit.test( 'getUnusedAllowedSiteIds()', function( assert ) {
		...
		hackyValueInsertionAndSave( subject, [ 'English (enwiki)', 'London' ], 'enwiki' );
	...
	QUnit.test( 'adding a new editable site link', function( assert ) {
		...
		var editableValue = this.subject.enterNewValue();
		editableValue.siteIdInterface.setValue( rawNewValue );

		assert.strictEqual(
			editableValue.siteIdInterface.getSelectedSiteId(),
			null,
