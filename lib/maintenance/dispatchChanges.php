...
class DispatchChanges extends \Maintenance {
	 ...
	 * * chd_disabled: whether updates are currently disable for this wiki (should always be 0 or false).
	 *
	...
	protected function trySelectClient() {
		...
		$state = $db->selectRow(
			$this->stateTable,
			array( 'chd_wiki', 'chd_seen', 'chd_touched', 'chd_lock', 'chd_disabled' ),
			array( 'chd_wiki' => $wikiId ),
			__METHOD__,
			array( 'FOR UPDATE' )
		);
		...
		if ( $state['chd_disabled'] ) {
			...
			$this->trace( "Updates to $wikiId are disabled." );
	...
	public function postChangeJobs( $wikiId, array $changes ) {
		...
		$this->trace( "Posted notification job for " . count( $changes ) . " changes to $wikiId." );
	}
