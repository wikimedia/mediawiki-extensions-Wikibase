...
( function( wb, $ ) {
	...
	function unserializeClaims( serialization ) {
		...
		// get claims:
		for( var i = 0; i < serialization.order.length; i++ ) {
			var propertyId = serialization.order[i];

			for( var j = 0; j < serialization.claims[propertyId].length; j++ ) {
				// TODO: use ClaimUnserializer here after it got implemented
				var claim = wb.Claim.newFromJSON( serialization.claims[propertyId][j] );
	 ...
	 * @since 0.4
	 */
	var SELF = MODULE.EntityUnserializer = wb.utilities.inherit( 'WbEntityUnserializer', PARENT, {
		/**
		...
		unserialize: function( serialization ) {
			...
			var entityMapData = {
				...
				claims: unserializeClaims( serialization.claimlist )
			};
