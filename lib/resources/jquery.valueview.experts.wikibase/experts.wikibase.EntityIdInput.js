...
( function( mw, wb, $, vv ) {
	...
	vv.experts.wikibase.EntityIdInput = vv.expert( 'wikibaseentityidinput', PARENT, {
		...
		_setRawValue: function( rawValue ) {
			...
			function update( simpleEntity ) {
				self.$input.data( 'entityselector' ).selectedEntity( simpleEntity );
				self._resizeInput();
			}

			if( entityId ) {
				this._options.entityStore.get( entityId ).done( function( fetchedEntity ) {
					var simpleEntity = null;

					if( fetchedEntity ) {
						// entity selector requires very basic data only, but ID has to be set which
						// is not the case in the entity store.
						simpleEntity = {
					...
					if( !fetchedEntity ) {
						// Entity not in the system, can be considered deleted. In this case we
						// display an empty input box while the "real" value is still that reference
						// to the entity not in the system. _getValue() has to return that one until
						// the user types something.
						// NOTE: This is *not* a hack!
						this._actualValue = entityId;
						// NOTE: this has to be done after entity selector change because there is a
						//  event registered to "entityselectoraftersetentity" which will set the
						//  actual value back to false.
					}
					// TODO: entityselector should just be able to handle wb.Entity without making
					//  it a dependency there.
				} );
			} else {
				update( null );
			}
