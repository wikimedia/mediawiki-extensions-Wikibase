 ...
 *
 * @licence GNU GPL v2+
 * @author H. Snater < wikibase@snater.com >
 *
...
( function( $, undefined ) {
	...
	$.widget( 'ui.suggester', $.ui.autocomplete, {
		...
		_create: function() {
			...
			this.element.addClass( 'ui-suggester' );
			this.menu.element.addClass( 'ui-suggester-list' );

			this.element.on( 'suggesteropen', $.proxy( function( event ) {
			...
			$( window ).off( 'suggester' );
			$( window ).on( 'resize.suggester', $.proxy( function() {
				if ( $( '.ui-suggester' ).length > 0 ) {
					$( '.ui-suggester' ).data( 'suggester' ).close( {} );
				}
			}, this ) );
		},
			...
			$.ajax( {
				...
				success: $.proxy( function( response ) {
					if ( response[0] === this.element.val() ) {
						...
						this._trigger( 'response', [response[1]] );
					}
				}, this ),
				error: $.proxy( function( jqXHR, textStatus, errorThrown ) {
					this.element.removeClass( 'ui-autocomplete-loading' );
					this.element.focus();
					this._trigger( 'error', [textStatus, errorThrown] );
				}, this )
			} );
		},
		...
		_getScrollbarWidth: function() {
			...
			var majorWidth = $outer[0].clientWidth;
