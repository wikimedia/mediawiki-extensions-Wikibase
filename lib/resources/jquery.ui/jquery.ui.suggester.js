 ...
 *         Default value: 8000
 *
 * @event suggesterresponse
 *        (1) {jQuery.Event}
 *        (2) {Array} List of retrieved items.
 *
 * @event suggestererror
 *        (1) {jQuery.Event}
...
( function( $, undefined ) {
	...
	$.widget( 'ui.suggester', $.ui.autocomplete, {
		...
		_create: function() {
			...
			.on( this.widgetName + 'open', function( event ) {
			...
			.on( 'resize.' + this.widgetName, function( event ) {
				if ( event.originalEvent === undefined && $( '.ui-suggester' ).length > 0 ) {
					$( '.ui-suggester' ).data( self.widgetName ).close( {} );
				}
			} );
		},
		...
		_highlightMatchingCharacters: function() {
			...
			this.menu.element.children().each( function( i ) {
				var itemLink = $( this ).find( 'a' ),
					...
					index = itemLink.text().toLowerCase().indexOf( value.toLowerCase() );

				if ( index !== -1 ) {
					// the suggestion does not necessarily have to start with the current value
					var prefix = itemLink.text().substring( 0, index ),
						match = itemLink.text().substr( index, value.length ),
						suffix = itemLink.text().substr( index + value.length );
					itemLink
					.text( prefix )
					.append( $( '<b/>' ).text( match ) )
					.append( document.createTextNode( suffix ) );
				}
			} );
		},
		...
		autocompleteString: function( incomplete, complete ) {
			if(
				// if nothing to complete, just return and don't move the curser
