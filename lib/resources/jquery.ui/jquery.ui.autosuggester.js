...
( function( $, undefined ) {
	...
	$.widget( 'ui.autosuggester', {
		...
		_create: function() {
			...
			this.$input.on( 'keydown.' + this.widgetName, function( event ) {
				if (
					event.keyCode === $.ui.keyCode.RIGHT
					&& self.getCursorOffset() === self.$input.val().length
				) {
					self.autocompleteWithCurrentSuggestion();
				}
			} );

			this.$input.on( 'keyup.' + this.widgetName, function( event ) {
				if ( self.options.enterKeyToAutocomplete ) {
					self.autocompleteWithCurrentSuggestion();
				}
			} );
		...
		/**
		 * Auto-completes the input box with the currently suggested string.
		 */
		autocompleteWithCurrentSuggestion: function() {
			// The suggestion might be empty (e.g. if language directions of input box and
			// suggestions mismatch)
			if ( this.$suggestion.val() !== '' ) {
				this.$input.val( this.$suggestion.val() );
			}
		},

		/**
		 * Gets the character offset of the input element's text cursor.
		 *
		 * @return {Number}
		 */
		getCursorOffset: function() {
			...
			if ( document.selection ) { // IE
				...
				selection.moveStart( 'character', -this.$input.val().length );
				offset = selection.text.length;
			} else if ( this.$input[0].selectionStart || this.$input[0].selectionStart === '0' ) {
