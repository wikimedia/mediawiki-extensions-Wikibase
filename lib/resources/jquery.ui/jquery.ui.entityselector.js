...
( function( $, undefined ) {
	...
	$.widget( 'ui.entityselector', $.ui.suggester, {
			...
			if ( response.success !== undefined ) {
				...
				if ( searchTerm === this.element.val() ) {
					...
					if ( this._lastKeyDown !== 8 && response.search.length > 0 ) {
						...
						// alias.
						if ( firstMatch.aliases !== undefined && firstMatch.aliases.length > 0
							&& (
		...
		_updateValue: function() {
			...
			if ( items.length > 0 ) {
				...
				if (
					...
					||
					( firstItem.aliases !== undefined
					&& firstItem.aliases.length 
		...
		_renderItem: function( ul, item ) {
			...
			if ( item.aliases !== undefined && item.aliases.length > 0 ) {
				section.find( '.ui-entityselector-itemcontent' ).append(
