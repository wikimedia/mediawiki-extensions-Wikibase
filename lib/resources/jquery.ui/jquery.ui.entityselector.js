				...
				if ( searchTerm === this.element.val() ) {
					for ( var i in response.search ) {
						...
						if ( !response.search[i].label ) {
							response.search[i].label = response.search[i].aliases.shift();
							if ( !response.search[i].aliases.length ) {
								delete response.search[i].aliases;
							}
						}
					}
					...
					if ( this._lastKeyDown !== 8 && response.search.length > 0 ) {
						...
						// alias.
						if ( firstMatch.aliases.length > 0
							&& (
