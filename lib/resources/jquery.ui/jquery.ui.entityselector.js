 ...
 *         Default value: 8000
 *
 * @option entityStore {Object} (optional) Entity store object that can be passed to gather any
 *         entity data from instead of querying the API.
 *         Default value: null
 *
...
( function( $, undefined ) {
	...
	$.widget( 'ui.entityselector', $.ui.suggester, {
		 ...
		 *
		 * @param {String|Object} [entity] Object containing entity data or a plain entity id
		...
		selectedEntity: function( entity ) {
			if ( entity === undefined ) {
				...
				var setEntity = function( entityId, label ) {
					...
					self._trigger( 'aftersetentity', 0, [entityId] );
				};

				// If an entity store is referenced, get the entity's label from the entity store.
				// Else, fetch it from the API.
				if ( this.options.entityStore !== null ) {
					setEntity( entityId, this.options.entityStore[entityId].label );
				} else {
					var params = {
