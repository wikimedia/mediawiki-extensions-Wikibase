...
( function( $, undefined ) {
	...
	var IS_MODULE_LOADED = (
		IS_MW_CONTEXT && $.inArray( 'jquery.ui.entityselector', mw.loader.getModuleNames() ) !== -1
	);
	...
	$.widget( 'ui.entityselector', $.ui.suggester, {
		...
		_create: function() {
			...
			.on( this.widgetName + 'close.' + this.widgetName, function( event ) {
				// Since it is possible to trigger the same search again after closing the
				// suggestion list, clear the cache to avoid concatenating the same results.
				self._cache = {};
			} )
		...
		_success: function( response ) {
			...
			if ( response.success !== undefined ) {
				...
				if ( searchTerm === this.element.val() ) {
					...
					if ( !this._cache || !this._cache[searchTerm] ) {
						this._cache = {};
		...
		_resizeMenu: function() {
			...
			// Circumventing suggester's _resizeMenu() which would make the list scrollable
