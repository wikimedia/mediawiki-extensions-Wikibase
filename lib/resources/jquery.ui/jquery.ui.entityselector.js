 ...
 *         Default value: 8000
 *
 * @option wikibase {Object} (optional) Global wikibase object that can be passed if available to
 *         gather any entity data from the entity store instead of querying the API.
 *         Default value: null
 *
	...
	$.widget( 'ui.entityselector', $.ui.suggester, {
		...
		selectedEntity: function( entity ) {
			if ( entity === undefined ) {
				...
				// If wikibase is referenced, get the entity's label from the entity store. Else,
				// fetch it from the API.
				if ( this.options.wikibase !== null ) {
					setEntity( entityId, this.options.wikibase.entities[entityId].label );
				} else {
					var params = {
