 ...
 * @since 0.1
 */
var SELF = wb.ui.PropertyEditTool.EditableSiteLink = util.inherit( PARENT, {
	/**
	...
	/**
	 * The part of the editable site link representing its badges
	 * @type wb.ui.PropertyEditTool.EditableValue.SiteBadgesInterface
	 */
	siteBadgesInterface: null,
	...
	_init: function( subject, options, interfaces, toolbar ) {
		if( interfaces.length < 2
			...
			|| !( interfaces[2] instanceof wb.ui.PropertyEditTool.EditableValue.SiteBadgesInterface )
		) {
		...
		this.siteBadgesInterface = interfaces.badges = interfaces[2]; // this._interfaces.badges
	...
	_interfaceHandler_onInputRegistered: function( relatedInterface ) {
		...
		var idInterface = this._interfaces.siteId,
			...
			badgesInterface = this._interfaces.badges;
		...
		if( site !== pageInterface.getSite() && site !== null ) {
			...
			badgesInterface._getValueContainer().removeClassByRegex( /^wb-sitelinks-badges-.+/ );
			badgesInterface._getValueContainer().addClass( 'wb-sitelinks-badges wb-sitelinks-badges-' + siteId );
		}
	...
	_getValueFromApiResponse: function( response ) {
		var siteId = this._interfaces.siteId.getSelectedSite().getId(),
			normalizedTitle = response[ this.API_VALUE_KEY ][ this._interfaces.siteId.getSelectedSite().getId() ].title,
			badges = response[ this.API_VALUE_KEY ][ this._interfaces.siteId.getSelectedSite().getId() ].badges;

		return [ siteId, normalizedTitle, badges ];
	},
	...
	_setRevisionIdFromApiResponse: function( response ) {
		wb.getRevisionStore().setSitelinksRevision( response.lastrevid, this._interfaces.siteId.getSelectedSite().getId() );
		return true;
	},
	 ...
	 *
	 * @param {boolean} save Whether to save the current value.
	 * @return jQuery.Promise
	 */
	stopEditing: function( save ) {
		var self = this,
			promise = PARENT.prototype.stopEditing.call( this, save );
		...
		// toolbar display the "remove" button after having saved a value successfully.
		promise.done( function() {
			if ( self._interfaces !== null ) {
				self._interfaces.siteId.setActive( self.isPending() );

				var editGroup = self._toolbar.$editGroup.data( 'toolbareditgroup' );
				editGroup.option( 'displayRemoveButton', true );
			}
		} );
		return promise;
	},
	...
	queryApi: function( apiAction ) {
		var remove = ( apiAction === this.API_ACTION.REMOVE || apiAction === this.API_ACTION.SAVE_TO_REMOVE );
		return this._api.setSitelink(
			...
			remove ? '' : this.getValue()[1],
			remove ? '' : this.badgesInterface.getBadges() // this.getValue()[2] ?
		);
	},
...
SELF.newFromDom = function( subject, options, toolbar, /** internal */ Constructor ) {
	...
	var $subject = $( subject ),
		...
		iSiteBadges = new ev.SiteBadgesInterface(),
	...
	// interface for setting badgse
	iSiteBadges.init(
		$tableCells[ $tableCells.length - 3 ], // @todo: create a new table cell or put it into the page cell?
		{ inputPlaceholder: mw.msg( 'wikibase-sitelink-badge-edit-placeholder' ) }
	);

	newEV.init( $subject, options, [ iSiteId, iSitePage, iSiteBadges ], toolbar );
	return newEV;
};
