...
SELF.newFromDom = function( subject, options, toolbar, /** internal */ Constructor ) {
	...
	$iSitePageSubject = $tableCells.eq( $tableCells.length - 2 );

	// Only select the child span.wb-sitelinks-page (if present)
	$iSitePageSubject = $iSitePageSubject.find( '.wb-sitelinks-page' ).length === 1 ?
		$iSitePageSubject.find( '.wb-sitelinks-page' ) : $iSitePageSubject;
