 ...
 * @since 0.1
 */
SELF = wb.ui.PropertyEditTool.EditableSiteLink = wb.utilities.inherit( PARENT, {
	/**
	 ...
	 */
	_init: function( subject, options, interfaces ) {
	...
	_interfaceHandler_onInputRegistered: function( relatedInterface ) {
		...
		var idInterface = this._interfaces.siteId,
			...
			site = idInterface.getSelectedSite(),
			siteId;
		...
		if( site !== pageInterface.getSite() && site !== null ) {
			...
			siteId = idInterface.getSelectedSiteId();
	...
	performApiAction: function( apiAction ) {
		...
		if ( apiAction !== this.API_ACTION.REMOVE ) {
			...
			promise.done( function( response ) {
				...
				if( page !== '' && site !== null ) {
					url = response.entity.sitelinks[ site.getGlobalSiteId() ].url;
					oldFn = site.getUrlTo;
