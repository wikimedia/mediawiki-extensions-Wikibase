 ...
 *         Default value: 'more'
 *
 * @option emulateSearch {boolean} (optional) Allows emulating the behaviour of a search box by
 *         linking the entities to their corresponding pages. Instead of selecting and entity, the
...
( function( $, undefined ) {
	...
	$.widget( 'wikibase.entityselector', $.ui.suggester, {
		...
		options: {
			...
			emulateSearch: false // TODO: Allow setting a custom target to trigger the redirect on
		},
		...
		_create: function() {
			...
			.on( 'keydown.' + this.widgetName, function( event ) {
				...
				if ( event.keyCode === $.ui.keyCode.ENTER ) {
					...
					if ( self.options.emulateSearch && self.selectedEntity() ) {
		...
		close: function( event ) {
			if ( !event || !event.originalEvent || event.originalEvent.type !== 'programmatic' ) {
				...
				$.ui.suggester.prototype.close.apply( this, event );
			}
		},
		...
		validateInput: function() {
			...
			if ( !found && !alreadySelected && !this.options.emulateSearch ) {
				this._setEntity( null );
			}
			return found;
		},
