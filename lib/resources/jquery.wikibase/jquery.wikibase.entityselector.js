...
( function( $, mw ) {
	...
	$.widget( 'wikibase.entityselector', $.ui.suggester, {
		...
		_create: function() {
			...
			var self = this,
				fnNativeMenuFocus, fnNativeMenuSelected;

			this.element
			.eachchange( function() {
				self._updateValue();
			} )
			.on( this.widgetName + 'refreshmenu.' + this.widgetName, function() {
			...
			// no label.
			fnNativeMenuFocus = this.menu.option( 'focus' );
			...
			fnNativeMenuSelected = this.menu.option( 'selected' );
		...
		_success: function( response ) {
			...
			if ( response.success !== undefined ) {
				var suggest = this._response(),
					...
					i, firstMatch, stringToAutocomplete;
				...
				if ( searchTerm === this.element.val() ) {
					...
					for ( i in this._cache[searchTerm] ) {
		...
		_updateValue: function() {
			...
			if ( items.length > 0 ) {
				firstItem = items.first().data( 'item.autocomplete' ),
