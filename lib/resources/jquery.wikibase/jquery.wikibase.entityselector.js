...
( function( $, mw ) {
	...
	$.widget( 'wikibase.entityselector', $.ui.suggester, {
		...
		_create: function() {
			...
			this.element
			.on( 'eachchange.' + this.widgetName, function( event, oldVal ) {
				self._updateValue();
			} )
		...
		close: function( event ) {
			...
			if (
				...
				// Reposition to be sure the menu is aligned correctly (e.g. on window's resize
				// event):
				this.repositionMenu();
		...
		_success: function( response ) {
			...
			if ( response.success !== undefined ) {
				...
				if ( searchTerm === this.element.val() ) {
					// Reset this.term to the actual search term. Native jquery.autocomplete
					// sets this.term to this.element.val() in search(). Auto-completing the string
					// to the first search hit's label results in setting this.term to the whole
					// label.
					...
					if (
						...
						var firstMatch = this._cache[searchTerm][0],
						...
						// alias.
						if ( this._shouldAutocompleteOnAlias( firstMatch, searchTerm ) ) {
							stringToAutocomplete = firstMatch.aliases[0];
						}

						if( this.options.adaptLetterCase ) {
							this.term = searchTerm = $.util.adaptlettercase(
								searchTerm,
						...
						this.element.autocompletestring( searchTerm, stringToAutocomplete );
						...
						if (
							...
							// Do not select when fetching more results since it would trigger
							// another API call.
							&& initialOffset === 0
						) {
		...
		_shouldAutocompleteOnAlias: function( firstMatch, searchTerm ) {
			...
			return !label
				...
				|| this._labelStartsWithAlias( label, firstMatch );
		},

		_labelStartsWithAlias: function( label, firstMatch ) {
			// If the label string starts with one of the alias strings,
		...
		_setEntity: function( entity ) {
			...
			this._trigger( 'aftersetentity', 0, [ entity ? entity.id : null ] );
		},
			...
			if ( entity === undefined ) {
				...
				.done( function ( result ) {
					...
					if ( result.error ) {
						...
						result.entities[entityId] && result.entities[entityId].labels !== undefined
					) {
						// the entity exists and has a label in the language the entity selector has been
						// initialised with
