...
$.widget( 'wikibase.statementview', PARENT, {
	...
	*/
	...
	_create: function() {
		...
		if( statement ) {
			var refs = statement.getReferences();
	...
	stopEditing: $.NativeEventHandler( 'stopEditing', {
		...
		natively: function( e, dropValue ) {
			...
			function stopEditing() {
				...
				// If the claimview has it's initial value, it refuses to leave edit mode.
				// Set dropValue = true in this case, so that it really leaves edit mode.
				self._claimview.stopEditing( self._claimview.isInitialValue() || dropValue );
				self.enable();
