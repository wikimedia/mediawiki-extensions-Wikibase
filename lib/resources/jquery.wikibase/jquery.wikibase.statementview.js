...
$.widget( 'wikibase.statementview', PARENT, {
	...
	_create: function() {
		...
		if( this.value() ) {
			...
			} ).on( 'listviewlistitemadded listviewlistitemremoved', function( e, value, $li ) {
	...
	drawReferencesCounter: function() {
		var count = this.$references.listview( 'nonEmptyListItems' ).length,
			$toggle = wb.utilities.ui.buildToggle(
				mw.msg( 'wikibase-statementview-referencesheading', count ),
				this.$references
			);
		this.$refsHeading.empty().append( $toggle );
	}
} );
