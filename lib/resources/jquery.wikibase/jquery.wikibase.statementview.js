...
$.widget( 'wikibase.statementview', PARENT, {
		...
		if( this.value() ) {
			...
			.on( 'referenceviewafterstopediting', function( event, dropValue ) {
				if( dropValue ) {
					...
					var referenceListview = self.$references.data( 'listview' ),
	...
	_updateReferenceIndices: function() {
		var $referenceviews = this.$references.data( 'listview' ).items();
...
$.wikibase.toolbarcontroller.definition( 'addtoolbar', {
	...
	events: {
		...
		listviewdestroy: function( event, toolbarController ) {
			var $listview = $( event.target ),
				$node = $listview.parent();
	...
	events: {
		'referenceviewstartediting': function( event ) {
			...
			var $referenceview = $( event.target ),
				...
				referencesListview = statementview.$references.data( 'listview' );
		...
		'referenceviewafterstopediting': function( event ) {
			...
			var $referenceview = $( event.target ),
				movetoolbar = $referenceview.data( 'movetoolbar' );

			if( movetoolbar ) {
				// Toolbar might be destroyed already by cancelling a pending referenceview.
				movetoolbar.destroy();
			}
		},
