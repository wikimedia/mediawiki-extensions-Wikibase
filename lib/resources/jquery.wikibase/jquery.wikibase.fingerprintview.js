...
$.widget( 'wikibase.fingerprintview', PARENT, {
	...
	destroy: function() {
		...
		function degrade() {
			...
			if( self.$aliases ) {
				self.$aliases.data( 'aliasesview' ).destroy();
			}
	...
	_createWidgets: function() {
		...
		$.each( ['label', 'description', 'aliases'], function( i, subjectName ) {
			...
			// Remove events that would trigger startItemPageEditMode/stopItemPageEditMode:
			// TODO: Once this widget is managed by entityview, handling those events should be done
			// there.
			$subject.off( '.' + widgetName );
		} );
	},
	...
	isValid: function() {
		return this.$label.data( 'labelview' ).isValid()
			...
			&& this.$aliases.data( 'aliasesview' ).isValid();
	},
	...
	isInitialValue: function() {
		return this.$label.data( 'labelview' ).isInitialValue()
			...
			&& this.$aliases.data( 'aliasesview' ).isInitialValue();
	},
	...
	startEditing: function() {
		...
		this.$aliases.data( 'aliasesview' ).startEditing();
	...
	stopEditing: function( dropValue ) {
		...
		var labelview = this.$label.data( 'labelview' ),
			...
			aliasesview = this.$aliases.data( 'aliasesview' ),
			stoppedEditing = 0;
		...
		this.$aliases.one( 'aliasesviewafterstopediting', detectAfterStopEditing );
	...
	value: function( value ) {
		...
		return {
			...
			aliases: this.$aliases.data( 'aliasesview' ).value().aliases
		};
	},
	...
	_setOption: function( key, value ) {
		if( key === 'value' ) {
			...
			this.$aliases.data( 'aliasesview' ).option( 'value', {
				language: value.language,
