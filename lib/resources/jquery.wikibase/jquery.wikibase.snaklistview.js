 ...
 *         Default: null
 *
 * @option removeFirstSnak {boolean} Defines whether it should be possible to remove the first snak
 *         just like the other snaks by displaying a "remove" link next to the snakview. If false,
 *         there will be no "remove" link next to the first snak which allows an other toolbar
 *         to be diplayed in that location.
 *         Default: true
 *
...
$.widget( 'wikibase.snaklistview', PARENT, {
	...
	createListView: function() {
		...
		.on( self._lia.prefixedEvent( 'stopediting' ),
			function( event, dropValue, newSnak ) {
				if ( !self.isValid() && !dropValue && !self.__continueSnakviewStopEditing ) {
	...
	startEditing: $.NativeEventHandler( 'startEditing', {
		...
		natively: function( e ) {
			...
			$.each( this._listview.items(), function( i, item ) {
				...
				if ( self.option( 'removeFirstSnak' ) || i > 0 ) {
					self._createRemoveToolbar( $( item ) );
				}
			} );

			// Append "add" toolbar to add new snaks:
			this.$addToolbar = mw.template( 'wb-toolbar', '' ).appendTo( this.element );
	...
	stopEditing: $.NativeEventHandler( 'stopEditing', {
		initially: function( e, dropValue ) {
			if(
				!this.isInEditMode()
				|| !this.isValid() && !dropValue && !this.__continueStopEditing
			) {
				e.cancel();
			}
	...
	isValid: function() {
		...
		return ( isValid && ( !this._snakList || !this._snakList.equals( snakList ) ) );
	},
	...
	enterNewItem: function() {
		...
		if ( this._listview.items().length > 1 ) {
