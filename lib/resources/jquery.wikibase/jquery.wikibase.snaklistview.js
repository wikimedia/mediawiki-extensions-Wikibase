...
$.widget( 'wikibase.snaklistview', PARENT, {
	...
	/**
	 * Updates the state (dis-/enabled) of the first list item's "remove" toolbar. If there is only
	...
	_updateRemoveToolbarState: function() {
		...
		var firstItemToolbar = $( this._listview.items()[0] ).data( 'removetoolbar' );
		if ( firstItemToolbar ) {
			firstItemToolbar[ ( this._listview.items().length === 1 ) ? 'disable' : 'enable' ]();
		}
	},
