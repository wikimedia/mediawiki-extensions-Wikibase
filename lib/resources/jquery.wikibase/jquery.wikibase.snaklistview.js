 ...
 *        (1) {jQuery.Event} event
 *
 * @event enabled: Triggered whenever the snaklistview gets enabled.
 *        (1) {jQuery.Event} event
 *
...
$.widget( 'wikibase.snaklistview', PARENT, {
	...
	stopEditing: $.NativeEventHandler( 'stopEditing', {
		...
		natively: function( e, dropValue ) {
			...
			if ( dropValue ) {
			...
			} else if ( this.option( 'value' ) !== null && this.__continueStopEditing ) {
				var self = this;
				...
				this._trigger( 'afterstopediting', null, [ dropValue ] );
			} else if ( !this.option( 'value' ) && !this.__continueStopEditing ) {
