	...
	focus: function( considerViewPort ) {
		...
		function filterInViewPort( $nodes ) {
			...
			var $filtered = $nodes.filter( function() {
				var $node = $( this );
				return $node.is( ':visible' ) && $node.offset().top > $window.scrollTop();
			} );
		...
		if( this.isValid() && $items.length ) {
			$toFocus = lia.liInstance(
				( considerViewPort ? filterInViewPort( $items ) : $items ).first()
			);
		} else {
			...
			$toFocus = lia.liInstance(
				( considerViewPort ? filterInViewPort( $invalid ) : $invalid ).first()
			);
		}
