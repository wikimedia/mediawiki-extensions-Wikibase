...
$.widget( 'wikibase.sitelinklistview', PARENT, {
	...
	_updateAutoInput: function() {
		var listview = this.$listview.data( 'listview' ),
			lia = listview.listItemAdapter(),
			$items = listview.items(),
			emptyCount = $.grep( $items, function( sitelinkview ) {
				return $( sitelinkview ).data( 'sitelinkview' ).isEmpty();
			} ).length,
		...
		if(
			...
			&& ( emptyCount > 1 || secondToLastInvalidPending )
		) {
			listview.removeItem( $lastSitelinkview );
		} else if( emptyCount === 0 && !this.isFull() ) {
			this.enterNewItem();
		}
	},
