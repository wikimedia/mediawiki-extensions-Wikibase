...
$.widget( 'wikibase.entitytermsforlanguage', PARENT, {
	options: {
		...
		templateShortCuts: {
			...
			$description : '.wikibase-entitytermsforlanguage-description',
	...
	stopEditing: function( dropValue ) {
		...
		function addStopEditToQueue( $queue, widget, dropValue ) {
			$queue.queue( 'stopediting', function( next ) {
				widget.element
				.one(
					widget.widgetEventPrefix + 'afterstopediting.entitytermsforlanguagestopediting',
					function( event ) {
						widget.element.off( '.entitytermsforlanguagestopediting' );
						setTimeout( next, 0 );
					}
				)
				.one(
					widget.widgetEventPrefix + 'toggleerror.entitytermsforlanguagestopediting',
					function( event ) {
						widget.element.off( '.entitytermsforlanguagestopediting' );
						$queue.clearQueue();
						self._resetEditMode();
