...
( function( $ ) {
	...
	$.widget( 'wikibase.' + WIDGET_NAME, {
		...
		update: function() {
			...
			var $claimGroup = $firstVisibleMainSnak.closest( '.wb-claim-section' ),
				$claimGroupLabel =
					$claimGroup.children( '.wb-claim-section-name' ).children( '.wb-claim-name' );

			this._log( 'positioning', $claimGroupLabel.get( 0 ), 'on', $firstVisibleMainSnak.get( 0 ) );

			var newLabelPosition =
				positionElementInOneLineWithAnother( $claimGroupLabel, $firstVisibleMainSnak );
	 ...
	 *
	 * @returns $.wikibase.claimgrouplabelscroll[]
	 */
	...
	/**
	 * Checks an Claim Group's element for Main Snak elements and returns the first one visible in
	 ...
	 * @returns {null|jQuery}
	 */
	function findFirstVisibleMainSnakElementWithinClaimList( $element ) {
		var $claimGroups = $element.find( '.wb-claim-section' ),
			result = null;
		...
		$claimGroups.each( function( i, claimGroupNode ) {
			if( elementPartlyVerticallyInViewport( claimGroupNode ) ) {
				result = findFirstVisibleMainSnakElement( $( claimGroupNode ) );
				if( result ) {
					return false;
				}
			}
	 ...
	 * @param {jQuery} $target
	 * @returns {false|string} false if the position requires no update, otherwise the string of
