...
( function( $ ) {
		...
		update: function() {
			...
			for( var i = 0; i < $visibleClaims.length; i++ ) {
				var $visibleClaim = $visibleClaims.eq( i ),
					$claimGroup = $visibleClaim.closest( '.wikibase-statementlistview' ),
					$claimNameSection
						= $claimGroup.children( '.wikibase-statementgrouplistview-groupname' ),
					$claimGroupLabel = $claimNameSection.children(
				...
				positionElementInOneLineWithAnother( $claimGroupLabel, $visibleClaim, $claimGroup );
	...
	function findFirstVisibleMainSnakElementsWithinClaimList( $searchRange ) {
		var $claimGroups = $searchRange.find( '.wikibase-statementlistview' ),
			$visibleClaimGroups = $();
	...
	function findFirstVisibleMainSnakElement( $searchRange ) {
		...
		// Caring about the visibility of ".wb-snak-value-container" is better than about ".wb-statement"
		// or ".wb-claim-mainsnak" since the label will align with the .wb-snak-value-container.
		var $mainSnaks
			= $searchRange.find( '.wikibase-statementview-mainsnak .wb-snak-value-container' );
