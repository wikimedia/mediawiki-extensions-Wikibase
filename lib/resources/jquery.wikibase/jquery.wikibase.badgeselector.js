...
$.widget( 'wikibase.badgeselector', PARENT, {
	...
	destroy: function() {
		...
		this._detachGlobalEventListeners();
	...
	_attachEventHandlers: function() {
		...
		.on( 'click.' + this.widgetName, function( event ) {
			...
			if( $menu && $menu.data( self.widgetName ) === self && $menu.is( ':visible' ) ) {
				$menu.hide();
				self._detachGlobalEventListeners();
				return;
			}
			...
			.done( function() {
				...
				var degrade = function( event ) {
					if( !$( event.target ).closest( $menu ).length ) {
						$menu.hide();
						$menu.data( self.widgetName ).element.removeClass( 'ui-state-active' );
						self._detachGlobalEventListeners();
					}
				};
	...
	_detachGlobalEventListeners: function() {
	...
	_initMenu: function() {
		...
		.done( function() {
			$menu.children( 'li' ).each( function() {
				...
				$li[$.inArray( badgeId, self.value() ) === -1
	...
	_fillMenu: function() {
		...
		.done( function() {
			...
			$.each( self.options.badges, function( itemId, cssClasses ) {
				...
				.append(
					$( '<a/>' )
					.text( item.getLabel( self.options.languageCode ) )
					.on( 'click.' + self.widgetName, function( event ) {
						event.preventDefault();
					} )
					.prepend( mw.template( 'wb-badge',
						itemId + ' ' + cssClasses,
						item.getLabel( self.options.languageCode ) || itemId,
						itemId
					) )
				)
				.appendTo( $menu );
	...
	_toggleBadge: function( badgeId ) {
		var $li = $menu.find( '.' + this.widgetBaseClass + '-menuitem-' + badgeId ),
			widget = $menu.data( this.widgetName );

		if( $li.hasClass( 'ui-state-active' ) ) {
			widget.element.children( '.wb-badge-' + badgeId ).remove();
			if( !widget.element.children( '.wb-badge' ).length ) {
				$.proxy( this._addEmptyBadge, widget )();
			}
		} else {
			$.proxy( this._addBadge, widget)( badgeId );
			$.proxy( this._getEmptyBadge, widget )().remove();
		}
	...
	_addBadge: function( badgeId ) {
		this.element.append(
			mw.template( 'wb-badge',
				...
				badges[badgeId].getLabel( this.options.languageCode ) || badgeId,
				badgeId
			)
		);
	},
