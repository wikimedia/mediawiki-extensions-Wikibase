...
$.widget( 'wikibase.badgeselector', PARENT, {
	...
	_fillMenu: function() {
		...
		.done( function() {
			...
			$.each( self.options.badges, function( itemId, cssClasses ) {
				...
				if( item ) {
					$item.text( item.getLabelFor( self.options.languageCode ) || item.getId() );
				} else {
					$item.append(
				...
				.append( $item
					.prepend( mw.wbTemplate( 'wb-badge',
						itemId + ' ' + cssClasses,
						( item && item.getLabelFor( self.options.languageCode ) ) || itemId,
						itemId
					) )
				)
				.appendTo( $menu );
	...
	_addBadge: function( badgeId ) {
		...
		var $badge = mw.wbTemplate( 'wb-badge',
			badgeId + ' ' + this.options.badges[badgeId],
			badgeItem && badgeItem.getLabelFor( this.options.languageCode ) || badgeId,
			badgeId
		);
