...
$.widget( 'wikibase.entityview', PARENT, {
	...
	_initFingerprints: function() {
		...
		if( !this.$fingerprints.length ) {
			...
			this.$fingerprints.find( '.wikibase-entitytermsforlanguage' ).each( function() {
				$.each( $( this ).attr( 'class' ).split( ' ' ), function() {
					if( this.indexOf( 'wikibase-entitytermsforlanguage-' ) === 0 ) {
						self.options.languages.push(
							this.split( 'wikibase-entitytermsforlanguage-' )[1]
						);
						return false;
					}
				} );
