	...
	_handleEditModeAffairs: function() {
		...
		var toolbarStatesSetter = function( action ) {
			return function( event, origin ) {
				...
				var originToolbars = ( origin ) ? $( origin ).find( '.wb-ui-toolbar' ) : null;
		...
		$( this.element )
		.on( 'statementviewafterstartediting referenceviewafterstartediting', function( event ) {
			$( wb ).trigger( 'startItemPageEditMode', [event.target] );
		} )
		...
		.on( 'statementviewafterstopediting statementviewafterremove '
				+ 'referenceviewafterstopediting referenceviewafterremove',
			function( event ) {
				$( wb ).trigger( 'stopItemPageEditMode', [event.target] );
			}
		);
