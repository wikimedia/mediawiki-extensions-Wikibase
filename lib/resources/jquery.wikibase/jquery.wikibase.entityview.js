...
$.widget( 'wikibase.entityview', PARENT, {
		...
		this.$label.labelview( {
			value: {
				...
				label: this.$label.hasClass( 'wb-empty' )
					...
					: this.options.value.getFingerprint().getLabelFor( mw.config.get( 'wgUserLanguage' ) )
			},
			helpMessage: mw.msg(
		...
		this.$description.descriptionview( {
			value: {
				...
				description: this.$description.hasClass( 'wb-empty' )
					? null
					// FIXME: entity object should not contain fallback strings
					: this.options.value.getFingerprint().getDescriptionFor( mw.config.get( 'wgUserLanguage' ) )
			},
			helpMessage: mw.msg(
				'wikibase-description-input-help-message',
				wb.getLanguageNameByCode( mw.config.get( 'wgUserLanguage' ) )
	...
	_initAliases: function() {
		...
		this.$aliases.aliasesview( {
			value: {
				language:  mw.config.get( 'wgUserLanguage' ),
				aliases: this.options.value.getFingerprint().getAliasesFor( mw.config.get( 'wgUserLanguage' ) )
			},
			entityId: this.options.value.getId(),
			api: this.options.api
		} );
	...
	_initFingerprints: function() {
		...
		for( var i = 0; i < this.options.languages.length; i++ ) {
			value.push( {
				...
				label: this.options.value.getFingerprint().getLabelFor( this.options.languages[i] ) || null,
				description: this.options.value.getFingerprint().getDescriptionFor( this.options.languages[i] ) || null,
