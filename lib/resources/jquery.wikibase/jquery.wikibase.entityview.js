...
$.widget( 'wikibase.entityview', {
	...
	_create: function() {
		...
		var $claimsHeading =
			$( mw.template( 'wb-section-heading', mw.msg( 'wikibase-statements' ), this.option( 'value' ).id+'/statements' ) );
	...
	_handleEditModeAffairs: function() {
		...
		.on( 'snakviewafterstopediting claimviewremove statementviewremove referenceviewremove', function( event ) {
			$( wb ).trigger( 'stopItemPageEditMode' );
		} );
	}
} );
