...
$.widget( 'wikibase.snakview', PARENT, {
	...
	destroy: function() {
		...
		if( snakTypeSelector ) {
			snakTypeSelector.data( 'snaktypeselector' ).destroy();
	...
	drawSnakTypeSelector: function() {
		...
		if( !selector ) {
			...
			this.$snakTypeSelector.append( $selector );
			selector = $selector.data( 'snaktypeselector' );
		}

		selector.selectSnakType( this.snakType() );
	},
	...
	_buildSnakTypeSelector: function() {
		var self = this,
			$anchor = $( '<span/>' ),
			// initiate type selector widget which is a normal widget just
			// without a jQuery.widget.bridge...
			widget = new $.wikibase.snakview.TypeSelector( {}, $anchor );

		// ...add the data information nevertheless:
		$anchor.data( 'snaktypeselector', widget );
