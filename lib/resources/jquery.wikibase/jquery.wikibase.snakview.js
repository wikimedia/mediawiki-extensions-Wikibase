...
$.widget( 'wikibase.snakview', PARENT, {
	...
	_buildSnakTypeSelector: function() {
		var self = this,
			$anchor = $( '<span/>' ),
			$menu = $( '<ul/>' ).addClass( 'wb-snaktype-selector-menu' );

		$.each( $.wikibase.snakview.variations.getCoveredSnakTypes(), function( i, type ) {
			$menu.append(
				$( '<li/>' )
				.data( 'snakType', type )
		...
		$menu.appendTo( 'body' ).hide().menu();

		/**
		 * Disables a given snak type while enabling all others.
		 *
		 * @param {String} snakType
		 */
		$menu.data( 'menu' ).disableSnakType = function( snakType ) {
			this.element.children( 'li' ).each( function( i ) {
				var $linkNode = $( this ).children( 'a' ).first();
				if ( $( this ).data( 'snakType' ) === snakType ) {
					$( this ).addClass( 'ui-state-disabled' );
					$linkNode.off( 'click' );
				} else {
					$( this ).removeClass( 'ui-state-disabled' );
					$linkNode.on( 'click', function( event ) {
						var selectedSnakType = $( this ).closest( 'li' ).data( 'snakType' );
						// TODO: Actually set the Snak type to update the variation.
						self._updateVariation();
						$menu.data( 'menu' ).disableSnakType( selectedSnakType );
					} );
				}
			} );
		};

		$menu.data( 'menu' ).disableSnakType( this.snakType() );

		/**
		 * Repositions the menu.
		 */
		$menu.data( 'menu' ).reposition = function() {
			var isRtl = $( 'body' ).hasClass( 'rtl' );

			this.element.position( {
				of: $anchor,
				my: ( isRtl ) ? 'left top' : 'right top',
		...
		.data( 'menu', $menu )
		.on( 'click.wb-snaktype-selector', function( event ) {
			if ( $menu.is( ':visible' ) ) {
				$menu.hide();
			} else {
				$menu.show();

				$menu.data( 'menu' ).reposition();

				$( document )
				.on( 'mouseup.wb-snaktype-selector', function( event ) {
					if ( event.srcElement !== $anchor[0] ) {
						$menu.hide();
					}
					$( this ).off( '.wb-snaktype-selector' );
				} );
			}
		} );

		$( window ).on( 'resize.wb-snaktype-selector', function( event ) {
			$menu.data( 'menu' ).reposition();
		} );
	 ...
	 */
	destroy: function() {
		if ( this.$snakTypeSelector.data( 'wb-snaktype-selector' ) ) {
			this.$snakTypeSelector.data( 'wb-snaktype-selector' ).data( 'menu' ).remove();
			this.$snakTypeSelector.empty().removeData( 'wb-snaktype-selector' );
		}
		$( window ).off( '.wb-snaktype-selector' );
	...
	stopEditing: $.NativeEventHandler( 'stopEditing', {
		...
		initially: function( e, dropValue ) {
			...
			if( dropValue ) {
				...
				snak = this._variation.newSnak( this._getPropertyId() );
			}
		...
		natively: function( e, dropValue, newSnak ) {
			...
			this._setValue( newSnak ); // triggers this.draw()
	...
	initialValue: function() {
		return this.isInEditMode() ? this._initialSnak : this._getValue();
	},
	...
	drawSnakTypeSelector: function() {
		...
		this.$snakTypeSelector.append( $selector ).data( 'wb-snaktype-selector', $selector );
	},
	...
	drawVariation: function() {
		...
		if( propertyId && this._variation ) {
			this.variation().draw( this.isInEditMode(), propertyId, this._getValue() );
		} else {
