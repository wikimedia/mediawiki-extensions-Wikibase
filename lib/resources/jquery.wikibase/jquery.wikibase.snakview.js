...
$.widget( 'wikibase.snakview', {
	...
	_create: function() {
		...
		this.$snakTypeSelector = this.element.find( '.wb-snak-typeselector' );
	...
	_buildSnakTypeSelector: function() {
		return $( '<div/>' ); // TODO: implement something useful
	},
	...
	stopEditing: $.NativeEventHandler( 'stopEditing', {
		...
		natively: function( e, dropValue ) {
			...
			if( !this._variation ) {
				// no variation, e.g. because no valid data type is chosen
				this.draw();
			} else {
	 ...
	 */
	draw: function() {
		// NOTE: never move this further down, if for any reasons draw functions start changing the
		//  outcome of the variation (or Snak type), then something must be incredibly wrong!
		this.drawProperty();
