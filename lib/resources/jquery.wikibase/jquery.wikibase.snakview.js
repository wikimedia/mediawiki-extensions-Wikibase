		...
		natively: function( e ) {
			...
			this.element.on( 'keydown', function( event ) {
				...
				if ( event.keyCode === $.ui.keyCode.ESCAPE ) {
					...
					if ( propertySelector && event.target === propertySelector.element[0] ) {
						// Have the current property input validated instead of just blurring since
						// the value input element will not appear if the property is not valid and
						// setting the focus would remain unresolved.
