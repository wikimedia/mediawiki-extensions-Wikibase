	...
	$.widget( 'wikibase.siteselector', $.ui.suggester, {
		...
		_getSuggestionsFromArray: function( term, source ) {
			...
			if ( suggestedSites.length === 0 ) {
				var siteId = this._grepSiteIdFromTerm();
				if ( siteId ) {
					suggestedSites = $.grep( source, function( site ) {
						return site.getId().indexOf( siteId ) === 0;
					} );
				}
			}
		 ...
		 * @private
		 */
		_grepSiteIdFromTerm: function() {
			// This finds "zh-min-nan" in "https://zh-min-nan.wikipedia.org".
			var matches = /\W(\w[\w-]+)/.exec( this._term );
			return matches ? matches[1].toLowerCase().replace( /[\W_]+/g, '_' ) : null;
		},
