...
( function( $, wb, undefined ) {
	...
	$.widget( 'wikibase.siteselector', $.ui.suggester, {
		...
		_create: function() {
			...
			this.element.on( this.widgetName + 'open', function() {
				self.menu.activate(
			...
			this.menu.element.on( 'menublur', function() {
		...
		_request: function( request, suggest ) {
			var i,
				...
				results = $.grep( this.options.resultSet, function( result ) {
					return (
				...
				additionallyFiltered = $.grep( results, function( result ) {
		...
		_highlightMatchingCharacters: function() {
			...
			this.menu.element.children().each( function() {
		...
		getSelectedSiteId: function() {
			...
			for ( i in this.options.resultSet ) {
				// search the site which matches the input string in any way
				currentItem = this.options.resultSet[i];
