...
( function( $, wb ) {
	 ...
	 *
	 * @option resultSet {wb.Site[]} Set of sites that shall be filtered for any result(s).
	 */
	$.widget( 'wikibase.siteselector', $.ui.suggester, {
		...
		_request: function( request, suggest ) {
			...
			// If some site id or language code is specified exactly, move that site / those sites
			// to the top. The uppermost one be the one picked when leaving the input field.
			var filteredById = $.grep( results, function( result, i ) {
				return ( request.term === result.site.getId() );
			} );

			var filteredByLanguageCode = $.grep( results, function( result, i ) {
				return ( request.term === result.site.getLanguageCode() );
			} );

			// Placing sites hit by id before sites hits by language code:
			var directlyHitSites = $.merge( filteredById, filteredByLanguageCode );

			// Remove directly hit sites from original result set:
			if( directlyHitSites.length ) {
				for( var i in results ) {
					for( var j in directlyHitSites ) {
						if ( results[i].site.getId() === directlyHitSites[j].site.getId() ) {
							results.splice( i, 1 );
							break;
						}
					}
			...
			// Put site(s) exactly hit to the top of the complete result set:
		...
		getSelectedSiteId: function() {
			...
			for ( var i in this.options.resultSet ) {
				...
				if ( value === currentItem.site.getId().toLowerCase()
					...
					|| value === currentItem.site.getLanguageCode()
				) {
