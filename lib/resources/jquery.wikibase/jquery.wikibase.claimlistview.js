...
$.widget( 'wikibase.claimlistview', PARENT, {
	...
	_create: function() {
		...
		.on( 'snakviewafterstopediting snakviewstartediting ' + lmwRemoveEvent, function( e ) {
		...
		$newClaim.on( 'snakviewstopediting', function( event, dropValue, newSnak ) {
			...
			if( dropValue || !newSnak ) {
				...
				.fail( function( errorCode, details ) {
					var $newClaim = self.element.find( '.wb-claim-new' ).filter( '.' + self._lmwClass() ),
						claimview = self._lmwInstance( $newClaim ),
