...
$.widget( 'wikibase.claimlistview', PARENT, {
	...
	_insertClaimRow: function( claimSectionPropertyId, $content ) {
		...
		var $claimviews = $claimSection.children( '.wb-claimview' );
	...
	enterNewClaim: function( sectionPropertyId ) {
		...
		.on( this._lmwEvent( 'afterstopediting' ), function( event, dropValue ) {
			...
			if( dropValue || !snak ) {
				...
				var newClaimWithGuid = self._lmwInstance( $newClaim ).value();
	...
	events: {
		statementviewchange: function( event ) {
			var $target = $( event.target ),
				...
				snaklistviews = ( statementview._qualifiers ) ? statementview._qualifiers.value() : [],
				areInitialQualifiers = true;

			// Statementview's isValid() validated the qualifiers already. However, the information
			// whether all qualifiers (grouped by property) have changed, needs to be gathers
			// separately:
			...
			if( areInitialQualifiers && statementview.isInitialValue() ) {
				enable = false;
			}

			$btnSave.data( 'toolbarbutton' )[ enable ? 'enable' : 'disable' ]();
		}
	},
	options: {
