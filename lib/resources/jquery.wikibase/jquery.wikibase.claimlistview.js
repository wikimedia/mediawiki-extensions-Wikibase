...
$.widget( 'wikibase.claimlistview', {
	...
	_create: function() {
		...
		this.element.on( 'snakviewstopediting snakviewstartediting', function( e ) {
			var command = e.type === 'snakviewstopediting' ? 'removeClass' : 'addClass';
			$( e.target ).parents( '.wb-claim-section' )[ command ]( 'wb-edit' );
		} );
	},
	...
	_serveClaimSection: function( mainSnakPropertyId ) {
		...
		if( $section.length === 0 ) {
			...
			$section = $( mw.template( 'wb-claim-section',
				...
				( property.label ) ? property.label : mw.msg( 'wikibase-label-empty' ), // property name
				'' // claim
	...
	enterNewClaim: function() {
		...
		$newClaim.claimview().addClass( 'wb-claim-new' );
