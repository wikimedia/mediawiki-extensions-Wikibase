...
$.widget( 'wikibase.claimlistview', PARENT, {
	...
	_create: function() {
		...
		.on( 'claimviewtoggleerror', function( e, hasError ) {
			var $claimSections = self.element.find( '.wb-claim-section' );
			if( hasError ) {
				$claimSections.addClass( 'wb-error' );
			}
			// explicitly check whether any other error still set (considering multiple edit modes)
			else if( $claimSections.has( '>.wb-error' ).length === 0 ) {
				$claimSections.removeClass( 'wb-error' );
			}
		} );
	},
			...
			if( dropValue || !newSnak ) {
				...
				.fail( function( errorCode, details ) {
					...
					$btnSave.getTooltip().on( 'hide', function( e ) {
						$newClaim.removeClass( 'wb-error' ).addClass( 'wb-edit' );
					} );
				} );
			}
		} );
