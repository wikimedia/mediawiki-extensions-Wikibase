 ...
 *         Default: null
 *
 * @option [firstClaimIndex] {number|null} The index of the claimlistview's first claim within a
 *         list of claims.
 *         Default: null
...
$.widget( 'wikibase.claimlistview', PARENT, {
	...
	_createListView: function() {
		...
		.on( 'listviewitemadded listviewitemremoved', function( event, value, $li ) {
			self._updateClaimIndices();
		} );

		this._updateClaimIndices();
	},
	...
	* @since 0.5
	*/
	_updateClaimIndices: function() {
		var listview = this.$listview.data( 'listview' ),
			$claimviews = listview.items();

		for( var i = 0; i < $claimviews.length; i++ ) {
			var claimview = listview.listItemAdapter().liInstance( $claimviews.eq( i ) );
			claimview.option( 'index', this.options.firstClaimIndex + i );
		}
	},
