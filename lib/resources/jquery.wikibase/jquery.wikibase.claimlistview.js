...
$.widget( 'wikibase.claimlistview', PARENT, {
	...
	_insertClaimRow: function( claimSectionPropertyId, $content ) {
		...
		// add new row as last one but before "add" button placeholder row if we have one in this section:
		var $claimAddRow = $claimSection.children( '.wb-claim-add' );

		if( $claimAddRow.length ) {
			$claimAddRow.last().before( $content );
		} else {
			$claimSection.append( $content );
		}
	},
	...
	_serveClaimSection: function( mainSnakPropertyId ) {
		...
		if( $section.length === 0 ) {
			...
			// TODO: (bug 45673) Get code adding the "add" placeholder DOM out of here since the
			//  button itself also is not part of the claimlistview anymore.
			if( fetchedProperty ) {
				...
				$section = mw.template( 'wb-claim-section',
					...
					$addClaim // claims (in this case link to create a new claim)
				);
			} else {
	...
	enterNewClaim: function( sectionPropertyId ) {
		...
		.on( this._lmwEvent( 'stopediting' ), function( event, dropValue ) {
			var newClaimview = self._lmwInstance( $newClaim ),
				newSnak = newClaimview.$mainSnak.data( 'snakview' ).snak();

			if ( !self._lmwInstance( $newClaim ).isValid() && !dropValue ) {
			...
			if( !dropValue && newSnak ) {
				...
				var api = new wb.RepoApi(),
					newClaim,
					guidGenerator = new wb.utilities.ClaimGuidGenerator(),
				...
				api.setClaim( newClaim, wb.getRevisionStore().getBaseRevision() )
