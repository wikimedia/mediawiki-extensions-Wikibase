		...
		if( $section.length === 0 ) {
			...
			if( fetchedProperty ) {
				...
				$section = mw.template( 'wb-claim-section',
					...
					$addClaim // claim (in this case link to create a new claim)
				);
			} else {
				// we have to add a dummy node here since the function for adding rows relies on
				// having this node in each section (which is kind of bad now). This should fully
				// go out of here, also see TODO above.
				$addClaim = mw.template( 'wb-claim', '', '', '', '' );
