...
$.widget( 'wikibase.claimlistview', PARENT, {
	...
	_create: function() {
		var self = this,
			...
			lmwErrorEvent = this._lmwEvent( 'error' ),
		...
		this.element.on( lmwRemoveEvent, function( e ) {
			var $claim = $( e.target ),
		...
		.on(
			[ lmwAfterStopEditingEvent, lmwStartEditingEvent, lmwRemoveEvent ].join( ' ' ),
			function( e ) {
	...
	_serveClaimSection: function( mainSnakPropertyId ) {
		...
		if( $section.length === 0 ) {
			...
			$section = mw.template( 'wb-claim-section',
				...
				wb.utilities.ui.buildEntityLink( property ), // property name
				$addClaim // claim
	...
	enterNewClaim: function( sectionPropertyId ) {
		...
		.on( this._lmwEvent( 'stopediting' ), function( event, dropValue ) {
			...
			if( !dropValue && newSnak ) {
				...
				self._lmwInstance( $newClaim ).toggleActionMessage(
					{ message: 'wikibase-save-inprogress' },
					self._saveNewClaim( $newClaim, newSnak )
				);
			}
		} )
		.on( this._lmwEvent( 'afterstopediting' ), function( event, dropValue ) {
			if( dropValue || !self._lmwInstance( $newClaim ).$mainSnak.data( 'snakview' ).snak() ) {
				...
				if ( $newClaim.parent()[0] === self.$claims[0] ) {
					...
					self.findClaimSection( $newClaim ).find( '.wb-claim-add .wb-ui-toolbar' )
	...
	/**
	 * Will save a new Claim to the represented entity. Does all the related API action.
	 * @since 0.4
	 *
	...
	_saveNewClaim: function( $newClaim, mainSnak ) {
		...
		.done( function( newClaimWithGUID, pageInfo ) {
			...
			// TODO: Depending on how the actual interaction flow of adding a new
			// claim will be, the focus should probably be set to somewhere more
