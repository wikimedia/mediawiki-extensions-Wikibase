...
$.widget( 'wikibase.claimlistview', PARENT, {
	...
	_create: function() {
		...
		.on( 'claimviewerror', function( e ) {
			var $claimSections = self.element.find( '.wb-claim-section' );
			$claimSections.has( '>.wb-error' ).addClass( 'wb-error' );
			$claimSections.not( ':has( >.wb-error )' ).removeClass( 'wb-error' );
		} );
	},
		...
		$newClaim.on( 'snakviewstopediting', function( event, dropValue, newSnak ) {
			...
			if( dropValue || !newSnak ) {
				...
				.fail( function( errorCode, details ) {
					var $newClaim = self.element.find( '.wb-claim-new' ).filter( '.wb-claimview' ),
						claimview = $newClaim.data( 'claimview' ),
						toolbar = $newClaim.find( '.wb-ui-toolbar' ).data( 'wb-toolbar' ),
						btnSave = toolbar.editGroup.btnSave;

					claimview._displayError( errorCode, details, btnSave );
					claimview.enable();
				} );
			}
		} );
