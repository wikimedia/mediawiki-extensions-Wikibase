 ...
 *         Default: null
 *
 * @option {string} entityType Type of the entity that the claimgroupsview referes to.
 *         Default: wikibase.Item.type
 *
 * @dependency jQuery
 * @dependency jQuery.Widget
 * @dependency jQuery.TemplatedWidget
 * @dependency jQuery.wikibase.claimlistview
 * @dependency jQuery.wikibase.listview
 * @dependency jQuery.wikibase.toolbarcontroller
 * @dependency mediaWiki.template
 * @dependency wikibase
 * @dependency wikibase.utilities.ui.buildLinkToEntityPage
 * @dependency wikibase.utilities.ui.buildMissingEntityInfo
 */
$.widget( 'wikibase.claimgroupsview', PARENT, {
	widgetBaseClass: 'wb-claimgroupsview',
	...
	_create: function() {
		...
		this._createClaimGroupsListview();
		...
		.on( 'listviewitemadded.' + this.widgetName, function( event, value, $claimlistview ) {
			...
			// Be sure to only react on the claimgroupsview's listview event:
		...
		.on( afterRemoveEvent, function( event, value, $claimview ) {
			// Check whether the whole claimlistview may be removed from the claimgroupsview if the
			// last claimview has been removed from the claimlistview.
		...
		if( claims ) {
			// TODO: this.option( 'value' ) should already contain order information
			var propertyOrder = [],
				claimsByProperty = {},
				i;
	 ...
	 * @since 0.5
	 */
	_createClaimGroupsListview: function() {
		var self = this;
	...
	_createGroupTitle: function( propertyId ) {
		...
		return mw.template( 'wb-claimgroupsview-groupname', $title );
	},
	...
	_toggleGroupTitleClass: function( $claimlistview, cssClass ) {
		var selector =  '.' + cssClass + ':not(.wb-claimgroupsview-groupname)',
	...
	_addGroupTitleClass: function( $claimlistview, cssClass ) {
		var $groupTitle = $claimlistview.children( '.wb-claimgroupsview-groupname' );
	...
	/**
	 * Returns the listview widget containing the claimlistviews managed by the claimgroupsview.
	 * @since 0.5
	 *
	...
	/**
	 * Triggers entering a new claimlistview to the claimgroupsview. This involves triggering
	 ...
	 * claimview which itself is added to the claimgroupsview.
	 * @since 0.5
	 */
	enterNewItem: function() {
		...
		.one( 'listviewenternewitem.' + this.widgetName, function( event, $claimlistview ) {
			...
			.one( afterStopEditingEvent, function( event, dropValue ) {
				...
				// TODO: Do not re-add the claimlistview to the claimgroupsview if there is not
...
$.wikibase.toolbarcontroller.definition( 'addtoolbar', {
	widgetName: 'wikibase.claimgroupsview',
	options: {
		interactionWidgetName: $.wikibase.claimgroupsview.prototype.widgetName
	}
} );
