	...
	stopEditing: $.NativeEventHandler( 'stopEditing', {
		...
		natively: function( e, dropValue ) {
			...
			if ( dropValue ) {
				...
				if( this._qualifiers ) {
					...
					if ( snaklistviews.length ) {
						for( i = 0; i < snaklistviews.length; i++ ) {
							...
							// Remove snaklistview from qualifier listview if no snakviews left in
							// that snaklistview:
							if( !snaklistviews[i].value() ) {
				...
				// Destroy listview which will also send out events to erase the "add qualifier"
				// toolbar:
				this._destroyQualifiersListView();

				// Refill the qualifier listview with the initial qualifiers:
				...
				.done( function( savedClaim, pageInfo ) {
					...
					if( self._qualifiers ) {
						...
						self._initialQualifiers = new wb.SnakList();

						if ( snaklistviews.length ) {
							for( i = 0; i < snaklistviews.length; i++ ) {
								snaklistviews[i].stopEditing();
								// Gather all the snaks in a SnakList:
								self._initialQualifiers.add( snaklistviews[i].value() );
							}
						}

						// Destroy and (if qualifiers still exist) re-create the qualifier listview
						// in order to re-group the qualifiers by their property:
						self._destroyQualifiersListView();

						var propertyIds = self._initialQualifiers.getPropertyOrder(),
							qualifierSnakLists = [];

						if( propertyIds ) {
							for( i = 0; i < propertyIds.length; i++ ) {
								qualifierSnakLists.push(
									self._initialQualifiers.getFilteredSnakList( propertyIds[i] )
								);
							}

							self._createQualifiersListview( qualifierSnakLists );
						}
					}
