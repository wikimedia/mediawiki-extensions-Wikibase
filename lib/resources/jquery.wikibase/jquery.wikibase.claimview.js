...
$.widget( 'wikibase.claimview', PARENT, {
		...
		.on( 'snakviewstopediting', function( e, dropValue, newSnak ) {
			...
			if ( self._claim ) {
				...
				.done( function( savedClaim, pageInfo ) {
					// No need to call self.enable() on "done" because the Snak section will be
					// removed completely and re-inserted in the Claim section.
	...
	disable: function() {
		...
		this.$mainSnak.data( 'snakview' ).variation().disable();
	},
	...
	enable: function() {
		...
		this.$mainSnak.data( 'snakview' ).variation().enable();
	},
