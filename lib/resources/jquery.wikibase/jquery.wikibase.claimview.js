...
$.widget( 'wikibase.claimview', PARENT, {
	...
	_create: function() {
		...
		.on( 'snakviewstopediting', function( e, dropValue, newSnak ) {
			if ( self._saveDisabled && !dropValue ) {
				e.preventDefault();
				return;
			}
				...
				self.toggleActionMessage( { message: 'wikibase-save-inprogress' }, function() {
					...
					if ( self._claim ) {
						...
						.fail( function( errorCode, details ) {
							var btnSave = self.$toolbar.find( '.wb-ui-toolbar' )
								.data( 'wb-toolbar' ).editGroup.btnSave,
		...
		// start editing initially if no Main Snak yet:
		if( !this.$mainSnak.snakview( 'snak' ) ) {
			this.$mainSnak.snakview( 'startEditing' );
		}
	},
	...
	_createToolbar: function() {
		...
		if( this.$mainSnak.snakview( 'isInEditMode' ) ) {
			...
			if ( !snakview.isValid() || snakview.isInitialSnak() ) {
				self._disableSave();
				toolbar.editGroup.btnSave.disable();
			}
		}
		*/
