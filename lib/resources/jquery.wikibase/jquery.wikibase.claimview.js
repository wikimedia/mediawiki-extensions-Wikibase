...
$.widget( 'wikibase.claimview', PARENT, {
	...
	_create: function() {
		...
		if( this._claim ) { // TODO: Allow adding qualifiers when adding a new claim.
			...
			this._createQualifiersListview( this._claim.getQualifiers() );
		}
...
$.wikibase.toolbarcontroller.definition( 'addtoolbar', {
	...
	events: {
		...
		'listviewdestroy snaklistviewafterstopediting': function( event ) {
			var $target = $( event.target ),
				$qualifiers = $target.closest( '.wb-claim-qualifiers' ),
				hasListviewWidget = $target.closest( ':wikibase-listview' ).length === 0;

			if (
				$qualifiers.data( 'addtoolbar' )
				&& ( hasListviewWidget || event.type !== 'listviewdestroy' )
			) {
