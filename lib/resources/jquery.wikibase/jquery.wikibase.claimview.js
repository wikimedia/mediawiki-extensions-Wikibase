...
$.widget( 'wikibase.claimview', PARENT, {
	...
	// Stub implementations of methods (events) required for external interaction (e.g. toolbar).
	// Move to a reasonable place further down in the code, if actually using the methods.
	afterStartEditing: $.NativeEventHandler( 'afterstartediting', { natively: function( e ) {} } ),
	afterStopEditing: $.NativeEventHandler( 'afterstopediting', {
		natively: function( e, dropValue ) {}
	} ),
	change: $.NativeEventHandler( 'change', { natively: function( e ) {} } ),
	toggleError: $.NativeEventHandler( 'toggleerror', { natively: function( e, error ) {} } ),
	...
	_create: function() {
		...
		if ( this._claim || this.options.predefined.mainSnak ) {
			...
			this.options.helpMessage = mw.msg( 'wikibase-claimview-snak-tooltip', propertyName );
		}
	},
	...
	stopEditing: $.NativeEventHandler( 'stopEditing', {
		...
		natively: function( e, dropValue ) {
			...
			if ( dropValue ) {
				...
				self.afterStopEditing( dropValue );
				...
				this.afterStopEditing( dropValue );
			}
		}
	} ),
