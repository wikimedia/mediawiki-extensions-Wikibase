...
( function( mw, wb, $ ) {
	...
	$.wikibase.snakview.SnakTypeSelector = wb.utilities.inherit( PARENT, {
		...
		_create: function() {
			...
			this.element
			// TODO: Add other ui states like "hover" implementing and using .ui-state-<state> css classes
			.addClass( 'ui-icon ui-icon-snaktypeselector ' + this.widgetBaseClass )
			.on( 'click.wb-snaktypeselector', function( event ) {
				...
				self._unbindGlobalListenersFn = function() {
					...
					$( document ).add( $( window ) ).off( 'mouseup resize', degrade );
		...
		destroy: function() {
			...
			this.element.removeClass( 'ui-icon ui-icon-snaktypeselector ' + this.widgetBaseClass );
