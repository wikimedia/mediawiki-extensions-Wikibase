...
( function( mw, wb, $ ) {
	...
	$.wikibase.snakview.SnakTypeSelector = wb.utilities.inherit( PARENT, {
		...
		_create: function() {
			...
			this.element.addClass( 'ui-icon ui-icon-gear ' + this.widgetBaseClass );
		...
		/**
		 * Disables the snak type selector.
		 * @since 0.4
		 */
		disable: function() {
			this._menu.element.hide();
			this.element.off( '.wb-snaktypeselector' );
			$( document ).add( $( window ) ).off( '.wb-snaktype-selector' );
		},

		/**
		 * Enables the snak type selector.
		 * @since 0.4
		 */
		enable: function() {
			var self = this,
				$menu = this._menu.element;

			this.element.on( 'click.wb-snaktypeselector', function( event ) {
				if ( $menu.is( ':visible' ) ) {
					$menu.hide();
					return;
				}
