	...
	MODULE.variation( wb.PropertyValueSnak, PARENT, {
		...
		_createNewValueView: function( dataValue, dataType ) {
			...
			var valueViewOptions = {
				expertProvider: expertStore,
				valueFormatterProvider: formatterStore,
				valueParserProvider: parserStore,
				value: dataValue,
				mediaWiki: mw
			};
			...
			if( dataType ) {
				$.extend( valueViewOptions, {
					...
					dataValueType: dataType.getDataValueType()
				} );
			} else if( dataValue ) {
				$.extend( valueViewOptions, {
...
}( mediaWiki, wikibase, jQuery, wikibase.experts, wikibase.formatters, wikibase.parsers ) );
