	...
	var SELF = MODULE.variation( wb.PropertyValueSnak, PARENT, {
		...
		draw: function() {
			...
			if( newValue !== false ) { // newValue could also be null for empty value
				...
				if( this._valueView
					...
					&& this._valueView.isMostSuitableFor( dataType || newValue )
				) {
		...
		_createNewValueView: function( dataValue, dataType ) {
			...
			if( !$.valueview.canChooseView( valueviewCriteria ) ) {
				...
				if( dataType ) {
					// no view for that values of that data type or the data type's data value type
					unsupportedMsg = mw.msg(
						'wikibase-snakview-variation-unsupporteddatatype', dataType.getLabel() );
					// NOTE: this also implies that the data value type is unsupported but the
				...
				$valueViewDom
				.text( unsupportedMsg )
				.addClass( this.variationBaseClass + '-unsupporteddatavalue' );
