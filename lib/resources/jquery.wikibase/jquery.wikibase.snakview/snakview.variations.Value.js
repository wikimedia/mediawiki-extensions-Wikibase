			...
			if( newValue !== false ) { // newValue could also be null for empty value
				...
				if( newValue && dataType && newValue.getType() !== dataType.getDataValueType() ) {
					...
					if( this._viewState.isInEditMode() ) {
						// Doesn't make sense to edit the value of the wrong data value type
						// directly, this will set the value to empty and create a valueview for
						// entering an entirely new value instead.
