...
( function( mw, wb, dv, dt, $ ) {
	 ...
	 * @since 0.4
	 */
	SELF = MODULE.variation( wb.PropertyValueSnak, PARENT, {
		/**
		 ...
		 */
		draw: function() {
			var propertyId, fetchedProperty, dataType,
			...
			if( newValue !== false ) { // newValue could also be null for empty value
				propertyId = this._viewState.propertyId();
				...
				dataType = fetchedProperty ?
					fetchedProperty.getContent().getDataType() : false;
		...
		_attachEventHandlers: function() {
			...
			.on( 'valueviewparse.' + this.variationBaseClass, function() {
				self._viewState.notify( 'invalid' );
			} )
			.on( 'valueviewafterparse.' + this.variationBaseClass, function() {
			...
			.on( 'inputextenderanimationstep.' + this.variationBaseClass, function( event, now, tween ) {
				...
				if( $extension.is( ':visible' ) ) {
					...
					currentHeight = self.$viewPort.height();
				...
				if( tween === undefined ) {
					self.$viewPort.animate( { height: newHeight }, 250 );
				} else {
