...
( function( mw, wb, dv, dt, $ ) {
	...
	MODULE.variation( wb.PropertyValueSnak, PARENT, {
		...
		draw: function() {
			...
			function _render () {
				if( self._valueView ) {
					...
					self._valueView[ ( self._viewState.isInEditMode() ? 'start' : 'stop' ) + 'Editing' ]();
			...
			if( newValue !== false ) { // newValue could also be null for empty value
				this._entityStore.get( this._viewState.propertyId() ).done( function (fetchedProperty) {
					...
					self.$viewPort.removeClass( self.variationBaseClass + '-datavaluetypemismatch' );
		...
		_createNewValueView: function( dataValue, dataType ) {
			...
			// Since ValueViews dynamically create their experts, and do not provide any standard way
			// of passing options down to the experts, we have to monkey-patch our way into the private
			// method ValueViews use to create experts.
			// FIXME: This has to die.
			var oldUpdateExpert = this._valueView._updateExpert;
			this._valueView._updateExpert = function () {
				var _return = oldUpdateExpert.apply( this, arguments );
				this._expert._entityStore = self._entityStore;
				this._expert._editableExpertOptions = this._expert._editableExpertOptions || {};
				this._expert._editableExpertOptions.entityStore = self._entityStore;
				this._expert._staticExpertOptions = this._expert._staticExpertOptions || {};
				this._expert._staticExpertOptions.entityStore = self._entityStore;
				return _return;
			};
