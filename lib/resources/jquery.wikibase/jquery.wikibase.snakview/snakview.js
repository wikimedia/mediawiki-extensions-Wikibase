 ...
 *
 * @option locked {Object|boolean} Key-value pairs determining which snakview elements to lock for
...
$.widget( 'wikibase.snakview', PARENT, {
	...
	_buildPropertySelector: function() {
		...
		.on( 'entityselectorselect', function( e, ui ) {
			...
			self._entityStore.get( entityId ).then( function (entity) {
	...
	drawProperty: function() {
		...
		this._entityStore.get( propertyId ).then( function( fetchedProperty ) {
			var property = fetchedProperty ? fetchedProperty.getContent() : null;

			if( self.options.locked.property || !self.isInEditMode() ) {
				...
				$propertyDom = self._buildPropertySelector().val( propertyLabel );

				// propagate snakview state:
				$propertyDom.data( 'entityselector' ).option( 'disabled', self.isDisabled() );
			}

			self.$property.empty().append( $propertyDom );
		} );
	},
