 ...
 * @since 0.3
 *
 * @option value {Object|wb.Snak|null} The snak this view should represent. If omitted, an empty
 *         view will be served, ready to take some input by the user. The value can be overwritten
 *         later, by using the value() or snak() functions.
 *         Default: { property: null, snaktype: wb.PropertyValueSnak.TYPE }
 *
 * @option locked {Object|boolean} Key-value pairs determining which snakview elements to lock. May
...
$.widget( 'wikibase.snakview', PARENT, {
	...
	options: {
		...
		locked: {
			...
			// TODO: Implement mechanism to lock elements of the variation
		},
	...
	_buildPropertySelector: function() {
		...
		.on( 'entityselectorselect', function( e, ui ) {
			...
			// in place.
			if ( !self._propertyId || self._propertyId !== ui.item.id ) {
				self.$snakValue.empty().append(
					$( '<div/>' ).append( $( '<span/>' ).addClass( 'mw-small-spinner' ) )
				);
			}

			api.getEntities( entityId, null, [ language ] ).done( function( entities ) {
				...
				wb.fetchedEntities[ entityId ] = new wb.store.FetchedContent( {
	...
	isInitialSnak: function() {
		...
		return ( !initialSnak && !snak || !!initialSnak && snak.equals( initialSnak ) );
	},
	...
	drawProperty: function() {
		var $propertyDom,
			...
			fetchedProperty = propertyId ? wb.fetchedEntities[ propertyId ] : null,
			property = fetchedProperty ? fetchedProperty.getContent() : null;
