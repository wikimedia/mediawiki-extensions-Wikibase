...
$.widget( 'wikibase.snakview', PARENT, {
	...
	/**
	 * Disables the Snak view.
	 * @since 0.4
	 */
	disable: function() {
		if( this._getPropertySelector() ) {
			this.$property.addClass( 'ui-state-disabled' );
			this._getPropertySelector().disable();
		}
		if ( this._getSnakTypeSelector() ) {
			this.$snakTypeSelector.addClass( 'ui-state-disabled' )
			this._getSnakTypeSelector().disable();
		}
		this._variation.disable();
	},

	/**
	 * Enables the Snak view.
	 * @since 0.4
	 */
	enable: function() {
		if( this._getPropertySelector() ) {
			this.$property.removeClass( 'ui-state-disabled' );
			this._getPropertySelector().enable();
		}
		if ( this._getSnakTypeSelector() ) {
			this.$snakTypeSelector.removeClass( 'ui-state-disabled' )
			this._getSnakTypeSelector().enable();
		}
		this._variation.enable();
	}
