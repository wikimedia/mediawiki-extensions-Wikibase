...
$.widget( 'wikibase.snakview', PARENT, {
	...
	_updateVariation: function() {
		...
		if( this._variation
			...
			// The Variation itself does not empty the view port on the upcoming destruction since
			// the view port contents are supposed to remain even without the Variation being
			// instantiated. However, since a new variation will be initialized, the view port
			// content may interfere with the process of initialization:
			this._variation.$viewPort.empty();
	 ...
	 * @since 0.4
	 *
	 * @param {wikibase.Property|null} property
