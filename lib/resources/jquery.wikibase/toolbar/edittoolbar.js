...
( function( mw, wb, $ ) {
	...
	$.widget( 'wikibase.edittoolbar', PARENT, {
		...
		_create: function() {
			var self = this,
				missingMethods, namespace, name, prototype, parentWidgetNode, prefix, editGroup, m;
			...
			if ( this.options.parentWidgetFullName ) {
				namespace = this.options.parentWidgetFullName.split( '.' )[ 0 ];
			...
			prefix = this._interactionWidget.widgetEventPrefix;

			this.element
			.on( prefix + 'afterstartediting', function() {
			...
			.on( prefix + 'afterstopediting', function() {
				editGroup.toNonEditMode();
				self.enable();
				self.toggleActionMessage( function() { editGroup.btnEdit.setFocus(); } );
			} )
			.on( prefix + 'afterstartediting ' + prefix + 'change', function() {
				var isEmpty = $.isFunction( self._interactionWidget.isEmpty ) &&
						self._interactionWidget.isEmpty(),
					isInitial = $.isFunction( self._interactionWidget.isInitialValue ) &&
						self._interactionWidget.isInitialValue();

				if ( self._interactionWidget.isValid() && !isEmpty && !isInitial ) {
					editGroup.btnSave.enable();
				} else {
					editGroup.btnSave.disable();
				}
			} )
		...
		displayError: function( error, anchor ) {
			...
			anchor.getTooltip().on( 'afterhide', function() {
