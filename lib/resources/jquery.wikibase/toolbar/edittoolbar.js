...
( function( mw, wb, $ ) {
	...
	$.widget( 'wikibase.edittoolbar', PARENT, {
		...
		_create: function() {
			...
			// Support promises instead of strings, too
			// This is necessary since ClaimView does not know immediately after creation
			// which help message to show.
			// FIXME: This should be replaced by a dynamic getter so that views can
			// arbitrarily change their help messages anywhere in their lifecycle.
			if( this._interactionWidget.options.helpMessage.then &&
				!(typeof this._interactionWidget.options.helpMessage === 'string' ) ) {
				this._interactionWidget.options.helpMessage.then( addTooltip );
			} else {
				addTooltip( this._interactionWidget.options.helpMessage );
			}
