...
( function( mw, wb, $ ) {
	...
	$.widget( 'wikibase.addtoolbar', PARENT, {
		...
		_create: function() {
			 ...
			 * interaction's widget's event prefix. In addition, it attaches the namespace to the
			 * prefixed event name.
			 *
			...
			function prefixed( eventName ) {
				...
				return eventPrefix + eventName + '.' + self.widgetName + ' ';
			}
			...
			.on(
				...
				function( event, value, $node ) {
					if ( $node !== undefined && $node.parent()[0] !== self.element.parent()[0] ) {
						...
						self.toolbar.btnAdd.data( 'toolbarbutton' ).setFocus();
					}
				}
			);
