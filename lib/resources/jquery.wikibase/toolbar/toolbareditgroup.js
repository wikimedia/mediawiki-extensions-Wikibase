...
$.widget( 'wikibase.toolbareditgroup', PARENT, {
		...
		this.element.on( 'toolbarbuttonaction.' + this.widgetName, function( event ) {
			switch( event.target ) {
				...
				case self.$btnSave.get( 0 ):
					...
					break;
				case self.options.displayRemoveButton && self.$btnRemove.get( 0 ):
	...
	toEditMode: function() {
		...
		this.innerGroup.addElement( this.getButton( '$btnSave' ) );

		if ( this.options.displayRemoveButton ) {
			this.innerGroup.addElement( this.getButton( '$btnRemove' ) );
		}

		this.innerGroup.addElement( this.getButton( '$btnCancel' ) );
	...
	toNonEditMode: function() {
		...
		this.innerGroup.addElement( this.getButton( '$btnEdit' ) );
	...
	clone: function( options ) {
		...
		// Clone buttons:
		clone.$btnEdit = this.$btnEdit ? this.$btnEdit.data( 'toolbarbutton' ).clone() : null;
		clone.$btnCancel = this.$btnCancel ? this.$btnCancel.data( 'toolbarbutton' ).clone() : null;
		clone.$btnSave = this.$btnSave ? this.$btnSave.data( 'toolbarbutton' ).clone() : null;
		clone.$btnRemove = this.$btnRemove ? this.$btnRemove.data( 'toolbarbutton' ).clone() : null;
