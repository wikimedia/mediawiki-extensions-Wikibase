...
( function( mw, wb, $ ) {
	...
	$.widget( 'wikibase.toolbarcontroller', {
				...
				$.each( self.options[type], function( j, id ) {
					...
					if ( def.events ) {
						...
						$.each( def.events, function( eventNames, callbackOrKeyword ) {
							$initNode.off( self._nameSpace( eventNames, type ) );
						} );
						...
						$.each( def.events, function( eventNames, callbackOrKeyword ) {
							...
							$initNode.on( self._nameSpace( eventNames, type ), function( event ) {
		...
		_nameSpace: function( eventNames, toolbarType ) {
			var toolbarTypeClass = this.widgetName + '-' + toolbarType;
			return ( eventNames + ' ' )
				.split( ' ' )
				.join( '.' + this.widgetName + '.' + toolbarTypeClass + ' ' );
		},
