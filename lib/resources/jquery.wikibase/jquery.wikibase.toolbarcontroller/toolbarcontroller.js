						...
						$.each( def.events, function( eventName, callbackOrKeyword ) {
							if ( callbackOrKeyword === 'create' ) {
								callbackOrKeyword = function( event ) {
									...
									mw.template( 'wb-toolbar', '' ).appendTo( $node );
									$node[type]( options );
								}
								...
								callbackOrKeyword = function( event ) {
									...
									if ( $node.data( type ) ) {
										...
										$node.children( '.wb-ui-toolbar' ).remove();
									}
								}
							}

							// Create and destroy events have to be defined.
							$initNode.on( def.eventPrefix + eventName, function( event ) {
								callbackOrKeyword( event, $initNode );
							} );
						} );
					} else {
