...
$.widget( 'wikibase.listview', PARENT, {
	...
	_create: function() {
		...
		this.element.on( liRemoveEvent, function( e ) {
			...
			self._lia.liInstance( $listItem ).destroy();
	...
	enterNewListItem: function() {
		...
		this._lia.newListItem( $newLi, null );
		...
		$newLi.on( this._lia.prefixedEvent( 'stopediting' ), function( event, dropValue ) {
			...
			if( !dropValue && newSnak ) {
				self._lia.liInstance( $newLi ).toggleActionMessage(
					...
					function() {
						...
						api.setReference(
							...
							self._insertListItem( newRefWithHash );
						} )
						.fail( function( errorCode, details ) {
							...
							listview.toggleActionMessage( function() {
								listview.displayError( error, btnSave );
								listview.enable();
							} );
						} );
					}
