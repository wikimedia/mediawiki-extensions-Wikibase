 ...
 *            be initialized. The widget will be initilized on this DOM node after the DOM node is
...
$.widget( 'wikibase.listview', PARENT, {
	...
	/**
	 * Returns all list items which do have an value not considered empty (not null).
	 *
	 * @since 0.4
	 *
	 * @return {jQuery}
	...
	addListItem: $.NativeEventHandler( 'addListItem', {
		...
		natively: function( event, value, $newLi ) {
			// first insert DOM so value's widget's events can already bubble during initialization!
	...
	removeListItem: $.NativeEventHandler( 'removeListItem', {
		initially: function( event, $itemNode ) {
			...
			// provide the value here for convenience and for consisten event argument order in all
			// add/remove events
		...
		$newLi.on( 'snakviewstopediting', function( event, dropValue, newSnak ) {
			...
			if( dropValue || !newSnak ) {
				...
				api.setReference(
					...
					// destroy new list item which was initially initialized as empty. Instead,
					// enter new list item with the value provided by the API
					self.removeListItem( $newLi );
