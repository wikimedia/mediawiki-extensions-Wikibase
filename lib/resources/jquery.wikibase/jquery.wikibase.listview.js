 ...
 *
 * @event itemmoved: Triggered when an item node is moved within the list.
 *        (1) {jQuery.Event}
...
$.widget( 'wikibase.listview', PARENT, {
	...
	move: function( $itemNode, toIndex ) {
		...
		if(
			currIndex < 0
			|| currIndex === toIndex
			|| currIndex === itemCount - 1 && toIndex >= itemCount
		) {
			return;
		}

		if( toIndex >= this.items().length ) {
			$itemNode.insertAfter( this.items().last() );
		} else {
			$itemNode.insertBefore( this.items().eq( toIndex ) );
		}

		this._trigger( 'itemmoved', null, [ this.indexOf( $itemNode ), this.items().length ] );
	},
