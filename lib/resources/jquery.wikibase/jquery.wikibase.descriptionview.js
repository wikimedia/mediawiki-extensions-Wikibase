...
$.widget( 'wikibase.descriptionview', PARENT, {
	...
	_afterStopEditing: function( dropValue ) {
		if( !dropValue ) {
			...
			this.$text.children( 'input,textarea' ).val( '' );
		}
	...
	_setOption: function( key, value ) {
		...
		if( key === 'disabled' && this._isInEditMode ) {
			this.$text.children( 'input,textarea' ).prop( 'disabled', value );
		}
	...
	value: function( value ) {
		...
		return new wb.datamodel.Term(
			...
			$.trim( this.$text.children( 'input,textarea' ).val() )
		);
	},
	...
	focus: function() {
		if( this._isInEditMode ) {
			this.$text.children( 'input,textarea' ).focus();
		} else {
			this.element.focus();
		}
	}
