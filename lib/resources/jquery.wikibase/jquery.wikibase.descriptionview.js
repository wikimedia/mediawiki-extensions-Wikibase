...
$.widget( 'wikibase.descriptionview', PARENT, {
	...
	_draw: function() {
		...
		var $input = $( '<' + this.options.inputNodeName + ' />', {
	...
	_afterStopEditing: function( dropValue ) {
		if( !dropValue ) {
			...
			this.$text.children( this.options.inputNodeName ).val( '' );
		}
	...
	_setOption: function( key, value ) {
		...
		if( key === 'disabled' && this._isInEditMode ) {
			this.$text.children( this.options.inputNodeName ).prop( 'disabled', value );
		}
	...
	value: function( value ) {
		...
		return new wb.datamodel.Term(
			...
			$.trim( this.$text.children( this.options.inputNodeName ).val() )
		);
	},
	...
	focus: function() {
		if( this._isInEditMode ) {
			this.$text.children( this.options.inputNodeName ).focus();
		} else {
			this.element.focus();
		}
	}
