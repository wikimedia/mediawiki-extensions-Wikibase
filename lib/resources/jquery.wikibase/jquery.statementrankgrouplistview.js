...
/**
 * View for displaying claimlistviews filled with statements separated into sections according to
 * their ranks.
 * @since 0.5
 * @extends jQuery.TemplatedWidget
 *
 * @option value {wikibase.Statement[]|null} The statements to be displayed by this view.
 *         Default: null
 *
 * @option [entityType] {string} The entity type the widget is used by. This option is passed on to
 *         claimlistview.
 *         Default: null
 */
$.widget( 'wikibase.claimgrouplistview', PARENT, {
	 ...
	 */
	options: {
		template: 'wb-statementgrouplistview',
		templateParams: ['', '', ''],
		templateShortCuts: {
			$listviewPreferred: '.wb-statementgrouplistview-preferred',
			$listviewNormal: '.wb-statementgrouplistview-normal',
			$listviewDeprecated: '.wb-statementgrouplistview-deprecated'
		},
		value: null,
		entityType: null
	},
	...
	_create: function() {
		...
		this.$listviews = this.$listviews
			.add( this.$listviewDeprecated )
			.add( this.$listviewNormal )
			.add( this.$listviewPreferred );

		this._createStatementRankGroupListviews();
	 ...
	 * @since 0.5
	 */
	_createStatementRankGroupListviews: function() {
		var self = this,
		...
		if( statements ) {
			orderedStatements = [];
			$.each( wb.Statement.RANK, function( key, rank ) {
				for( var i = 0; i < statements.length; i++ ) {
					if( statements[i].getRank() === rank ) {
						orderedStatements.push( statements[i] );
					}
				}
			} );
		}
		...
		$.each( wikibase.Statement.RANK, function( rank, i ) {
			this.$listviews.eq( i ).listview( {
				listItemAdapter: new $.wikibase.listview.ListItemAdapter( {
					...
					newItemOptionsFn: function( value ) {
						return {
							...
							firstClaimIndex: indexOf( value, orderedStatements )
						};
					}
				} )
			} );
	...
	enterNewItem: function() {
		...
		.one( 'listviewenternewitem.' + this.widgetName, function( event, $claimlistview ) {
			...
			.one( afterStopEditingEvent, function( event, dropValue ) {
				...
				// Move the new claimlistview to the rank group of its rank. Claimlistview returns
				// an array of statements. However, just one statement may have been added:
				var newStatements = lia.liInstance( $claimlistview ).value();

				var $rankListview = self.$listview.eq( newStatements[0].getRank() ),
					rankListview = $rankListview.data( 'listview' ),
					lia = rankListview.listItemAdapter();

				self.$listviews.eq( newStatements[0].getRank() ).data( 'listview' )
					.addItem( newStatements );
