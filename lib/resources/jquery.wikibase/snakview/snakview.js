...
$.widget( 'wikibase.snakview', PARENT, {
	...
	_create: function() {
		...
		// Re-render on previsouly generated DOM should be avoided. However, when regenerating the
		...
		var propertyIsEmpty = !this.$property.contents().length,
			...
			allEmpty = propertyIsEmpty && snakTypeSelectorIsEmpty && snakValueIsEmpty;

		if( propertyIsEmpty && !this._isEncapsulated() || allEmpty ) {
			this.drawProperty();
		}

		if( snakTypeSelectorIsEmpty || allEmpty ) {
			this.drawSnakTypeSelector();
		}

		if( snakValueIsEmpty || allEmpty ) {
			this.drawVariation();
		}
	...
	_isEncapsulated: function() {
		return !!( $.expr[':']['wikibase-snaklistview']
			&& this.element.closest( ':wikibase-snaklistview' ).length
			|| $.expr[':']['wikibase-statementview']
				&& this.element.closest( ':wikibase-statementview' ).length
		);
	},
	...
	drawProperty: function() {
		...
		if( this.options.locked.property && this.$property.contents().length ) {
	 ...
	 * @private
	 *
	 * @param {wikibase.datamodel.Property|string} [property] `Property` object or `Property` id.
