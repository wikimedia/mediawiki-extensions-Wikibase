...
$.widget( 'wikibase.statementlistview', PARENT, {
	...
	_create: function() {
		...
		.on( startEditingEvent, function( event ) {
			// Forward "startediting" event for higher components (e.g. statementgrouplistview) to
	...
	_createGroupName: function() {
		...
		if( this.element.find( '.wikibase-statementgrouplistview-groupname' ).length > 0 ) {
			return;
		}
		...
		this.option( 'entityStore' ).get( propertyId ).done( function( property ) {
			...
			self.element.append(
				mw.wbTemplate( 'wikibase-statementgrouplistview-groupname', $title )
			);
		} );
	},
	...
	/**
	 * Returns the current list of `Statement` objects represented by the `statementlistview`. If
	 * there are no `Statement`s, `null` is returned.
	 * @since 0.5
	 *
