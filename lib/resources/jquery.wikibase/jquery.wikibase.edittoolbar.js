...
( function( mw, wb, $ ) {
	 ...
	 * @since 0.4
	 *
	 * This widget acts interfaces the more generic wb.Toolbar/wb.Toolbar.EditGroup objects
	 * and an an object (widget) that may be interacted with (e.g. edited).
	 * The widget the toolbar shall interact with has to have implemented certain methods listed
	 * in the _requiredMethods attribute. The interaction widget may have an isEmpty() method which
	 ...
	 *         on.)
	 *
	 * @option toolbarParentSelector {string} CSS3 selector to find the node the actual toolbar
	 *         buttons shall be appended to.
	 *
	 * @option enableRemove {boolean} (optional) Whether a remove button shall be shown. Regardless
	 *         of setting this options, the "remove" button will not be shown if the interaction
	...
	$.widget( 'wikibase.editToolbar', {
		...
		_create: function() {
			...
			if ( !this.element.data( this.options.interactionWidgetName ) ) {
				throw new Error( 'jquery.wikibase.edittoolbar: Interaction widget not defined' );
			}
			if ( !this.options.toolbarParentSelector ) {
				throw new Error( 'jquery.wikibase.edittoolbar: Missing toolbar parent selector' );
			}
			...
			missingMethods = this.checkMethods();
			...
			this.editGroup = new wb.ui.Toolbar.EditGroup( {
				displayRemoveButton:
					( this.options.enableRemove && $.isFunction( this._interactionWidget.remove ) )
			} );
			...
			.on( prefix + 'afterstartediting ' + prefix + 'change', function( event ) {
				...
				if ( self._interactionWidget.isValid() && !isEmpty ) {
