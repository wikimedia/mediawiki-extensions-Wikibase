...
$.widget( 'wikibase.claimgrouplistview', PARENT, {
	 ...
	 * enterNewItem() method that instantiates a pending statementview to be added to the pending
	 * statementview which itself is added to the claimgrouplistview.
	 * @since 0.5
	 */
			...
			.on( 'addtoolbaradd.addtoolbar', function( e ) {
				...
				$claimgrouplistview.one(
					...
					function( event, $claimlistview ) {
						if( event.target === claimgrouplistview.$listview[0] ) {
							$claimlistview.one(
								// TODO: There should be a "claimlistviewafterstartediting" event.
								'statementviewafterstartediting.addtoolbar',
								function() {
									var listview = claimgrouplistview.$listview.data( 'listview' ),
										lia = listview.listItemAdapter();
