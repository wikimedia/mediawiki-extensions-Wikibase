...
$.widget( 'wikibase.claimgrouplistview', PARENT, {
	...
	_create: function() {
		...
		.on( 'listviewitemadded.' + this.widgetName, function( event, value, $claimlistview ) {
			...
			if( value ) {
				...
				self._createGroupTitle( propertyId ).done( function( groupTitle ) {
					$claimlistview.append( groupTitle );
				} );
			}
		} )
		...
		.on( afterStopEditingEvent, function( event, dropValue ) {
			...
			if( dropValue ) {
				// Re-oder claimlistviews according to their initial indices:
	...
	_createGroupTitle: function( propertyId ) {
		...
		this.option( 'entityStore' ).get( propertyId ).done( function( property ) {
			...
			deferred.resolve( mw.template( 'wb-claimgrouplistview-groupname', $title ) );
		} );
