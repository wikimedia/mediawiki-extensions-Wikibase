	...
	_createEntiytermsforlanguagelistviewToggler: function() {
		...
		this.$entitytermsforlanguagelistviewToggler = $( '<div/>' )
			...
			.on( 'toggleranimation.' + this.widgetName, function( event, params ) {
				...
				// Show "help" link after toggling not before list the first time (decided by
				// Product Management):
				if( self.$entitytermsforlanguagelistviewHelp && params.visibility ) {
					self.$entitytermsforlanguagelistviewHelp.removeClass(
		...
		var toggler = this.$entitytermsforlanguagelistviewToggler.data( 'toggler' ),
			...
			title = new mw.Title( 'Configuring_languages', helpNamespace );
		...
		.done( function( response ) {
			var cssClass = response.query.pages[1] ? '' : 'new',
				cssClassPrefix = 'wikibase-entitytermsview-entitytermsforlanguagelistview-';

			self.$entitytermsforlanguagelistviewHelp =
				$( '<span/>' )
				.addClass( cssClassPrefix + 'configure' )
				.append(
					$( '<a/>' )
				...
				.insertAfter( self.$entitytermsforlanguagelistviewToggler );

			if( !toggler.option( '$subject' ).is( ':visible' ) ) {
				self.$entitytermsforlanguagelistviewHelp
					.addClass( cssClassPrefix + 'configure-hidden' );
			}
		} );
	},
