...
$.widget( 'wikibase.entitytermsforlanguagelistview', PARENT, {
	...
	/**
	 * Creates the listview widget managing the entitytermsforlanguage widgets
	 */
	...
	stopEditing: function( dropValue ) {
		...
		/**
		 * @param {jQuery} $queue
		 * @param {jQuery.wikibase.entitytermsforlanguage} entitytermsforlanguage
		 * @param {boolean} dropValue
		 */
		function addStopEditToQueue( $queue, entitytermsforlanguage, dropValue ) {
			$queue.queue( 'stopediting', function( next ) {
				entitytermsforlanguage.element
				.one( 'entitytermsforlanguageafterstopediting.' + eventNamespace,
					function( event ) {
						entitytermsforlanguage.element.off( '.' + eventNamespace );
						setTimeout( next, 0 );
					}
				)
				.one( 'entitytermsforlanguagetoggleerror.' + eventNamespace,
					function( event ) {
						entitytermsforlanguage.element.off( '.' + eventNamespace );
						$queue.clearQueue();
						self._resetEditMode();
		...
		listview.items().each( function() {
			...
			addStopEditToQueue(
				...
				dropValue || entitytermsforlanguage.isInitialValue()
			);
		} );
