...
$.widget( 'wikibase.referenceview', PARENT, {
	...
	value: function( reference ) {
		if ( reference ) {
			// value is set in snaklistview, but jshint doesn't know that
			/* global value */
	...
	events: {
		'snakviewstartediting snakviewcreate listviewitemadded listviewitemremoved': function( event ) {
			var $target = $( event.target ),
				$referenceviewNode, referenceview, listview, firstItemToolbar;

			if ( event.type.indexOf( 'snakview' ) !== -1 ) {
				// Create toolbar for each snakview widget:
				$referenceviewNode = $target.closest( '.wb-referenceview' );
			...
			listview = $target.closest( '.wb-referenceview' ).data( 'referenceview' )._listview;
			if ( listview.items() ) {
				firstItemToolbar = $( listview.items()[0] ).data( 'removetoolbar' );
				if ( firstItemToolbar ) {
