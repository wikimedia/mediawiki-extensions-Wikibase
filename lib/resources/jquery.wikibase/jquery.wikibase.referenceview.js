	...
	value: function( reference ) {
		if ( reference ) {
			...
			for( var i = 0; i < snaklistviews.length; i++ ) {
				var curSnakList = this.options.listItemAdapter.liInstance( snaklistviews.eq( i ) ).value();
				if( curSnakList ) {
		...
		if( $snaklistviews.length ) {
			for( i = 0; i < $snaklistviews.length; i++ ) {
				...
				if( dropValue && !snaklistview.value() ) {
					...
					// initial qualifiers:
					this._initialSnakList.addItem( snaklistview.value() );
				}
			}
		}
	...
	isInitialValue: function() {
		...
		if( $snaklistviews.length ) {
			for( var i = 0; i < $snaklistviews.length; i++ ) {
				...
				if( snakview.value() ) {
					snakList.addItem( snakview.value() );
				}
			}
		}
