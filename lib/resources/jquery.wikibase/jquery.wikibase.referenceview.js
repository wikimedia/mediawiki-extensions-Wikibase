...
$.widget( 'wikibase.referenceview', PARENT, {
	...
	value: function( reference ) {
		if ( reference ) {
			...
			var listview = this.$listview.data( 'listview' ),
				...
				snaklistviews = this.$listview.data( 'listview' ).items(),
				snakList = new wb.datamodel.SnakList();

			for( var i = 0; i < snaklistviews.length; i++ ) {
				var curSnakList = lia.liInstance( snaklistviews.eq( i ) ).value();
				if( curSnakList ) {
	...
	startEditing: function() {
		...
		for( var i = 0; i < $snaklistviews.length; i++ ) {
			lia.liInstance( $snaklistviews.eq( [i] ) ).startEditing();
		}
	...
	isInitialValue: function() {
		...
		if( $snaklistviews.length ) {
			for( var i = 0; i < $snaklistviews.length; i++ ) {
				var snakview = lia.liInstance( $snaklistviews.eq( i ) );
				if( snakview.value() ) {
