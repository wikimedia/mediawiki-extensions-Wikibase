...
( function( mw, wb, $ ) {
	 ...
	 *
	 * @option interactionWidgetName {string} (optional) Name of the widget the toolbar shall
	 *         interact with. (That widget needs to be initialized on the same DOM node this toolbar
	 *         is initialized on.) If the interactionWidgetName option is omitted, the toolbar will
	 *         be initialized as well but proper interaction is not ensured (see _create() method).
	 ...
	 *
	 * @option action {function} (optional) Custom action the add button shall trigger. The action
	 *         will be triggered only when no interaction widget via the interactionWidgetName
	 *         option is set.
	 *         Default value: null (no custom action)
	 *
	 * @option eventPrefix {string} (optional) Custom event prefix the events the toolbar will
	 *         listen to will be prefixed with. If not set but an interaction widget is defined via
	 *         the interactionWidgetName option, the interaction widget's event prefix will be used.
