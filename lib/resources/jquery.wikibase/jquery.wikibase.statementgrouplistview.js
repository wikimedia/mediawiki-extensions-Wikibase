 ...
 *         Default: null
 *
 * @option {string} entityType Type of the entity that the statementgrouplistview referes to.
...
$.widget( 'wikibase.statementgrouplistview', PARENT, {
	...
	_create: function() {
		...
		var listview = this.listview(),
			...
			afterStartEditingEvent = lia.prefixedEvent( 'afterstartediting.' + this.widgetName ),
		...
		.on( afterRemoveEvent, function( event ) {
			// Check whether the whole statementlistview may be removed from the
			// statementgrouplistview if the last statementview has been removed from the
			// statementlistview.
	...
	_toggleGroupTitleClass: function( $statementlistview, cssClass ) {
		var selector =  '.' + cssClass + ':not(.wikibase-statementgrouplistview-groupname)',
	...
	_addGroupTitleClass: function( $statementlistview, cssClass ) {
		var $groupTitle
			= $statementlistview.children( '.wikibase-statementgrouplistview-groupname' );
	...
	/**
	 * Returns a reference to the `listview` widget containing the `statementlistview`s managed by
	...
	/**
	 * Triggers adding a new `statementlistview` to the `statementgrouplistview`. This involves
	 * triggering the corresponding process for the new pending `statementlistview` by triggering
	 * the `statementlistview`'s `enterNewItem()` method that instantiates a pending `statementview`
	 * to be added to the pending `statementlistview` which itself is added to the
	...
	enterNewItem: function() {
		...
		return this.listview().enterNewItem().done( function( $statementlistview ) {
			...
			.one( afterStopEditingEvent, function( event, dropValue ) {
				...
				// A new Statement(List) has been saved successfully. If the new StatementList
				// features a Property that already is represented by a statementlistview, the new
				// StatementList's Statements have to be appended to it. If there is no
				// statementlistview featuring the property yet, a new statementlistview is added.
				// TODO: Do not re-add the statementlistview to the statementgrouplistview if there
				// is not statementlistview featuring the specific Property yet. Instead, use the
	...
	_findStatementlistview: function( propertyId ) {
		...
		for( var i = 0; i < $statementlistviews.length; i++ ) {
			var statementlistview = lia.liInstance( $statementlistviews.eq( i ) ),
				statements = statementlistview.value();

			if( !statements.length ) {
				continue;
			}

			var mainSnak = statements[0].getMainSnak
				? statements[0].getMainSnak()
				: statements[0].getClaim().getMainSnak();
