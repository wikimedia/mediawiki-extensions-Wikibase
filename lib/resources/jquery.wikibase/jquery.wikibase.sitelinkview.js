...
$.widget( 'wikibase.sitelinkview', PARENT, {
	...
	isInitialValue: function() {
		...
		// TODO: Implement wikibase.datamodel.SiteLink.equals()
		var currentValue = this.value();

		if(
			currentValue.getSiteId() !== this.options.value.getSiteId()
			|| currentValue.getPageName() !== this.options.value.getPageName()
			|| currentValue.getBadges().length !== this.options.value.getBadges().length
		) {
			return false;
		}

		var currentBadges = currentValue.getBadges(),
			initialBadges = this.options.value.getBadges();

		for( var i = 0; i < currentBadges.length; i++ ) {
			if( $.inArray( currentBadges[i], initialBadges ) === -1 ) {
				return false;
			}
		}
	...
	_setOption: function( key, value ) {
		if( key === 'value' ) {
			throw new Error( 'Cannot set value after initialization' );
		}
