...
wb.ui.PropertyEditTool.EditableValue.Interface = wb.utilities.inherit( PARENT, {
	...
	_initInputElement: function() {
		...
		inputParent = this._getValueContainer();
		inputParent.empty();
		...
		if( this.getOption( 'autoExpand' ) ) {
			 ...
			 */
			evCls = wb.ui.PropertyEditTool.EditableValue.prototype.UI_CLASS;
	...
	stopEditing: function( save ) {
		...
		var initialValue = this.getInitialValue(),
			value = save ? this.getValue() : initialValue,
			fireEvent;
		...
		fireEvent = this.isInEditMode() ?
			this._setValue_inEditMode( value ) : this._setValue_inNonEditMode( value );
	...
	getValue: function() {
		var value = this.isInEditMode() ?
			this._getValue_inEditMode() : this._getValue_inNonEditMode();
