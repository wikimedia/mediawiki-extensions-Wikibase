...
wb.ui.PropertyEditTool.EditableValue.Interface = util.inherit( PARENT, {
	...
	updateLanguageAttributes: function() {
		...
		if( this._inputElem ) {
			var lang = this.getSubject().attr( 'lang' ) || mw.config.get( 'wgUserLanguage' ),
				dir = this.getSubject().css( 'direction' ) || 'auto';
	...
	/**
	 * Sets a value.
	 * Returns the current value after setting has been performed. The returned string can be
	 * different from the given value since the given value will go through some normalization
	 * first.
	 *
	 * IMPORTANT: If the given value is invalid, no change will happen and the previous value will
	 *            be returned. This might be confusing behavior since, in the "new UI", setting
	 *            invalid values will result into emptying the widget's value (setting the value
	 *            to null)
	 *
	 * @param {string} value
