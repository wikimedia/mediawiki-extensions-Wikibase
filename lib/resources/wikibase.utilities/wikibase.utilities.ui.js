...
( function( mw, wb, $ ) {
	...
	wb.utilities.ui.buildPrettyEntityLabel = function( entity ) {
		var label = entity.getLabel(),
			text = label || entity.getId() || '', // New entities without ID are handled well below
	 ...
	 *
	 * @param {wb.Entity} entity
	 * @return {string} Either the label, ID or placeholder text in parentheses
	 */
	wb.utilities.ui.buildPrettyEntityLabelText = function( entity ) {
		return entity.getLabel() || entity.getId() || mw.msg( 'parentheses', mw.msg( 'wikibase-label-empty' ) );
	}
