...
( function( mw, wb, $ ) {
	...
	wb.utilities.ui.buildPrettyEntityLabel = function( entity ) {
		var label = entity.getLabel(),
			text = label || entity.getId() || '', // TODO: nicely handle new entities without ID
		...
		if( !label ) {
			...
			$undefinedInfo = $( '<span>' )
				.attr( 'class', 'wb-entity-undefinedinfo' )
	...
	wb.utilities.ui.buildPendingCounter = function(
		...
		msg = !pendingQuantity || pendingQuantity === '0' ?
		...
		$msg = $( ( '<span>' + msg + '</span>' ).replace( /__3__/g, '<span/>' ) );
