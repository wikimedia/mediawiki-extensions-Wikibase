...
( function( $, mw, wb, undefined ) {
	 ...
	 *   the jQueryEvent object itself will always be given to the custom handlers.
	 ...
	 * no custom handlers registered or if 'allowCustomResult' is set to false - the initial handler return value.
	 *
	 ...
	 *        - initally: should be a function representing the initial handler
	...
	$.NativeEventHandler = function( eventName, fn ) {
		...
		var handler = function() {
			...
			var defaultPreventedEarly = event.isDefaultPrevented(), // store this so custom callbacks can't influence
				...
				customHandlerArgs = event.customHandlerArgs || args,
				nativeHandlerArgs = event.nativeHandlerArgs || args;

			// don't reveal this to custom handlers
			event.customHandlerArgs = event.nativeHandlerArgs = event.cancel = undefined;

			// trigger all registered events (custom handlers)
			// this might be prevented by the initial handler for some reason
			if( !event.isImmediatePropagationStopped() ) {
				// Don't use trigger(); it might end up in an endless loop since it would try to execute a function
				// named after the event in the object
			...
			if( !defaultPreventedEarly && !event.isDefaultPrevented() ) {
				...
				// if native handler returns undefined, return previously gattered return value (might be the same)
		 ...
		 * @return undefined|true|false|Array if undefined or true the native handler as well as all custom callbacks
