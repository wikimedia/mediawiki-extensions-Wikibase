	...
	$.widget( 'ui.tagadata', {
		...
		_create: function() {
			...
			this.tagList.children( 'li' ).each( function() {
				var newTag,
					newTagLabel = $( this ).text();

				if( self._formatLabel( newTagLabel ) !== '' ) { // don't initialize empty tags here
					newTag = self.createTag( newTagLabel, $( this ).attr( 'class' ) );
		...
		getTagLabel: function( tag ) {
			...
			return this._formatLabel(
				this.options.editableTags ?
					$( tag ).find( 'input' ).val() : $( tag ).find( '.tagadata-label' ).text()
			);
		},
		...
		createTag: function( value, additionalClasses ) {
			var self = this,
				tag, label, input, removeTag, removeTagIcon;
			...
			if( this.options.editableTags ) {
				...
				.on( 'focus', function() {
					input
						.closest( '.tagadata-choice' )
						.addClass( 'tagadata-choice-active' );
				} )
				.on( 'blur', function() {
					var $tag = input
						.closest( '.tagadata-choice' )
						.removeClass( 'tagadata-choice-active' );
				...
				tag.on( 'tagadatatagchanged', function( e, oldValue ) {
					var tagLabel = self.getTagLabel( tag ),
						...
						equalTags = self._getTags( oldValue ).add( tag );
