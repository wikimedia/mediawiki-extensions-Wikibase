...
wb.ui.PropertyEditTool.EditableValue.SitePageInterface = wb.utilities.inherit( PARENT,
	...
	updateLanguageAttributes: function() {
		...
		if ( this._inputElem !== null ) {
			...
			this._inputElem.data( 'autosuggester' ).updateLanguage();
		}
	},
	...
	_setState: function( state ) {
		var success = wb.ui.PropertyEditTool.EditableValue.Interface.prototype._setState.call( this, state );
		if ( this._inputElem !== null ) {
			if ( state === this.STATE.DISABLED ) {
				this._inputElem.data( 'autosuggester' ).disable();
			} else {
				this._inputElem.data( 'autosuggester' ).enable();
			}
		}
		return success;
	}
