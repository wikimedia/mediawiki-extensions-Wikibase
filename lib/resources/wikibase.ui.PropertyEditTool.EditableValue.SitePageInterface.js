...
wb.ui.PropertyEditTool.EditableValue.SitePageInterface = util.inherit( PARENT, {
	...
	_buildInputElement: function() {
		...
		inputElement.on( 'keyup close', function( event ) {
		...
		inputElement.on( 'suggesterresponse suggesterselect', function( event, results ) {
		...
		inputElement.on( 'suggestererror', function( event, textStatus, errorThrown ) {
			if ( textStatus !== 'abort' ) {
				var error = {
					code: textStatus,
