...
wb.ui.PropertyEditTool.EditableValue.SitePageInterface = util.inherit( PARENT, {
	...
	_initBadgeEditing: function() {
		var self = this,
			$badgeSpan = this._subject.parent().find( '.wb-sitelinks-badges' );
	...
	_disableBadgeEditing: function() {
		var $badgeSpan = this._subject.parent().find( '.wb-sitelinks-badges' );

		if ( $badgeSpan.find( 'span.wb-badge-empty' ).length ) {
			$badgeSpan.find( 'span.wb-badge-empty' ).remove();
			return;
		}

		$badgeSpan.find( 'span' ).off( 'click' );
	},
	...
	_getSetSiteLinkUrl: function() {
		...
		// @XXX: This is ugly as hell, but we don't which item we're editing in here
		// thus we have to rely on the global -.-

		return mw.util.getUrl(
			'Special:SetSiteLink/' + wbEntityId + '/' + this.getSite().getGlobalSiteId()
		);
	},
