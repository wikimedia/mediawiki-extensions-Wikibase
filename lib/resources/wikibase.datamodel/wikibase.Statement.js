...
var PARENT = wb.Claim,
	constructor = function( mainSnak, qualifiers, references, rank, guid ) {
		...
		this.setRank( rank === undefined ? this.constructor.RANK.NORMAL : rank );
	};
...
wb.Statement = wb.utilities.inherit( 'WbStatement', PARENT, constructor, {
	...
	setRank: function( rank ) {
		...
		for( var i in this.constructor.RANK ) {
			if( this.constructor.RANK[i] === rank ) {
				this._rank = rank;
				return;
			}
		}
	...
	equals: function( statement ) {
		if ( this === statement ) {
			...
			!( statement instanceof this.constructor )
	...
	toJSON: function() {
		...
		if ( this._rank ) {
			$.each( this.constructor.RANK, function ( rank, i ) {
				if ( self._rank === i ) {
