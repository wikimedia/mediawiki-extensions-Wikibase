...
wb.RepoApi = wb.utilities.inherit( PARENT, {
	...
	getEntitiesByPage: function( sites, titles, props, languages, sort, dir ) {
		...
		var params = {
			action: 'wbgetentities',
			sites: normalizeParam( sites ),
			titles: normalizeParam( titles ),
			props: normalizeParam( props ),
			languages: normalizeParam( languages ),
			sort: normalizeParam( sort ),
			dir: dir || undefined
	...
	post: function( params ) {
		...
		if ( mw.config.get( 'wbRepoUrl' ) + mw.config.get( 'wbRepoScriptPath' ) === mw.config.get( 'wgServer' ) + mw.config.get( 'wgScriptPath' ) ) {
		...
		if ( mw.config.exists( 'wbRepoEditToken' ) ) {
			...
			$.extend( params, { token: mw.config.get( 'wbRepoEditToken' ) } );
			...
			.fail( function( code, result ) {
				repoPostDeffered.rejectWith.apply( repoPostDeffered, arguments );
			} )
			.done( function( data ) {
				...
				self._api.post( params, options )
					...
					.done( function( result ) {
						repoPostDeffered.resolve.apply( repoPostDeffered, arguments );
					} );
			} );
...
/**
* Temporary alias for backwards compatiblity
* @deprecated
