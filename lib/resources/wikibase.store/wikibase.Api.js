...
wb.Api = wb.utilities.inherit( PARENT, {
	 ...
	 */
	_extend_repo_call: function( params, options ) {
		var localServerRaw = mw.config.get( 'wgServer' ).replace( /(https?)?:?\/\//, '' ),
			repoServerRaw = mw.config.get( 'wbRepoUrl' ).replace( /(https?)?:?\/\//, '' ).replace( /\/.*/, '' );
	...
	repoPost: function( params ) {
		var options = {};
		this._extend_repo_call( params, options );
