...
wb.Api = wb.utilities.inherit( PARENT, {
	...
	editEntity: function( id, baserevid, data, clear ) {
		...
		params.data = this._toJSON( params.data );
	...
	_toJSON: function( json ) {
		if ( typeof json !== 'object' || json === null ) {
			...
			$.each( json, function( key, value ) {
				toSerialize.push( ( $.isArray( json ) ? '' : '"' + key + '":' ) + self.toJSON( value ) );
			} );
			var serialized = String( toSerialize );
			return ( $.isArray( json ) ) ? '[' + serialized + ']' : '{' + serialized + '}';
		}
	}
