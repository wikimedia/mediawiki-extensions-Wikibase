...
( function( $ ) {
	...
	$.fn.inputAutoExpand = function( options ) {
		...
		this.filter( 'input:text, textarea' ).each( function() {
			...
			/**
			 * TODO/FIXME:
			 * This could be optimized when called on several(!) input elements at the same time.
			 * The reason why this is so slow for many input elements (e.g. with tag-a-data) is,
			 * because we insert and manipulate many DOM elements after another instead of inserting
			 * them into the DOM in one go. Calculating the best width for one element will involve
			 * inserting and removing a 'ruler' into the DOM and then setting the final width of
			 * the input box (equals three re-renderings per element we calculate the width for).
			 */
			if( inputAE ) {
	...
	var AutoExpandInput = function( inputElem, options ) {
		...
		// TODO:
		// this should all go into a separate function. When changing options after first
		// initialization, e.g. expandWidth disabled before and enabled afterwards, expansion will
		// fail because we don't initialize ruler when options change. Nothing in here should be
		// dependent on options!

		// TODO:
		// Currently all expanding inputs share the same rulers. This won't work in case the styles
		// of two inputs are different. In that case only the last initialized auto expansion will
		// work properly!

		if ( options.expandWidth ) {
			this.$rulerX = $( '#AutoExpandInput_rulerX' );
			if ( this.$rulerX.length === 0 ) {
				this.$rulerX = $( '<div/>' )
