 ...
 *        (1) {jQuery.Event}
 */
this.wikibase = this.wb = new ( function Wb( mw, $, GenericSet ) {
	'use strict';
	...
	/**
	 * Returns a set of all the known sites.
	 *
	...
	this.getSites = function() {
		...
		this._sites = new GenericSet( this.Site );
		...
		return this._sites.makeImmutable().copy();
	};

	/**
	 * Returns a set of all known sites of one group.
	 *
	 * @since 0.4
	 *
	 * @return {GenericSet<wb.Site>}
	...
	this.getSitesOfGroup = function( groupId ) {
		var sitesOfGroup = new GenericSet( this.Site ),
			allSites = this._sites || this.getSites();

		allSites.each( function( site ) {
			if( site.getGroup() === groupId ) {
				sitesOfGroup.add( site );
			}
		} );
	...
	this.getSiteGroups = function() {
		...
		this.getSites().each( function( site ) {
			var group = site.getGroup();
	...
	this.getSite = function( siteId ) {
		if( typeof siteId !== 'string' ) {
			throw new Error( 'Site ID has to be a string, "' + $.type( siteId ) + '" given.' );
		}
		var result = null;
