...
( function( mw, $, undefined ) {
	...
	mw.template = function( key, parameter_1 /* [, parameter_2] */ ) {
		...
		function stripTags( string ) {
			var tagsToIgnore = [ 'thead', 'tbody', 'tfoot' ];
			...
			// rip off any XML notation since jQuery will parse to HTML
			string = string.replace( /\s*\/>/g, '>' );

			return string;
		}
		strippedTemplate = stripTags( template.plain() );
		strippedParsedTemplate = stripTags( parsedTemplate );
