...
( function( mw, wb, util ) {
	...
	wb.RepoApiError.newFromApiResponse = function( errorCode, details, apiAction ) {
		...
		if ( details.error ) {
			if ( details.error.messages && details.error.messages[0] && details.error.messages[0].html ) {
				if ( details.error.messages.length > 1 ) {
					detailedMessage = '<ul>';
					for ( var i = 0; i < details.error.messages.length; i++ ) {
						detailedMessage += '<li>' + details.error.messages[i].html['*'] + '</li>';
					}
					detailedMessage += '</ul>';
				} else {
					detailedMessage = details.error.messages[0].html['*'];
				}
			} else if ( details.error.messages && details.error.messages.html ) {
				// TODO: when was messages not an array? Should we keep this for compatibility?
				// TODO: where does this "*" come from? we should get rid of it!
				// TODO: there's a "\n" inside the HTML, we should get rid of it in the backend.
				detailedMessage = details.error.messages.html['*'];
			} else {
