...
$.extend( wb.AbstractedRepoApi.prototype, {
	...
	setReference: function( statementGuid, snaks, baseRevId, referenceHash ) {
		return this._abstract(
			this._repoApi.setReference( statementGuid, snaks, baseRevId, referenceHash ),
			function( result ) {
				return [
	...
	setClaim: function( claim, baseRevId ) {
		return this._abstract(
			this._repoApi.setClaim( claim, baseRevId ),
			function( result ) {
				return [
	...
	createClaim: function( entityId, baseRevId, mainSnak ) {
		return this._abstract(
			this._repoApi.createClaim( entityId, baseRevId, mainSnak ),
			function( result ) {
				return [
					wb.Claim.newFromJSON( result.claim ),
					result.pageinfo
	...
	setClaimValue: function( claimGuid, baseRevId, mainSnak ) {
		return this._abstract(
			this._repoApi.setClaimValue( entityId, baseRevId, mainSnak ),
			function( result ) {
				return [
					wb.Claim.newFromJSON( result.claim ),
					result.pageinfo
	 ...
	 * @since 0.4
	 *
	 * @param apiPromise jQuery.Promise
