 ...
 * – in the generated API (api.php). Valid values are listed there, for example for the createClaim's snaktype parameter
 *   on <api.php?action=help&modules=wbcreateclaim> or entity types for createEntity (wbeditentity module).
 * – on mediawiki.org with some usage examples: https://www.mediawiki.org/wiki/Extension:Wikibase/API
 *
 * @constructor
 ...
 *
 * @param {mw.Api} api
 * @param {string} [apiEndpoint] API Url; Will use the current page if not specified.
 * @param {string} [editToken]
 * @param {string} [corsOrigin] CORS origin parameter; Won't perform a CORS request if not set.
 *
...
wb.RepoApi = function wbRepoApi( api, apiEndpoint, editToken, corsOrigin  ) {
	...
	this._api = api;
	this._repoApiEndpoint = apiEndpoint || undefined;
	this._corsOrigin = corsOrigin || null;
	this._repoEditToken = editToken || null;
};
...
$.extend( wb.RepoApi.prototype, {
	...
	_logFailure: function( errorCode, errorInfo ) {
		if ( errorInfo.error ) {
			mw.log( 'API error (' + errorCode + '): ' + errorInfo.error.info );
		} else if ( errorInfo.exception ) {
			mw.log( 'API exception (' + errorInfo.textStatus + '): ' + errorInfo.exception );
		}
	},
		...
		if ( this._repoEditToken ) {
			...
			.done( function( data ) {
				...
				self._api.post( params, options )
					.fail( self._logFailure )
					.fail( repoPostDeferred.reject )
					.done( repoPostDeferred.resolve );
			} );
			return repoPostDeferred.promise();
		}
	},
