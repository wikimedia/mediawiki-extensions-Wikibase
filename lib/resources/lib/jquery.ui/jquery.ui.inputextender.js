	...
	var inputExtendersWithVisibleExtension = ( function () {
		...
		return {
			add: function ( inputExtenderInstance ) {
				var index = inputExtenders.indexOf( inputExtenderInstance );
				if ( index < 0 ) {
			...
			// Make sure this is up to date and they are really visible.
				return inputExtenders.filter( function ( inputExtenderInstance ) {
 ...
 */
	$.ui.inputextender.redrawVisibleExtensions = function () {
		$.ui.inputextender.getInstancesWithVisibleExtensions().forEach( function ( i, instance ) {
			instance.draw();
		} );
	};
