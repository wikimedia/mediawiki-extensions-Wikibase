 ...
 */
	function measureMaximumStringWidths( $container, strings ) {
		var widths = [];
		strings.forEach( function ( i, string ) {
 ...
 */
	 ...
	 */
		...
		// Determine the maximum width a label may have and apply that width to each section:
			...
			this.options.values.forEach( function ( i, v ) {
				labels.push( v.label );
			} );
			...
			stringWidths.forEach( function ( i, width ) {
	 ...
	 */
		_showMenu: function () {
			...
			function flip( string ) {
				var segments = string.split( ' ' ).map( function ( segment ) {
