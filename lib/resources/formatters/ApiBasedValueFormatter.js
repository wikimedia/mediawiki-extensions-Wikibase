...
( function( $, wb, vf, util ) {
	...
	wb.formatters.ApiBasedValueFormatter = util.inherit( 'WbApiBasedValueFormatter', PARENT, {
		 ...
		 * @param {string} [dataType] The ID of the data type
		 * @param {string} [outputType] The output's preferred MIME type
		 *
		 * @return {$.Promise}
		 */
		format: function( dataValue, dataType, outputType ) {
			var deferred = $.Deferred();

			// Since dataType and outputType are optional parameters to this function as well as to
			// wb.formatters.api.formatValue, we use this complicated apply arguments pattern.
			wb.formatters.api.formatValue.apply( wb.formatters.api, $.makeArray( arguments ).concat( [this._options] ) )
