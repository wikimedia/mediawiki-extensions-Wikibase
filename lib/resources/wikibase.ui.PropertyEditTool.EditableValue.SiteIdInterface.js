...
wb.ui.PropertyEditTool.EditableValue.SiteIdInterface = wb.utilities.inherit( PARENT, {
	...
	_buildInputElement: function() {
		...
		if ( this._currentResults !== null ) {
			inputElement.suggester( {
				source: function( request, response ) {
					...
					var results = $.grep( self._currentResults, function( result, i ) {
						return (
			...
			// Check for valid input when the suggester menu is opened since fallback selection will
			// require the suggestion menu to be filled which will be done after the eachchange
			// has been triggered (see this.getResultSetMatch()).
			inputElement.on( 'suggesteropen', function( event, results ) {
				self._onInputRegistered();
			} );
		}

		inputElement.on( 'autocompleteopen', function( event ) {
		...
		inputElement.data( 'suggester' ).menu.element.on(
			'mouseover',
			'li',
			function( event ) {
				...
				if (
					event.target !==
						self._inputElem.data( 'suggester' ).menu.element.children().first()
							.children('a')[0]
					) {
					self._inputElem.data( 'suggester' ).menu.element.children().first()
	...
	_highlightMatchingCharacters: function() {
		...
		this._inputElem.data( 'suggester' ).menu.element.children().each( function( i ) {
	...
	getResultSetMatch: function( value ) {
		...
		if( this.isInEditMode() ) {
			...
			var autoComplete = this._inputElem.data( 'suggester' );
