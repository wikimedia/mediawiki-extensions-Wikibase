...
wb.ui.PropertyEditTool.EditableValue.SiteIdInterface = wb.utilities.inherit( $PARENT, {
	...
	_buildInputElement: function() {
		...
		inputElement.data( 'suggester' ).menu.element.on(
			...
			$.proxy( function( event ) {
				...
				if (
					event.target !==
						this._inputElem.data( 'suggester' ).menu.element.children().first()
						.children('a')[0]
				) {
					this._inputElem.data( 'suggester' ).menu.element.children().first()
	...
	_setValue_inNonEditMode: function( value ) {
		...
		if( site !== null ) {
			...
			.attr( 'colspan', '0' )
			.attr( 'class', '' )
			.attr( 'lang', siteLanguage.code )
			.attr( 'dir', siteLanguage.dir )
			.addClass( 'wb-sitelinks-sitename wb-sitelinks-sitename-' + siteId )
			.text( site.getShortName() )
			.after(
				$( '<td/>' ) // TODO interface should be independent from any DOM structure
				.addClass( 'wb-sitelinks-siteid wb-sitelinks-siteid-' + siteId )
				.attr({ 'dir' : 'ltr' })
				.text( siteId )
			);
			return true;
