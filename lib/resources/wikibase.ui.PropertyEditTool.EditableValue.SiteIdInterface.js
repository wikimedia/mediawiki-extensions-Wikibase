...
wb.ui.PropertyEditTool.EditableValue.SiteIdInterface = util.inherit( PARENT, {
	...
	_initSiteList: function() {
		...
		for ( var siteId in wb.sites.getSites() ) {
			if( $.inArray( siteId, ignoredSites ) === -1 ) {
				this._sites.push( wb.sites.getSite( siteId ) );
			}
		}
	...
	setValue: function( value ) {
		if ( this._inputElem !== null ) {
			...
			this._inputElem.data( 'siteselector' ).setSelectedSite( wb.sites.getSite( value ) );
		}
	...
	_setState: function( state ) {
		...
		if( this._inputElem !== null ) {
			if( state === this.STATE.DISABLED ) {
				this._inputElem.data( 'siteselector' ).disable();
			} else {
