...
wb.ui.PropertyEditTool.EditableValue.SiteIdInterface = wb.utilities.inherit( PARENT, {
	...
	_buildInputElement: function() {
		...
		inputElement.on( 'suggesteropen', function( event ) {
			self._highlightMatchingCharacters();
			// make sure that when the autocomplete list opens and nothing was considered a valid
			// choice, we just select the first entry as the valid choice (see getResultSetMatch()
			// for details)
			self._onInputRegistered();
		} );
