...
wb.ui.PropertyEditTool.EditableValue.SiteIdInterface = util.inherit( PARENT, {
	...
	_buildInputElement: function() {
		// get basic input box
		var self = this,
			inputElement = PARENT.prototype._buildInputElement.call( this );

		// extend input element with site selector widget
		if ( this._sites !== null ) {
			inputElement
			.siteselector( { resultSet: this._sites } )
	...
	_initSiteList: function() {
		...
		if( this.isInEditMode() ) {
			...
			this._inputElem.data( 'siteselector' ).setResultSet( this._sites );
		}
	},
