...
wb.ui.PropertyEditTool.EditableValue.AutocompleteInterface = wb.utilities.inherit( PARENT, {
	...
	_buildInputElement: function() {
		...
		var $container = $( '<div/>' ).autosuggester( {
		...
		$inputElement.on( 'keyup close suggesterselected', $.proxy( function( event ) {
		...
		$inputElement.on( 'suggesterresponse', $.proxy( function( event, results ) {
	...
	/**
	 * @see wb.ui.PropertyEditTool.EditableValue.Interface._setValue_inEditMode
	 *
	 * @param {String} value
	 * @return {Boolean} Whether the value has been changed
	 */
	_setValue_inEditMode: function( value ) {
		this._inputElem.data( 'autosuggester' ).value( value );
		return true;
	},

	/**
	 * @see wb.ui.PropertyEditTool.EditableValue.Interface._getValue_inEditMode
	 *
	 * @return string
	 */
	_getValue_inEditMode: function() {
		return this._inputElem.data( 'autosuggester' ).value();
	},

	/**
	 * @see wb.ui.PropertyEditTool.EditableValue.AutocompleteInterface._destroyInputElement
	 */
	_destroyInputElement: function() {
		this._inputElem.data( 'autosuggester' ).destroy();
		PARENT.prototype._destroyInputElement.call( this );
	},

	/**
	 * @see wb.ui.PropertyEditTool.EditableValue.Interface.setFocus
	 */
	setFocus: function() {
		if( this._inputElem !== null ) {
			this._inputElem.data( 'autosuggester' ).$input.focus();
		}
	},

	/**
	 * @see wb.ui.PropertyEditTool.EditableValue.Interface.removeFocus
	 */
	removeFocus: function() {
		if( this._inputElem !== null ) {
			this._inputElem.data( 'autosuggester' ).$input.blur();
		}
	},
