( function ( wb, util ) {
	...
	SELF.newFromApiResponse = function ( details, apiAction ) {
		...
		if ( details.error ) {
			...
			if ( firstError.data && firstError.data.messages ) {
				parameters = firstError.data.messages[ 0 ] && firstError.data.messages[ 0 ].parameters;
				detailedMessage = messagesObjectToHtml( firstError.data.messages );
			} else if ( firstError[ '*' ] ) {
				detailedMessage = mw.html.escape( String( firstError[ '*' ] ) );
			}
