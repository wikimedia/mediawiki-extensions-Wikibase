...
QUnit.test( 'check _post asserts user when logged in ', function ( assert ) {
	var mock = mockApi();
	mw._mockUser = 'fooBarUser';
	mock.api._post( { action: 'foobar' } );
	assert.strictEqual( getParam( mock.spy, 'assertuser' ), 'fooBarUser' );
} );

QUnit.test( 'check _post does not assert user when not logged in ', function ( assert ) {
	var mock = mockApi();
