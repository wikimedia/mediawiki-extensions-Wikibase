 ...
 */
	...
	QUnit.test( 'formatValue()', function ( assert ) {
		...
		assert.strictEqual( getParam( mock.spy, 'options' ), JSON.stringify( { option: 'option value' } ) );
	...
	QUnit.test( 'parseValue()', function ( assert ) {
		...
		assert.strictEqual( getParam( mock.spy, 'uselang' ), 'de' );
		assert.strictEqual(
			getParam( mock.spy, 'options' ),
			JSON.stringify( { lang: 'de', option: 'option value' } )
		);
		...
		assert.strictEqual( getParam( mock.spy, 'uselang', 1 ), undefined );
		assert.strictEqual( getParam( mock.spy, 'options', 1 ), undefined );
	} );
	...
	QUnit.test( 'searchEntities()', function ( assert ) {
		...
		assert.strictEqual( getParam( mock.spy, 'search' ), 'label' );
		assert.strictEqual( getParam( mock.spy, 'language' ), 'language code' );
		assert.strictEqual( getParam( mock.spy, 'type' ), 'entity type' );
		assert.strictEqual( getParam( mock.spy, 'limit' ), 10 );
		assert.strictEqual( getParam( mock.spy, 'continue' ), 5 );

		assert.strictEqual( getParam( mock.spy, 'search', 1 ), 'label' );
		assert.strictEqual( getParam( mock.spy, 'language', 1 ), 'language code' );
		assert.strictEqual( getParam( mock.spy, 'type', 1 ), 'entity type' );
	...
	QUnit.test( 'createClaim()', function ( assert ) {
		...
		assert.strictEqual( getParam( mock.spy, 'entity' ), 'entity id' );
		assert.strictEqual( getParam( mock.spy, 'baserevid' ), 12345 );
		assert.strictEqual( getParam( mock.spy, 'snaktype' ), 'snak type' );
		assert.strictEqual( getParam( mock.spy, 'property' ), 'property id' );
		assert.strictEqual( getParam( mock.spy, 'value' ), JSON.stringify( 'snak value' ) );

		assert.strictEqual( getParam( mock.spy, 'entity', 1 ), 'entity id' );
		assert.strictEqual( getParam( mock.spy, 'baserevid', 1 ), 12345 );
		assert.strictEqual( getParam( mock.spy, 'snaktype', 1 ), 'snak type' );
		assert.strictEqual( getParam( mock.spy, 'property', 1 ), 'property id' );
	...
	QUnit.test( 'getClaims()', function ( assert ) {
		...
		assert.strictEqual( getParam( mock.spy, 'property' ), 'property id' );
		assert.strictEqual( getParam( mock.spy, 'claim' ), 'claim GUID' );
		assert.strictEqual( getParam( mock.spy, 'rank' ), 'rank' );
		assert.strictEqual( getParam( mock.spy, 'props' ), 'claim props' );
	...
	QUnit.test( 'setReference()', function ( assert ) {
		...
		assert.strictEqual( getParam( mock.spy, 'statement' ), 'statement GUID' );
		assert.strictEqual( getParam( mock.spy, 'snaks' ), JSON.stringify( { 'I am': 'serialized Snaks' } ) );
		assert.strictEqual( getParam( mock.spy, 'baserevid' ), 12345 );
		assert.strictEqual( getParam( mock.spy, 'reference' ), 'reference hash' );
		assert.strictEqual( getParam( mock.spy, 'index' ), 67890 );

		assert.strictEqual( getParam( mock.spy, 'statement', 1 ), 'statement GUID' );
		assert.strictEqual(
	...
	QUnit.test( 'removeReferences()', function ( assert ) {
		...
		assert.strictEqual( getParam( mock.spy, 'statement' ), 'statement GUID' );
		assert.strictEqual( getParam( mock.spy, 'references' ), '\x1freference hash 1\x1freference hash 2' );
		assert.strictEqual( getParam( mock.spy, 'baserevid' ), 12345 );

		assert.strictEqual( getParam( mock.spy, 'statement', 1 ), 'statement GUID' );
		assert.strictEqual( getParam( mock.spy, 'references', 1 ), '\x1freference hash' );
		assert.strictEqual( getParam( mock.spy, 'baserevid', 1 ), 12345 );
	} );
	...
	QUnit.test( 'mergeItems()', function ( assert ) {
		...
		assert.strictEqual( getParam( mock.spy, 'fromid', 1 ), 'entity id from' );
		assert.strictEqual( getParam( mock.spy, 'toid', 1 ), 'entity id to' );
		assert.strictEqual(
			getParam( mock.spy, 'ignoreconflicts', 1 ),
			'\x1fproperty to ignore conflict for'
		);
		assert.strictEqual( getParam( mock.spy, 'summary', 1 ), 'edit summary' );

		assert.strictEqual( getParam( mock.spy, 'fromid', 2 ), 'entity id from' );
		assert.strictEqual( getParam( mock.spy, 'toid', 2 ), 'entity id to' );
