...
wb.ui.PropertyEditTool.EditableValue.ListInterface = wb.utilities.inherit( PARENT, {
	...
	_setValue_inNonEditMode: function( value ) {
		...
		// FIXME: Emptying valList causes property edit tool's toolbar elements to be dropped
		//  without clearing their references in the property edit tool instance. Just removing the
		//  <li> nodes does not work either since the property edit tool's toolbar should actually
		//  be removed. Accessing valList.data( 'wb-ui-propertyedittool' ) as a workaround.
		if( valList.data( 'wb-ui-propertyedittool' ) ) {
			valList.data( 'wb-ui-propertyedittool' )._toolbar.destroy();
			valList.data( 'wb-ui-propertyedittool' )._toolbar = null;
		}
		valList.empty();
