...
wb.ui.Tooltip = wb.utilities.inherit( PARENT, {
	...
	init: function( subject, options, tooltipContent, tipsyConfig ) {
		...
		$( window )
		.off( '.' + this.UI_CLASS )
		.on( 'resize.' + this.UI_CLASS, function( event ) {
	...
	_initTooltip: function() {
		if( !this._subject.data( 'tipsy' ) ) {
			...
			this._subject.data( 'tipsy' ).options.gravity = this._tipsyConfig.gravity;
		}
	...
	_toggleEvents: function( activate ) {
		if ( activate ) {
			...
			if (
				...
				this._subject.on( 'mouseover.' + this.UI_CLASS, jQuery.proxy( function() { this.show(); }, this ) );
				this._subject.on( 'mouseout.' + this.UI_CLASS, jQuery.proxy( function() { this.hide(); }, this ) );
			}
		} else {
			this._subject.off( 'mouseover.' + this.UI_CLASS );
			this._subject.off( 'mouseout.' + this.UI_CLASS );
		}
	},
