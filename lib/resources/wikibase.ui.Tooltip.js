...
wb.ui.Tooltip = wb.utilities.inherit( PARENT, {
	...
	show: function( permanent ) {
		if ( !this._isVisible ) {
			...
			// Note: The DOM content needs to be cloned since IE8 will lose the reference to the DOM
			// content when the inner HTML is removed within tipsy's native show() method.
			// However, this makes the copyright tooltip sticky in other browsers, so
			// @todo find a way that works for IE8 and does not cause issues elsewhere.
			this._tipsy.show();
