...
wb.ui.SiteLinksEditTool = wb.utilities.inherit( PARENT, {
	...
	_init: function( subject, options ) {
		...
		this._options = $.extend( {}, PARENT.prototype._options, {
			 ...
			 * @type {GenericSet<wb.Site>}
			 */
			allowedSites: ( new GenericSet( wb.Site ) ).makeImmutable()
		} );

		PARENT.prototype._init.call( this, subject, options );

		GenericSet.requireInstanceOfType( this.getOption( 'allowedSites' ), wb.Site );
	...
	_buildSingleValueToolbar: function( options ) {
		...
		toolbar.editGroup.tooltipAnchor.data( 'toolbarlabel' ).getTooltip().setGravity( 'se' );
		return toolbar;
	},
	...
	enterNewValue: function( value ) {
		...
		// Have the editableValues prototype regenerated each time a new value is about to be
		// entered in order to reflect any change to the set of site links (in the means of
		// "ignoreSiteLinks").
		this._editableValuesProto = null;

		var newValue = PARENT.prototype.enterNewValue.call(
	 ...
	 */
	isFull: function() {
		return this.getUnusedAllowedSites().length === 0;
	},
	...
	getEditableValuePrototype: function() {
		...
		// group).
		var ignoredSites = wb.getSites();
		ignoredSites.removeAll( this.getUnusedAllowedSites() );
		...
		EnhancedProto.prototype.ignoredSiteLinks = ignoredSites;
		...
		// FIXME: Fixating the EditableValue prototype prevents reflecting any change in the
		//  "ignoredSiteLinks" when instantiating additional EditableValues. Apart from that, when
		//  fixating the EditableValue prototype while initializing the prototype the first time,
		//  "ignoredSiteLinks" is empty since the site ids already used are gather by querying all
		//  initialized EditableValues. To work around this problem, _editableValuesProto is
		//  reset in enterNewValue().
	...
	_editableValueHandler_onAfterRemove: function( editableValue ) {
		...
		if( removedSite !== null ) {
			// remove site from ignored sites shared by all values managed by this:
			this._editableValuesProto.prototype.ignoredSiteLinks.remove( removedSite );
	...
	_newValueHandler_onAfterStopEditing: function( newValue, save, wasPending ) {
		...
		if( save ) {
			...
			this._editableValuesProto.prototype.ignoredSiteLinks.add( addedSite );
		}
	},
	...
	_fixateTableLayout: function() {
		...
		wb.getSites().each( function( site ) {
			$rulerTd.text( site.getId() ).append( $( '<br/>' ) );
		} );
		...
		// Make site id columns accept the content width of its child elements. Although, there is
		// no specific width set on the $siteIdColumns, they simply need to be poked to re-render.
		$siteIdColumns.css( 'width', 'auto' );
	...
	/**
	 * Returns a set of sites already used in site links of this SiteLinkEditTool.
	 *
	 * @since 0.4
	 *
	 * @return {GenericSet<wb.Site>}
	 */
	getRepresentedSites: function() {
		var site, editableSiteLink, representedSiteId,
			sites = new GenericSet( wb.Site );


		for( var i in this._editableValues ) {
			editableSiteLink = this._editableValues[i];
			representedSiteId = editableSiteLink.siteIdInterface.getOption( 'siteId' );
			site = representedSiteId !== null ? wb.getSite( representedSiteId ) : null;
	 ...
	 * @return {GenericSet<wb.Site>}
	 */
	getUnusedAllowedSites: function() {
		var allowedSites = this.getOption( 'allowedSites' ).mutableCopy(),
			representedSites = this.getRepresentedSites();

		allowedSites.removeAll( representedSites );
		return allowedSites;
	}
} );
...
} )( mediaWiki, wikibase, jQuery, GenericSet );
