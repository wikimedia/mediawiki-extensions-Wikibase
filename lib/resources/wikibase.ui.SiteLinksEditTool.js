...
wb.ui.SiteLinksEditTool = wb.utilities.inherit( PARENT, {
	...
	_fixateTableLayout: function() {
		var self = this,
			sites = wb.getSites(),
			$ruler = $( '<div/>' ).addClass( 'wb-ruler' ),
			$siteIdColumn,
			maxSiteIdWidth,
			originalTableWidth;

		// Measure the maximum width the site id column (which is set to not wrap) may take by
		// inserting all the site ids (and the contents of the corresponding table header cell) into
		// a dummy row/column to be sure to measure with the correct styling (font size etc.).
		// TODO: Since the table header's styling is different, for an optimal result, there would
		// need to be a second ruler just for the header column.
		self._subject.find( 'th.wb-sitelinks-siteid' ).each( function( i ) {
			$ruler.append( $( this ).text() ).append( $( '<br/>' ) );
		} );
		$.each( sites, function( i, site ) {
			$ruler.append( site.getLanguageCode() ).append( $( '<br/>' ) );
		} );
		this._subject.children( 'tbody' ).append(
			mw.template( 'wb-sitelink', '', 'wb-ruler', '', $ruler, '', '', $( '<td/>') )
		);

		$siteIdColumn = this._subject.find( 'tr.wb-ruler .wb-sitelinks-siteid' );
		maxSiteIdWidth = $siteIdColumn.children( '.wb-ruler' ).width();
		...
		$siteIdColumn.children( '.wb-ruler' )
		.empty()
		...
		function setWidths( widthModel ) {
			self._subject.children( 'colgroup' ).children().each( function( i ) {
				...
				$( this ).width( ( widthModel === 'outer' ) ? $td.outerWidth() : $td.width() );
			} );
		}

		originalTableWidth = this._subject.width();
		setWidths( 'outer' );
		...
		// definitions, their width have to be set to the dummy row columns' inner widths.
		if ( this._subject.width() !== originalTableWidth ) {
		...
		this._subject.children( 'tbody' ).children().last().remove();
	},
