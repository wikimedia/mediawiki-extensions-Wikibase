...
wb.ui.SiteLinksEditTool = wb.utilities.inherit( PARENT, {
	...
	_initSingleValue: function( valueElem ) {
		var editableSiteLink = PARENT.prototype._initSingleValue.call( this, valueElem );
	...
	enterNewValue: function( value ) {
		var newValue = wikibase.ui.PropertyEditTool.prototype.enterNewValue.call(
			this, value, { prepend: true }
		);
		...
		$( newValue ).on( 'afterStopEditing', function( event, save, wasPending ) {
			if ( save ) {
				...
				// when adding the first site link, no table body exists
				if ( $( 'table.wb-sitelinks tbody' ).length === 0 ) {
					$( 'table.wb-sitelinks' ).append( $( '<tbody/>' ) );
				}
	...
	getEditableValuePrototype: function() {
		...
		EnhancedProto.prototype.ignoredSiteLinks = this.getRepresentedSites();

		// don't forget to forward factory function
		EnhancedProto.newFromDom = function( subject, toolbar ) {
			// make sure factory creates an instance of EnhancedProto (3rd parameter)
			return BaseProto.newFromDom.call( this, subject, toolbar, EnhancedProto  );
		};
