...
wb.ui.SiteLinksEditTool = wb.utilities.inherit( PARENT, {
	...
	_init: function( subject, options ) {
		...
		$( 'table.wb-sitelinks' ).on( 'sortEnd.tablesorter', function() {
	...
	_initSingleValue: function( valueElem, options ) {
		var editableSiteLink = PARENT.prototype._initSingleValue.call( this, valueElem, options ),
			valElem = editableSiteLink._subject;
	...
	_editableValueHandler_onAfterRemove: function( editableValue ) {
		...
		var removedSiteId = editableValue.siteIdInterface.getSelectedSiteId(),
			index, pendingValues, i;
		if( removedSiteId !== null ) {
			index = $.inArray( removedSiteId, this._editableValuesProto.prototype.ignoredSiteLinks );
			if( index > -1 ) {
				...
				pendingValues = this.getPendingValues();
				for( i in pendingValues ) {
	...
	getRepresentedSiteIds: function() {
		var sites = [],
			i, editableSiteLink, site;

		for( i in this._editableValues ) {
			editableSiteLink = this._editableValues[i];
			site = editableSiteLink.siteIdInterface.getOption( 'siteId' );
			if( site !== null ) {
				sites.push( site );
			}
		}
