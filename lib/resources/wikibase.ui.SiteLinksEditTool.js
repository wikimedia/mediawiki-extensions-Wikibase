 ...
 * @since 0.1
 */
wb.ui.SiteLinksEditTool = util.inherit( PARENT, {
	/**
	...
	_fixateTableLayout: function() {
		...
		function getWidths( ) {
			var sum = 0;
			self._subject.children( 'colgroup' ).children().each( function( i, column ) {
				sum += $( column ).outerWidth();
			} );
			return sum;
		}
		...
		if ( columnWidths !== getWidths() && columnWidths !== this._subject.width() ) {
