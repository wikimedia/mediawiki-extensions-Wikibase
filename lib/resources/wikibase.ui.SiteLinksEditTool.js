...
wb.ui.SiteLinksEditTool = wb.utilities.inherit( PARENT, {
	 ...
	 */
	_initSingleValue: function( valueElem, options, toolbarOptions ) {
		toolbarOptions = toolbarOptions || {};

		var editableSiteLink = PARENT.prototype._initSingleValue.call( this,
			valueElem,
			options,
			$.extend( { gravity: 'se' }, toolbarOptions )
		);
	...
	enterNewValue: function( value ) {
		...
		if ( this._editableValues.length === 0 ) {
			...
			if ( $siteLinksTable.data( 'wb-sitelinks-group' ) === 'special' ) {
				var siteNameColumnHeading = mw.message( 'wikibase-sitelinks-sitename-columnheading-special' ).escaped();
			} else {
				var siteNameColumnHeading = mw.message( 'wikibase-sitelinks-sitename-columnheading' ).escaped();
			}
