...
wb.ui.PropertyEditTool = wb.utilities.inherit( PARENT, {
	...
	_initToolbar: function() {
		...
		if( this.getOption( 'allowsMultipleValues' ) || this.getOption( 'allowsFullErase' ) ) {
			...
			$( this._toolbar.btnAdd ).on( 'action', $.proxy( function() {
				this.enterNewValue();
			}, this ) );
	...
	isInAddMode: function() {
		...
		for( i = this._editableValues.length; i--; i >= 0 ) {
			editableValue = this._editableValues[ i ];
	...
	*/
	...
	_initSingleValue: function( valueElem, options ) {
		var editableValue, editableValueToolbar,
			self = this;
		...
		editableValue = this.getEditableValuePrototype().newFromDom( valueElem, options );
		this._editableValues.push( editableValue );

		editableValueToolbar = this._buildSingleValueToolbar( options );
	...
	_buildSingleValueToolbar: function( options ) {
		...
		// give the toolbar a edit group with basic edit commands:
			editGroup = new wb.ui.Toolbar.EditGroup( $.extend( {
	...
	enterNewValue: function( value, options ) {
		...
		var newValue,
			newValueElem = this._newEmptyValueDOM(); // get DOM for new empty value
		...
		newValue = this._initSingleValue( newValueElem, options );
	...
	_getFormattedCounterText: function() {
		var numberOfPendingValues = this.getPendingValues().length,
			...
			$counterMsg = wb.utilities.ui.buildPendingCounter(
				numberOfValues,
