 ...
 *
 * @option counterContainers {jQuery} One or more elements that will serve as additional nodes where
 *         the PropertyEditTool will draw and update counters.
 */
wb.ui.PropertyEditTool = util.inherit( PARENT, {
	/**
	 * @const
	...
	_init: function( subject, options ) {
		...
		this._subject.data( this.UI_CLASS, this );
	...
	_initToolbar: function() {
		var self = this,
			toolbarParent = this._getToolbarParent(),
			$toolbar = toolbarParent.children( '.wikibase-toolbar' );
		...
		this._toolbar = $toolbar.data( 'toolbar' );
		this._toolbar.$innerGroup = mw.template( 'wikibase-toolbar', '', '' ).toolbar( {
		...
		this._toolbar.$btnAdd = mw.template(
		...
		this._toolbar.$btnAdd.addClass( this.UI_CLASS + '-toolbarbutton-addbutton' );

		this._toolbar.$innerGroup.data( 'toolbar' ).addElement( this._toolbar.$btnAdd );

		if ( this.getOption( 'allowsMultipleValues' ) ) {
			// enable button only if this is not full yet, overwrite function directly
			this._toolbar.$btnAdd.data( 'toolbarbutton' ).setState = function( state ) {
				var origState = state;
				...
				$.wikibase.toolbarbutton.prototype.setState.call( this, state );

				// only return success if intended state is set now
				return origState === self._toolbar.$btnAdd.data( 'toolbarbutton' ).getState();
			};
			this._toolbar.$btnAdd.data( 'toolbarbutton' ).enable(); // will run the code above initially
		}
	},
	 ...
	 *
	 * @return {jQuery.wikibase.toolbar|null}
	 */
	getToolbar: function() {
	...
	/**
	 * Returns whether there is a toolbar allowing user interaction with this PropertyEditTool.
	 *
	 * @return {boolean}
	 */
	hasToolbar: function() {
	...
	/**
	 * Returns the node the PropertyEditTool's toolbar shall be appended to.
	 *
	 * @return {jQuery}
	 */
	...
	*/
	 ...
	 * @param {Object} [options] EditableValue options
	...
	_initSingleValue: function( valueElem, options, tooltipOptions ) {
		...
		editableValue.setToolbar( editableValueToolbar, tooltipOptions );
	...
	/**
	 * Called whenever an editable value managed by this PropertyEditTool is removed.
	 *
	...
	_editableValueHandler_onAfterRemove: function( editableValue ) {
		...
		this._toolbar.$btnAdd.data( 'toolbarbutton' )[ this.isInAddMode() ? 'disable' : 'enable' ]();
	},

	/**
	 * Returns the index of an EditableValue within this collection. If the element is not part of
	 * this PropertyEditTool, -1 will be returned.
	 *
	 * @param {wikibase.ui.PropertyEditTool.EditableValue} element
	 * @return {number}
	 */
	...
	/**
	 * Builds the toolbar for a single EditableValue.
	 *
	...
	_buildSingleValueToolbar: function( options ) {
		...
		var $editGroup = mw.template( 'wikibase-toolbareditgroup' ).toolbareditgroup( $.extend( {
			// Display remove button if EditableValue represents a list of values:
			displayRemoveButton: this.getOption( 'allowsMultipleValues' )
		}, options ) );

		toolbar.addElement( $editGroup );
		// Remember EditGroup in order to easily attach event handlers within the EditableValue:
	...
	_destroy: function() {
		...
		var $additionalCounterNodes = this.getOption( 'counterContainers' );
		if( $additionalCounterNodes instanceof $ ) {
			$additionalCounterNodes.removeClass( this.UI_CLASS + '-counter' );
		}
	},
	...
	enterNewValue: function( value, options ) {
		...
		if( this.getOption( 'allowsMultipleValues' ) && !this.getOption( 'allowsFullErase' ) ) {
			this._toolbar.$btnAdd.data( 'toolbarbutton' ).disable(); // disable 'add' button...
		}
	 ...
	 * @param {boolean} save Whether save has been triggered.
	 * @param {boolean} wasPending Whether value is a completely new value not yet stored in the
	 *        database.
	 */
	_newValueHandler_onAfterStopEditing: function( newValue, save, wasPending ) {
		this._toolbar.$btnAdd.data( 'toolbarbutton' ).enable(); // ...until stop editing new item
		if( save ) {
			this.refreshView( newValue );
		}
	},
	 ...
	 * @since 0.2
	 *
	 * @param {wikibase.ui.PropertyEditTool.EditableValue} [fromValue] The EditableValue which has
	 ...
	 *
	 * @param {boolean} getPendingValues If set to true, even pending values not yet stored will be
	 *        returned.
	...
	/**
	 * Returns the related property's title.
	 *
	 * @return {string}
	 */
	...
	/**
	 * Defines which editable value should be used for this. Returns the constructor for creating
	 * such a value.
	 *
	 * @return {wb.ui.PropertyEditTool.EditableValue}
	 */
...
wb.utilities.ui.StatableObject.useWith( wb.ui.PropertyEditTool, {
	/**
	 * Determines the state (disabled, enabled or mixed) of all edit tool elements (editable values
	 * and toolbar).
	...
	getState: function() {
		...
		// Consider PropertyEditTool toolbar's state if toolbar is set:
		if( this.hasToolbar() && this._toolbar.isStateChangeable() ) {
			// (bug workaround) If no toolbar is set, do not consider toolbar's state since it
			// always returns ENABLED...
			state = this._toolbar.getState();
		}
	 ...
	 *
	 * @param {number} state
	 * @param {wikibase.ui.PropertyEditTool.EditableValue} [skip] May be one value which shall not
	 *        be affected.
	 */
