...
wb.ui.PropertyEditTool = wb.utilities.inherit( PARENT, {
		...
		.on( 'stopItemPageEditMode',
			function( event, origin, wasPending ) {
				...
				if(
					...
					self._toolbar.$btnAdd.data( 'wbbutton' ).setFocus();
				}
			}
		);
	...
	_initToolbar: function() {
		...
		if ( this.getOption( 'allowsMultipleValues' ) ) {
			...
			this._toolbar.$btnAdd.data( 'wbbutton' ).setState = function( state ) {
				var origState = state;
				...
				$.wikibase.wbbutton.prototype.setState.call( this, state );

				// only return success if intended state is set now
				return origState === self._toolbar.$btnAdd.data( 'wbbutton' ).getState();
			};
			this._toolbar.$btnAdd.data( 'wbbutton' ).enable(); // will run the code above initially
		}
	...
	*/
	...
	_editableValueHandler_onAfterRemove: function( editableValue ) {
		...
		this._toolbar.$btnAdd.data( 'wbbutton' )[ this.isInAddMode() ? 'disable' : 'enable' ]();
	},
	...
	enterNewValue: function( value, options ) {
		...
		if( this.getOption( 'allowsMultipleValues' ) && !this.getOption( 'allowsFullErase' ) ) {
			this._toolbar.$btnAdd.data( 'wbbutton' ).disable(); // disable 'add' button...
		}
	...
	_newValueHandler_onAfterStopEditing: function( newValue, save, wasPending ) {
		this._toolbar.$btnAdd.data( 'wbbutton' ).enable(); // ...until stop editing new item
		if( save ) {
			this.refreshView( newValue );
		}
	},
