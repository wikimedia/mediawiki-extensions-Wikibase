	...
	_init: function( subject, options ) {
		...
		this._options = $.extend(
			...
			PARENT.prototype._options, {
				/**
				 * Defines whether this PropertyEditTool may contain more than one EditableValues
				 * @type {Boolean}
				 */
				allowsMultipleValues: true,

				/**
				 * Defines whether this PropertyEditTool's EditableValues can be removed completely
				 * @type {Boolean}
				 */
				allowsFullErase: false,

				/**
				 * Message displayed when this PropertyEditTool may contain multiple EditableValues but the
				 * maximum number of values has been reached
				 * @type {String}
				 */
	...
	*/
	...
	_getFormattedCounterText: function() {
		...
		var msg = numberOfPendingValues < 1
				? mw.msg( 'wikibase-propertyedittool-counter',
						mw.language.convertNumber( numberOfValues ) )
				: mw.msg(
						'wikibase-propertyedittool-counter-pending',
						mw.language.convertNumber( numberOfValues + numberOfPendingValues ),
						mw.language.convertNumber( numberOfValues ),
		...
		if( msgSpan.length > 0 ) {
			...
			msgSpan.attr( 'title', mw.msg( 'wikibase-propertyedittool-counter-pending-tooltip',
						mw.language.convertNumber( numberOfPendingValues ) ) );
			msgSpan.text( mw.msg( 'wikibase-propertyedittool-counter-pending-pendingsubpart',
						mw.language.convertNumber( numberOfPendingValues ) ) );
			msgSpan.tipsy( {
				'gravity': 'ne'
			} );
