...
wb.ui.Toolbar.EditGroup = wb.utilities.inherit( PARENT,
		...
		$( wb ).on(
			...
			$.proxy(
				function( event ) {
					...
					this.tooltipAnchor.data( 'wblabel' ).getTooltip().setContent(
						mw.message( messageId ).escaped()
					);

					this.tooltipAnchor.data( 'wblabel' ).getTooltip().setGravity( 'nw' );
				}, this
			)
		);
	},
	...
	_initToolbar: function() {
		...
		.wbbutton()
		.on( 'wbbuttonaction', $.proxy( $.NativeEventHandler( 'edit', function() {
			this.toEditMode();
		} ), this ) );
		...
		.wbbutton()
		.on( 'wbbuttonaction', $.proxy( $.NativeEventHandler( 'cancel', function() {
			this.toNonEditMode();
		} ), this ) );
		...
		.wbbutton()
		.on( 'wbbuttonaction', $.proxy( $.NativeEventHandler( 'save', function() {
			this.toNonEditMode();
		} ), this ) );
	...
	destroy: function() {
		...
		if ( this.btnEdit !== null ) {
			if( this.btnEdit.data( 'wbbutton' ) ) {
				this.btnEdit.data( 'wbbutton' ).destroy();
			}
			this.btnEdit.remove();
			this.btnEdit = null;
		}
		if ( this.btnCancel !== null ) {
			if( this.btnCancel.data( 'wbbutton' ) ) {
				this.btnCancel.data( 'wbbutton' ).destroy();
			}
			this.btnCancel.remove();
			this.btnCancel = null;
		}
		if ( this.btnSave !== null ) {
			if( this.btnSave.data( 'wbbutton' ) ) {
				this.btnSave.data( 'wbbutton' ).destroy();
			}
			this.btnSave.remove();
			this.btnSave = null;
		}
		if ( this.btnRemove !== null ) {
			if( this.btnRemove.data( 'wbbutton' ) ) {
				this.btnRemove.data( 'wbbutton' ).destroy();
			}
