...
wb.ui.Toolbar.EditGroup = wb.utilities.inherit( PARENT,
	...
	_initToolbar: function() {
		...
		$( this.btnEdit ).on(
			'action',
			$.NativeEventHandler( 'edit', function() {
				self.toEditMode();
			} )
		);
		...
		$( this.btnCancel ).on(
			'action',
			$.NativeEventHandler( 'cancel', function() {
				self.toNonEditMode();
			} )
		);
		...
		$( this.btnSave ).on(
			'action',
			$.NativeEventHandler( 'save', function() {
				self.toNonEditMode();
			} )
		);
		...
		// The remove button shall always be enabled when it is displayed. When there is no way to
		// perform the remove action, the button will not be shown anyway.
		this.btnRemove.stateChangeable = false;
	...
	/**
	 * TODO: this and the 'render' event should be refactored out of here, they are EditableValue
	 *       specific stuff.
