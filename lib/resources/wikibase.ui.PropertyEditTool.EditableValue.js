 ...
 *        (1) jQuery.Event
 */
SELF = wb.ui.PropertyEditTool.EditableValue = wb.utilities.inherit( PARENT,
	...
	_getToolbarParent: function() {
		...
		this.__toolbarParent = editSection.length > 0 ?
			editSection : $( '<span>' ).appendTo( this._subject );
	...
	save: $.PersistentPromisor( function() {
		...
		if( ! this.isValid() ) { // remove instead! Save equals remove in this case!
			...
			deferred = this.performApiAction( this.API_ACTION.SAVE ) // returns deferred
				.done( $.proxy( function() {
					this._reTransform( true );
	...
	stopEditing: $.PersistentPromisor( function( save ) {
		...
		wasPending = ( promise.promisor.wasPending !== undefined ) ?
			promise.promisor.wasPending : this.isPending();
	...
	performApiAction: function( apiAction ) {
		...
		.done( function( response ) {
			...
			if( didSaveAction ) {
				responseVal = self._getValueFromApiResponse( response.entity );
		...
		.fail( function( textStatus, response ) {
			...
			var action = ( apiAction === self.API_ACTION.SAVE ) ? 'save' : 'remove',
				anchor = ( apiAction === self.API_ACTION.REMOVE ) ?
					self._toolbar.editGroup.btnRemove : self._toolbar.editGroup.btnSave,
	...
	_interfaceHandler_onInputRegistered: function( relatedInterface ) {
		...
		var value = this.getValue(),
			...
			disableSave = ( isInvalid && !this.isEmpty() ) || this.valueCompare( this.getInitialValue(), value ),
			disableCancel;
		...
		*/
		disableCancel = !this.isInEditMode() && !this.isEmpty() ||
	...
	_destroy: function() {
		...
		for ( i in this._interfaces ) { // remove span added in _buildInterfaces()
			this._interfaces[i].destroy();
		}
		this._interfaces = null;
		span = this._subject.children( 'span' ); // span that was inserted in _buildInterfaces()
...
wb.utilities.ui.StatableObject.useWith( SELF, {
	...
	_setState: function( state ) {
		...
		if( this.hasToolbar() ) {
			lockedToolbarState = this.hasLockedToolbarState();
...
SELF.getValueLanguageContextFromDom = function( $subject ) {
	...
	return valueLang ?
		valueLang[0].replace( /\s*wb-value-lang-/g, '' ) : mw.config.get( 'wgUserLanguage' );
};
