...
var SELF = wb.ui.PropertyEditTool.EditableValue = wb.utilities.inherit( PARENT,
	...
	setToolbar: function( toolbar ) {
		...
		if( this.isEmpty() || this.isPending() ) {
			...
			this._toolbar.editGroup.btnEdit.trigger( 'click' );
	...
	_reTransform: function( save ) {
		...
		this._toolbar.editGroup.btnSave.data( 'toolbarbutton' ).removeTooltip();
		this._toolbar.editGroup.tooltipAnchor.data( 'toolbarlabel' ).getTooltip().hide();
		...
		.fail( function( textStatus, response ) {
			...
			var action = ( apiAction === self.API_ACTION.SAVE ) ? 'save' : 'remove',
				anchor = ( apiAction === self.API_ACTION.REMOVE )
					? self._toolbar.editGroup.btnRemove.data( 'toolbarbutton' )
					: self._toolbar.editGroup.btnSave.data( 'toolbarbutton' ),
	...
	_interfaceHandler_onInputRegistered: function( relatedInterface ) {
		...
		*/
		...
		this._toolbar.editGroup.btnSave.data( 'toolbarbutton' )[ disableSave ? 'disable' : 'enable' ]();
		this._toolbar.editGroup.btnCancel.data( 'toolbarbutton' )[ disableCancel ? 'disable' : 'enable' ]();
	...
	_interfaceHandler_onKeyUp: function( relatedInterface, event ) {
		if( event.which === $.ui.keyCode.ESCAPE ) {
			this._toolbar.editGroup.btnCancel.trigger( 'click' );
		}
	},
	...
	_interfaceHandler_onKeyPressed: function( relatedInterface, event ) {
		if( event.which === $.ui.keyCode.ENTER ) {
			if(
				...
				// try to save value
				this._toolbar.editGroup.btnSave.trigger( 'click' );
			}
		}
	},
