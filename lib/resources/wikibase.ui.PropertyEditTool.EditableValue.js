...
var SELF = wb.ui.PropertyEditTool.EditableValue = util.inherit( PARENT,
	...
	_init: function( subject, options, interfaces, toolbar ) {
		...
		if( !this._api ) {
			throw new Error( 'You have to pass an api instance to wikibase.ui.PropertyEditTools' );
		}
	},
	 ...
	 */
	setToolbar: function( toolbar, tooltipOptions ) {
		var self = this,
		...
		tbParent
			.addClass( this.UI_CLASS + '-toolbarparent wb-editsection' )
		...
		.on( 'toolbareditgroupedit', function( event, callback ) {
			...
			// requires that the mode has been changed already in order to detect whether to trigger
		...
		this._toolbar.$editGroup.data( 'toolbareditgroup' ).$tooltipAnchor.wbtooltip(
			$.extend( { content: this.getInputHelpMessage() }, tooltipOptions )
		);
	...
	remove: $.PersistentPromisor( function() {
		...
		var degrade = $.proxy( function() {
			if( !this.preserveEmptyForm ) {
				$( wb ).triggerHandler( 'stopItemPageEditMode', [ this,
	...
	stopEditing: $.PersistentPromisor( function( save ) {
		...
		if ( !save ) {
			...
			if( this.isPending() || ( this.preserveEmptyForm && this.isEmpty() && this.isNew() ) ) { // cancel pending edit...
				...
				$( wb ).triggerHandler( 'stopItemPageEditMode', [ this,
					{ save: save, wasPending: this.isPending() }
				] );
				return promise;
			}
		} else {
	...
	_reTransform: function( save ) {
		...
		if( editGroup.hasButton( 'save' ) && editGroup.getButton( 'save' ).data( 'wbtooltip' ) ) {
			editGroup.getButton( 'save' ).data( 'wbtooltip' ).degrade();
		}
	...
	performApiAction: function( apiAction ) {
		...
		.fail( function( textStatus, response ) {
			...
			if ( apiAction === self.API_ACTION.REMOVE ) {
				 ...
				 */
				$( wb ).triggerHandler( 'stopItemPageEditMode', [ self,
					{ save: false, wasPending: self.isPending() }
				] );
			}
	 ...
	 * @param {jQuery} $anchor Node the error tooltip shall be attached to
	 */
	showError: $.NativeEventHandler( 'showError', function( event, error, $anchor ) {
		...
		$anchor.on( 'wbtooltipafterhide', function() {
	...
	getInputHelpMessage: function() {
		...
		// TODO: Currently shows the code (which is better than nothing) if ULS is not available
		return this._options.inputHelpMessageKey
			? mw.msg( this._options.inputHelpMessageKey, langName || '[' + langCode + ']' )
			: '';
	},
	...
	_interfaceHandler_onInputRegistered: function( relatedInterface ) {
		...
		*/
		...
		var editGroup = this._toolbar.$editGroup.data( 'toolbareditgroup' );
		editGroup[ disableSave ? 'disableButton' : 'enableButton' ]( 'save' );
		editGroup[ disableCancel ? 'disableButton' : 'enableButton' ]( 'cancel' );
		...
		if ( disableSave && disableCancel && this.preserveEmptyForm ) {
			if ( this._propagateInterfaceEvents || this.isNew() ) {
				$( wb ).triggerHandler( 'stopItemPageEditMode', [ this, { wasPending: false } ] );
			}
			this._propagateInterfaceEvents = true;
		} else if ( this.isNew() ) {
	...
	_interfaceHandler_onKeyPressed: function( relatedInterface, event ) {
		if( event.which === $.ui.keyCode.ENTER ) {
			...
			if(
				...
				editGroup.getButton( 'cancel' ).trigger( 'click' );
			} else {
