...
var SELF = wb.ui.PropertyEditTool.EditableValue = wb.utilities.inherit( PARENT,
	...
	_reTransform: function( save ) {
		...
		this._toolbar.editGroup.btnSave.data( 'wbbutton' ).removeTooltip();
		this._toolbar.editGroup.tooltipAnchor.data( 'wblabel' ).getTooltip().hide();
		...
		.fail( function( textStatus, response ) {
			...
			var action = ( apiAction === self.API_ACTION.SAVE ) ? 'save' : 'remove',
				anchor = ( apiAction === self.API_ACTION.REMOVE )
					? self._toolbar.editGroup.btnRemove.data( 'wbbutton' )
					: self._toolbar.editGroup.btnSave.data( 'wbbutton' ),
	...
	_interfaceHandler_onInputRegistered: function( relatedInterface ) {
		...
		*/
		...
		this._toolbar.editGroup.btnSave.data( 'wbbutton' )[ disableSave ? 'disable' : 'enable' ]();
		this._toolbar.editGroup.btnCancel.data( 'wbbutton' )[ disableCancel ? 'disable' : 'enable' ]();
