...
class SiteTitle {
	 ...
	 * @var string
	 */
	protected $normalizedPageName = null;
	...
	public function __construct( $site, $pageName ) {
		if ( $site instanceof Site ) {
			...
			$this->siteId = Utils::trimToNFC( $siteId );
		}
	...
	public function normalizePageName() {
		...
		if ( \Wikibase\Settings::get( 'normalizeItemByTitlePageNames' ) !== true ) {
			...
			$this->normalizedPageName = $site->normalizePageName( $page );
			if ( !is_string( $this->normalizePageName ) ) {
				return null;
			}
