...
class EntityDiffView extends DiffView {
			...
			if ( $op->getType() === 'add' ) {
				...
				if ( !is_string( $newValue ) && $path[0] === 'claim' ) {
					...
					if ( array_key_exists( 'm', $claimDiff ) ) {
						...
						//TODO: we still need to handle qualifiers, etc..
						$newValue = '';
						$oldValue = '';
	 ...
	 * @return string
	 *
	 * TODO: This is just showing all references (and their snak lists) in a list,
	 * as we don't have diffs for references yet.
	 */
