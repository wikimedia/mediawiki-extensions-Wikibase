...
class MultiLangSerializationOptions extends SerializationOptions {
	 ...
	 * too currently, but don't depend on anything in this variable for forward compatibility reasons.
	 *
	 * @since 0.4
	 *
	 * @var null|array of array
	 */
	protected $languageFallbackChains = null;

	/**
	 * Sets the language codes or language fallback chains (keyed by requested language codes) of the languages
	 * for which internationalized data (ie descriptions) should be returned.
	 *
	 * @since 0.2
	 *
	...
	public function setLanguages( array $languages = null ) {
		...
		foreach ( $languages as $languageCode => $languageFallbackChain ) {
			if ( is_numeric( $languageCode ) ) {
				...
				$languageFallbackChain = Utils::getLanguageFallbackChain(
					Language::factory( $languageCode ), Utils::LANGUAGE_FALLBACK_SELF
				);
			}
