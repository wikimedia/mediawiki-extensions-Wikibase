 ...
 *
 * @note Having this interface extend EntityRevisionLookup allows us to keep the logic
 * for purging any caches when an entity is updated local to the respective implementation.
 * If it becomes necessary to have several different lookup instances which would all need
 * to be notified of changes, and be used in different contexts, it would be sensible to no
 * longer have EntityStore extends EntityLookup and EntityRevisionLookup, and rely on some
 * for of event notification service instead.
 *
 * @since 0.5
 *
 * @licence GNU GPL v2+
