...
class TermSqlIndex extends DBAccessBase implements TermIndex {
	...
	public function getLabelWithDescriptionConflicts( $entityType, array $labels, array $descriptions ) {
		...
		$labelConflicts = $this->getLabelConflicts( $entityType, $labels );
	 ...
	 * @param string $entityType
	 * @param string[] $descriptions
	 * @param EntityId[] $entitiesToCheck
	 *
	 * @return Term[]
	 */
	private function getDescriptionConflicts( $entityType, array $descriptions, array $entitiesToCheck ) {
		$options = array(
			'LIMIT' => $this->maxConflicts
		);
		...
		$queryTerms = $this->makeQueryTerms( $descriptions, Term::TYPE_DESCRIPTION );
	 ...
	 *
	 * @param array $base A set of Term groups, keyed by language, as returned by groupByEntity.
	 * @param array $filter A set of Term groups, keyed by language, as returned by groupByEntity.
	 *
