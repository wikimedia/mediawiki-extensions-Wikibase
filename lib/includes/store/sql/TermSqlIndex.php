...
class TermSqlIndex extends DBAccessBase implements TermIndex {
	...
	public function saveTermsOfEntity( EntityDocument $entity ) {
		...
		if ( $ok && $termsToInsert ) {
			...
			$ok = $dbw->deadlockLoop( function() use ( $this, $entity, $termsToInsert, $dbw ) {
				$this->insertTermsInternal( $entity, $termsToInsert, $dbw );
			} );
		}
	 ...
	 * @return boolean Success indicator
	 */
	private function insertTermsInternal( EntityDocument $entity, $terms, DatabaseBase $dbw ) {
		wfProfileIn( __METHOD__ );
