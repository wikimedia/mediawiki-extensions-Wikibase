...
class WikiPageEntityRevisionLookup extends DBAccessBase implements EntityRevisionLookup {
	 ...
	 * @throws StorageException
	 * @return array entityid -> EntityRevision, EntityRedirect or false (if not found)
	 */
	public function getEntityRevisions( array $entityIds, $mode = self::LATEST_FROM_SLAVE ) {
		$entityIds = array_map( array( $this, 'getProperEntityId' ), $entityIds );
	...
	private function selectRevisionInformation( array $entityIds, $connType ) {
		...
		$rows = array();
		foreach ( $entityIds as $entityId ) {
			$rows[$entityId->getSerialization()] = false;

			// This sucks, but is the sanest option given that we will only have few rows in here
			foreach ( $res as $row ) {
				if ( (int)$row->epp_entity_id === $entityId->getNumericId() &&
					$row->epp_entity_type === $entityId->getEntityType() ) {
					$rows[$entityId->getSerialization()] = $row;
					break;
				}
			}
		}
	...
	private function selectPageLatest( array $entityIds, $connType = DB_SLAVE ) {
		...
		foreach ( $entityIds as $entityId ) {
			...
			foreach ( $res as $row ) {
				if ( (int)$row->epp_entity_id === $entityId->getNumericId() &&
					$row->epp_entity_type === $entityId->getEntityType() ) {
					$pageLatest[$entityId->getSerialization()] = (int)$row->page_latest;
					break;
				}
			}
		}
	 ...
	 * @return mixed
	 */
	public function getProperEntityId( EntityId $id ) {
		// Needs to be public for PHP 5.3 compat
