...
class WikiPageEntityLookup extends DBAccessBase implements EntityRevisionLookup {
	...
	private function loadRevisionRow( EntityId $entityId, $revisionId ) {
		...
		if ( !$row ) {
			...
			wfDebugLog(  __CLASS__, __FUNCTION__ . ': try to load '
				. $entityId->getSerialization() . " with $revisionId from DB_MASTER." );
	 ...
	 */
	protected function selectRevisionRow( EntityId $entityId, $revisionId = 0, $connType = DB_READ ) {
		wfProfileIn( __METHOD__ );
	...
	private function loadEntity( $row ) {
		...
		wfDebugLog( __CLASS__, __FUNCTION__ . ': calling getRevisionText() on revision ' . $row->rev_id );
