...
class StringLengthValidator implements ValueValidator {
	 ...
	 * already known to be lnger than the maximum specified in the constructor.
	 *
	 * @param string $value
	 * @param int $maxLength The maximum length after which to truncate.
	 * Note that this is unrelated to the max length we validate against
	 *
	 * @return string
	 */
	private function truncateValue( $value, $maxLength = 32 ) {
		$length = call_user_func( $this->measure, $value );
