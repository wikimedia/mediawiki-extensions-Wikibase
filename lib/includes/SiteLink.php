...
class SiteLink {
	...
	public static function newFromText( $siteID, $page ) {
		...
		$title = static::normalizePageTitle( $site, $page );

		if ( $title === false ) {
			throw new \MWException( "failed to normalize title: $page" );
		}

		return new SiteLink( $site, $title );
	}
	...
	/**
	 * @param Site|String $site      The site the page link points to
	 * @param String $page    The target page's title. This is expected to already be normalized.
	 *
	 * @throws \MWException if the $siteID isn't known.
	 */
	public function __construct( $site, $page ) {
		if ( is_string( $site) ) {
			$this->site = Sites::singleton()->getSiteByGlobalId( $site );

			if ( !$this->site ) {
				throw new \MWException( "unknown site: $site" );
			}
		} else {
			$this->site = $site;
		}
