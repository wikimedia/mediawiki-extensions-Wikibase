...
class WikiPageEntityMetaDataLookup extends DBAccessBase implements WikiPageEntityMetaDataAccessor {
	...
	private function selectRevisionInformationMultiple( array $entityIds, $connType ) {
		...
		list( $where, $extraTables ) = $this->getWhere( $entityIds, $db );

		$res = $db->select( array_merge( $tables, $extraTables ), $fields, $where, __METHOD__, [], $join );
	...
	private function getWhere( array $entityIds, IDatabase $db ) {
		...
		foreach ( $entityIds as $entityId ) {
			...
			$conditions = [
				$db->addQuotes( $title->getDBkey() ) . '=page_title',
				$title->getNamespace() . '=page_namespace',
			];
			...
			if ( $slotRole !== 'main' ) {
				try{
					...
					// The slot role is not get saved, nothing to retrieve..
					continue;
				}

				$conditions[] = $slotRoleId . '=slot_role_id';
				$conditions[] = 'page_latest=slot_revision_id';
				$extraTables = [ 'slots' ];
			}
		...
		return [ $db->makeList( $where, LIST_OR ), $extraTables ];
	}
