...
class TermSqlIndex extends DBAccessBase implements TermIndex, LabelConflictFinder {
	 ...
	 * @param bool $readFullEntityIdColumn Whether it can read term_entity_full_id or not
	 */
	...
	public function deleteTermsOfEntity( EntityId $entityId ) {
		...
		if ( $this->writeFullEntityIdColumn ) {
			$numericEntityId = $dbw->addQuotes( $entityId->getNumericId() );
			$fullEntityId = $dbw->addQuotes( $entityId->getSerialization() );
	...
	private function fetchTerms(
		...
		foreach ( $entityIds as $id ) {
			...
			$fullIds[] = EntityId::splitSerialization( $id->getSerialization() )[2];
