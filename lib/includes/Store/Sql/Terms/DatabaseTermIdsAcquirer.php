...
class DatabaseTermIdsAcquirer implements TermIdsAcquirer {
	 ...
	 * will not result of deleting terms that has been acquired by this acquirer, should
	 * these two in-parallel processes happen to overlap on some existing term ids.
	 * The mechansim of achieving this guarantee is complete under the following two conditions:
	 * - external linking to acquired ids (e.g. using them as foreign keys in other tables)
	 *	 in must happen inside a callback passed through $acquiredIdsConsumerCallback
	 *	 constructor param.
	 * - the in-parallel cleaner is called with set of ids based on the absense of any
	 *	 links to those ids, in the same external places where the callback link to them.
	 *
