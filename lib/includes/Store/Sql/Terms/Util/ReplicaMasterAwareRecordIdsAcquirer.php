...
class ReplicaMasterAwareRecordIdsAcquirer {
	...
	private function insertNonExistingRecordsIntoMaster( array $neededRecords, array $idsToRestore = [] ) {
		...
		$uniqueIdsToRestore = array_values( array_unique( $idsToRestore ) );
		$idToRestoreIndex = count( $uniqueIdsToRestore) - 1;

		if ( $idToRestoreIndex >= 0 ) {
			foreach ( $uniqueRecords as &$record ) {
				if ( $idToRestoreIndex >= 0 ) {
					$record[$this->idColumn] = $uniqueIdsToRestore[$idToRestoreIndex--];
				} else {
					break;
				}
			}
