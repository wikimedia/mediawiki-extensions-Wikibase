...
class ReplicaMasterAwareRecordIdsAcquirer {
	...
	/**
	 * Flag that can be used in the constructor parameter $ignoreReplica.
	 * It will change this object's behavior so that it always queries
	 * master database to find existing items, bypassing replica database
	 * completely.
	 */
	const IGNORE_REPLICA = 0x1;
	 ...
	 * @param int $ignoreReplica if self::IGNORE_REPLICA flag is passed here,
	 *	finding existing records will always be attempted against master.
	 */
	...
	private function isIgnoringReplica() {
		return ( $this->ignoreReplica & self::IGNORE_REPLICA ) !== 0x0;
	}
