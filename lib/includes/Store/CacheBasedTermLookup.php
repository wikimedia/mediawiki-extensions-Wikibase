...
/**
 * TODO: Would this be ever useful?
 */
class CacheBasedTermLookup implements TermLookup {
	...
	public function getLabels(EntityId $entityId, array $languageCodes) {
		...
		foreach ( $languageCodes as $languageCode) {
			$keys[] = $this->makeKey( $entityId, $revisionId, $languageCode, 'label' );
		}
		// TODO: result of getMultiple does not match the expected return value format of getLabels
	...
	public function getDescription(EntityId $entityId, $languageCode) {
		...
		$key = $this->makeKey( $entityId, $revisionId, $languageCode, 'description' );
		return $this->cache->get( $key );
	}
	...
	public function getDescriptions( EntityId $entityId, array $languageCodes ) {
		...
		$keys = [];
		foreach ( $languageCodes as $languageCode) {
			$keys[] = $this->makeKey( $entityId, $revisionId, $languageCode, 'description' );
		}
		// TODO: result of getMultiple does not match the expected return value format of getDescriptions
		return $this->cache->getMultiple( $keys );
	}
