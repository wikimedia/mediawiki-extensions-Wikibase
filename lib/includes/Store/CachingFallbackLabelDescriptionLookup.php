...
class CachingFallbackLabelDescriptionLookup implements LabelDescriptionLookup {
	...
	private function serialize( TermFallback $termFallbackOrNull ) {
		return json_encode( [
			self::FIELD_LANGUAGE => $termFallbackOrNull->getActualLanguageCode(),
			self::FIELD_VALUE => $termFallbackOrNull->getText(),
			self::FIELD_REQUEST_LANGUAGE => $termFallbackOrNull->getLanguageCode(),
			self::FIELD_SOURCE_LANGUAGE => $termFallbackOrNull->getSourceLanguageCode(),
		] );
	}
