...
class ChangeHandler {
	 ...
	 */
	public function coalescRuns( array $changes ) {
		$coalesced = array();
		...
		foreach ( $changes as $change ) {
			...
			if ( !$break && ( $change instanceof ItemChange ) ) {
				...
				$break = isset( $siteLinkDiff[ $this->siteGlobalId ] );
			}
	 ...
	 */
	public function coalescChanges( array $changes ) {
		$coalesced = array();

		$changesByEntity = $this->groupChangesByEntity( $changes );
		foreach ( $changesByEntity as $entityChanges ) {
			$entityChanges = $this->coalescRuns( $entityChanges );
			$coalesced = array_merge( $coalesced, $entityChanges );
		}

		//FIXME: can't use usort because of https://bugs.php.net/bug.php?id=50688
	...
	public static function compareChangesByTimestamp( Change $a, Change $b ) {
		return wfTimestamp( $a->getTime() ) - wfTimestamp( $b->getTime() );
	}
	...
	public function handleChanges( array $changes ) {
		$changes = $this->coalescChanges( $changes );
