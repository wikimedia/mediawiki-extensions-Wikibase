...
class YearMonthTimeParser extends StringValueParser {
	...
	protected function stringParse( $value ) {
		//REGEX Matches Year and month separated by a separator, \pL matches letters outside of the ASCII range
		if( !preg_match( '/^([\d\pL\.\,]+)[\/\-\s]([\d\pL\.\,]+)$/', trim( $value ), $matches ) ) {
			throw new ParseException( 'Failed to parse year and month' );
		}
		list( ,$a, $b ) = $matches;

		$aIsInt = $this->isFormattedNumber( $a );
		$bIsInt = $this->isFormattedNumber( $b );
	 ...
	 * @return bool
	 */
	private function isFormattedNumber( $value ) {
