...
class DateTimeParser extends StringValueParser {
	...
	protected function stringParse( $value ) {
		...
		try{
			...
			if(
				$options->hasOption( TimeParser::OPT_PRECISION ) &&
				$options->hasOption( TimeParser::OPT_CALENDER ) &&
				is_int( $options->getOption( TimeParser::OPT_PRECISION ) )
			) {
				return new TimeValue(
					$timeString,
					0, 0, 0,
					$options->getOption( TimeParser::OPT_PRECISION ),
					$calanderModelParser->parse( $options->getOption( TimeParser::OPT_CALENDER ) )
				);
			}
