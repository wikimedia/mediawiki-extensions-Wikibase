...
class MWTimeIsoParser extends StringValueParser {
	...
	private function reconvertOutputString( $value ) {
		...
		foreach( self::$precisionMsgKeys as $msgKeySuffix => $possiblePrecisions ) {
			...
			//Due to an oddity in PHP explode does return 2 values in some versions but does not in others
			//For this reason we force a ' ' either side of our string before exploding to ensure 2 results
			//This is then removed again in the trims just below
			list( $start, $end ) = explode( strval( self::$NUM_PLACEHOLDER ), '!' . $string . '!', 2 );
//			list( $start, $end ) = preg_split( '/' . self::$NUM_PLACEHOLDER . '/', '!' . $string . '!', 2 );
