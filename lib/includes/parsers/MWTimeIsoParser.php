...
class MWTimeIsoParser extends StringValueParser {
	...
	private function reconvertOutputString( $value ) {
		...
		foreach( self::$precisionMsgKeys as $msgKeySuffix => $possiblePrecisions ) {
			...
			$res = explode( strval( self::$NUM_PLACEHOLDER ), $string, 2 );

			wfDebugLog( 'MWTimeIsoParser',
			"Value: " . $value . "\n" .
			"Message Key: " . self::MSG_PREFIX . $msgKeySuffix . "\n" .
			"Place Holder: " . strval( self::$NUM_PLACEHOLDER ) . "\n" .
			"String: " . $string . "\n".
			'Imploded around | : ' . implode( '|', $res ) . "\n"
		);
