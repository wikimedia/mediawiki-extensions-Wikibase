...
class MWTimeIsoParser extends StringValueParser {
	...
	/**
	 * @var array message key suffixes to be prefixed by MSG_PREFIX to get the message
	 */
	...
	private function reconvertOutputString( $value ) {
		foreach( self::$precisionMsgKeys as $msgKeySuffix ) {
			$msgKey = self::MSG_PREFIX . $msgKeySuffix;
			...
			//Try to work out why this would ever not explode....
			if( !strstr( $messageText, '$1' ) ) {
				throw new RuntimeException( "'\$1' is not found in string '{$messageText}' for msg key '{$msgKey}' " );
			}

			list( $start, $end ) = explode( '$1' , $messageText , 2 );
