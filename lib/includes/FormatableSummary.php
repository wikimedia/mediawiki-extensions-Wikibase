...
interface FormatableSummary {
	 ...
	 * - $1: Number of auto-summary arguments (@see getAutoSummaryArgs). Note that the auto-summary
	 *   arguments themselve are not passed as parameters to the message.
	 * - $2: Language code of the edited content, or empty if not applicable (@see getLanguageCode).
	 ...
	 * label, description, or set of aliases in a specific language. Not set if not applicable, e.g.
	 * when a quantity value was edited.
	 *
	 ...
	 * (@see getAutoSummaryArgs), the user's comment will be appended to the auto-summary, separated
	 * by a comma.
	 *
	 * @return string|null
	 */
