...
class EntityChange extends DiffChange {
	...
	public function setField( $name, $value ) {
		if ( $name === 'object_id' || $name === 'type' ) {
			throw new RuntimeException( "Use setEntityId or setAction to set field \"$name\"!" );
		}
	...
	public function getType() {
		...
		return 'wikibase-' . $this->entityId->getEntityType() . '~' . $this->action;
	}
	...
	public function getObjectId() {
		...
		return strtolower( $this->entityId->getSerialization() );
	}
	...
	public function __toString() {
		...
		$fields = array_merge( [ 'object_id' => $this->getObjectId() ], $this->getFields() );
		$info = $this->hasField( 'info' ) ? $this->getField( 'info' ) : array();
