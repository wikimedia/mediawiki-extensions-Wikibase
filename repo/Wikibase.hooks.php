...
final class RepoHooks {
	...
	public static function onSetupAfterCache() {
		...
		$contentHandlerInstantiators = $wikibaseRepo->getContentHandlerInstantiators();

		foreach ( $contentHandlerInstantiators as $entityType => $instantiator ) {
			if ( isset( $contentModelIds[$entityType] ) ) {
				$model = $contentModelIds[$entityType];

				// Construct a closure, because instantiators expect the entity type as a parameter.
				$wgContentHandlers[$model] = function () use ( $instantiator, $entityType ) {
