...
final class RepoHooks {
	...
	public static function onOutputPageBeforeHTML( OutputPage $out, &$html ) {
		...
		if ( $placeholders ) {
			...
			$userLanguages = $userLanguageLookup->hasSpecifiedAlternativeLanguages();
			// The variable is not guaranteed to be defined anyway
			if ( $userLanguages ) {
				$out->addJsConfigVars( 'wbUserLanguages', $userLanguages );
			}
		}
