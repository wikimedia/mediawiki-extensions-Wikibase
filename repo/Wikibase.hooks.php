...
final class RepoHooks {
	...
	public static function onLinkBegin( $skin, $target, &$html, array &$customAttribs, &$query, &$options, &$ret ) {
		...
		$labelData = $languageFallbackChain->extractPreferredValue( $entity->getLabels() );
		$descriptionData = $languageFallbackChain->extractPreferredValue( $entity->getDescriptions() );
