...
final class RepoHooks {
	...
	public static function onNewRevisionFromEditComplete( $article, Revision $revision, $baseID, User $user ) {
		...
		if ( $entityContentFactory->isEntityContentModel( $article->getContent()->getModel() ) ) {
			...
			// Nothing to do if an existing redirect was edited or the page was just created.
			if ( $action !== null ) {
	...
	public static function onArticleUndelete( Title $title, $created, $comment ) {
		...
		// TODO: Notify the client when a redirect is undeleted!
		...
		$rev = Revision::newFromId( $revisionId );
	...
	public static function onUserGetDefaultOptions( array &$defaultOptions ) {
		...
		$defaultLanguageCode = $defaultOptions['language'];
		$defaultOptions[ 'wb-languages-' . $defaultLanguageCode ] = 1;
	...
	public static function onOutputPageBodyAttributes( OutputPage $out, Skin $sk, array &$bodyAttrs ) {
		...
		if ( $entityContentFactory->isEntityContentModel( $out->getTitle()->getContentModel() ) ) {
			...
			if ( $entityContent !== null && !$entityContent->isRedirect() ) {
				...
				$entityHandler = $entityContent->getContentHandler();
				// TODO: preg_replace kind of ridiculous here, should probably change the ENTITY_TYPE constants instead
				$entityType = preg_replace( '/^wikibase-/i', '', $entityHandler->getEntityType() );
	...
	private static function newItemHandler() {
		...
		return new ItemHandler( $entityPerPage, $termIndex, $codec, array( $validator ),
			$errorLocalizer, $siteLinkStore
		);
	}
	...
	private static function newPropertyHandler() {
		...
		return new PropertyHandler( $entityPerPage, $termIndex, $codec, array( $validator ),
			$errorLocalizer, $propertyInfoStore
		);
	}
