...
class DescriptionsField implements WikibaseIndexField {
	...
	public function getMappingField( SearchEngine $engine, $name ) {
		...
		if ( !( $field instanceof SearchIndexFieldDefinition ) ) {
			return null;
		}

		$field->setMergeCallback( function ( $self, $that ) use ( $field ) {
			return $field;
		} );
		...
		foreach ( $this->languages as $language ) {
			$subfields[$language] = $engine->makeSearchFieldMapping( "$name.$language",
