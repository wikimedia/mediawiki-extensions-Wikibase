...
class EntitySearchElastic implements EntitySearchHelper {
	...
	private function expandGenericProfile( $languageCode, array $profile ) {
		$res = [
			'any' => $profile['any'],
			'tie-breaker' => $profile['tie-breaker'] ?? 0,
			'space-discount' => $profile['space-discount'] ?? 1,
		];
		$res['language-chain'] = $this->languageChainFactory
			->newFromLanguageCode( $languageCode )
			->getFetchLanguageCodes();
		$res["{$languageCode}-exact"] = $profile['lang-exact'];
		$res["{$languageCode}-folded"] = $profile['lang-folded'];
		$res["{$languageCode}-prefix"] = $profile['lang-prefix'];
	...
	protected function getElasticSearchQuery(
		...
		if ( $textExact !== $text ) {
			...
			$fieldsExact[] = [ "labels.{$languageCode}.prefix", $profile["{$languageCode}-prefix"] ];
		} else {
