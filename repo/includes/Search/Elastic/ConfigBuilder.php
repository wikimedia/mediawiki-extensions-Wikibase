...
class ConfigBuilder {
	...
	/**
	 * Check analyzers for usage of filters that are never defined.
	 * If we find any, and they were not previously defined,
	 * create a default definition for them, so that they would be
	 * defined explicitly.
	 *
	 * @param array[] $langConfig Language configuration being processed
	 * @param array[] $standardFilters Existing filters list
	 * @param array[] $defaultFilters List of default filters already in the config
	 */
	private function resolveDefaultFilters( &$langConfig, $standardFilters, $defaultFilters ) {
		foreach ( [ 'plain', 'plain_search', 'text', 'text_search' ] as $analyzer ) {
			if ( empty( $langConfig['analyzer'][$analyzer]['filter'] ) ) {
				continue;
			}
			foreach ( $langConfig['analyzer'][$analyzer]['filter'] as $filterName ) {
				if ( !isset( $langConfig['filter'][$filterName] ) &&
					!isset( $standardFilters[$filterName] ) &&
					!isset( $defaultFilters[$filterName] )
				) {
					// we have a filter that has not been defined anywhere
					// Define it so it does not clash with redefined filter later
					$langConfig['filter'][$filterName] = [ 'type' => $filterName ];
				}
			}
		}
	}
	...
	public function buildConfig( &$config ) {
		...
		foreach ( $this->languageList as $lang ) {
			...
			$this->resolveDefaultFilters( $langConfig, $config['filter'], $defaultFilters );
