...
class EntityFullTextQueryBuilder implements FullTextQueryBuilder {
	 ...
	 */
	private function getDelegateBuilder( SearchContext $searchContext ) {
		$feature = [];
		$config = $searchContext->getConfig();
		$profileService = $config->getProfileService();
		// searching only article namespaces - use parent
		$builderSettings = $profileService->loadProfile( SearchProfileService::FT_QUERY_BUILDER,
				$searchContext->getProfileContext() );
		$delegate = new $builderSettings['builder_class']( $config, $feature, $builderSettings['settings'] );
		if ( !( $delegate instanceof FullTextQueryBuilder ) ) {
