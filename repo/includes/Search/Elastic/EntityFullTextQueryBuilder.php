...
class EntityFullTextQueryBuilder implements FullTextQueryBuilder {
	 ...
	 */
	public function __construct( SearchConfig $config, array $feature, array $settings, WikibaseRepo $repo = null ) {
		// Unfortunately, we can't always get repo in the ctor since CirrusSearch controls construction
		if ( is_null( $repo ) ) {
	...
	protected function buildEntitySearch( WikibaseRepo $repo, SearchContext $searchContext, $term ) {
		...
		foreach ( $searchLanguageCodes as $fallbackCode ) {
			...
			if ( empty( $this->searchSettings['useStemming'][$fallbackCode]['query'] ) ) {
				...
				$fieldsTokenized[] = [ "descriptions.{$fallbackCode}.plain", $profile['lang-partial'] ];
			}
		...
		$query->setMinimumShouldMatch( 0.2 );
