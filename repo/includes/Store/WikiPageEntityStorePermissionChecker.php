...
class WikiPageEntityStorePermissionChecker implements EntityPermissionChecker {
	...
	public function getPermissionStatusForEntity(
		...
		if ( $id === null ) {
			...
			$result = $this->getPermissionStatusForEntityType(
				$user,
				self::ACTION_CREATE,
				$entityType,
				$quick
			...
			$result->merge(
				$this->getPermissionStatusForEntityType( $user, $action, $entityType, $quick )
			);
			return $result;
		}
	 ...
	 */
	private function getPermissionStatusForEntityType( User $user, $action, $type, $quick = '' ) {
