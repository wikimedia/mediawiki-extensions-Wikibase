...
class WikiPageEntityStorePermissionChecker implements EntityPermissionChecker {
	...
	public function getPermissionStatusForEntityId( User $user, $action, EntityId $entityId, $quick = '' ) {
		...
		return $this->checkPermission( $user, $action, $title, $entityId->getEntityType(), $quick );
	}
	...
	public function getPermissionStatusForEntityType( User $user, $action, $type, $quick = '' ) {
		...
		return $this->checkPermission( $user, $action, $title, $type, $quick );
	}
	...
	private function getMediaWikiPermissionsToCheck( $permission, $entityType ) {
		if ( $permission === EntityPermissionChecker::ACTION_CREATE ) {
		...
		if ( $permission === EntityPermissionChecker::ACTION_EDIT_TERMS ) {
		...
		if ( $permission === EntityPermissionChecker::ACTION_MERGE ) {
		...
		if ( $permission === EntityPermissionChecker::ACTION_REDIRECT ) {
		...
		if ( $permission === EntityPermissionChecker::ACTION_EDIT ) {
			return [ 'read', 'edit' ];
		}

		if ( $permission === EntityPermissionChecker::ACTION_READ ) {
			return [ 'read' ];
		}
