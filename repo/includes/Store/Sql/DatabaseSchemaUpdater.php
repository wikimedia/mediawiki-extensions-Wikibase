...
use Onoi\MessageReporter\ObservableMessageReporter as ObservableReporter;
...
use Wikibase\TermStore\TermStore;
...
class DatabaseSchemaUpdater {
	...
	private function createTermsStore( DatabaseUpdater $updater, IDatabase $db ) {
		$reporter = new ObservableReporter();

		$updater->output( "Installing/updating Wikibase Term Store\n" );

		$reporter->registerReporterCallback( function( $message ) use ( $updater ) {
			$updater->output( "    ..." . $message . "\n" );
		} );

		$this->newTermStore( $db )->install( $reporter );
	}

	private function newTermStore( IDatabase $db ): TermStore {
