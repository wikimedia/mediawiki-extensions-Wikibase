...
class SqlChangeDispatchCoordinator implements ChangeDispatchCoordinator {
	...
	private function getCandidateClients() {
		...
		$this->stats->timing(
			...
			( $endTimeSelectMax - $startTimeSelectMax ) * 1000
		);
		$this->stats->timing(
			'wikibase.repo.SqlChangeDispatchCoordinator.getCandidateClients-time.selectCandidates',
			( $endTimeSelectCandidates - $startTimeSelectCandidates ) * 1000
		);
	...
	public function lockClient( $siteID ) {
		...
		$this->stats->timing(
			...
			( $endTimeSelectClientSite - $startTimeSelectClientSite ) * 1000
		);
		$this->stats->timing(
			'wikibase.repo.SqlChangeDispatchCoordinator.lockClient-time.engageLock',
			( $endTimeEngageLock - $startTimeEngageLock ) * 1000
		);
