...
class ChangeOpQualifier extends ChangeOp {
	 ...
	 * @return bool
	 *
	 * @todo this could be made simpler, we do not need $snakHash because there are getSnak($snak) and reoveSnak($snak) methods
	 * @todo split up into several methods
	 */
	...
	protected function addQualifier( Snaks $qualifiers, Summary $summary ) {
		//TODO: check if snak with same hash already exists and throw error in that case
		$qualifiers->addSnak( $this->snak );
		//TODO: add the mainsnak as autocomment-arg & change messages
		#$summary->addAutoCommentArgs( $this->getSnakSummaryArgs( $claim->getMainSnak() ) );
	 ...
	 * @since 0.4
	 *
	 * @param Sanks $qualifiers
	 * @param Summary $summary
	 */
	protected function setQualifier( Sanks $qualifiers, Summary $summary ) {
	 ...
	 * @param Summary $summary
	 */
	protected function removeQualifiers( Snaks $qualifiers, Summary $summary ) {
