...
class ShowSearchHitHandler {
	 ...
	 * @param $html
	 */
	public function showPlainSearchHit( SpecialSearch $searchPage, SearchResult $result, array $terms,
	                                 &$link, &$redirect, &$section, &$extract, &$score, &$size, &$date, &$related, &$html
	) {
		...
		if ( $termData !== null ) {
			self::addDescription( $link, $termData, $searchPage );
		}
	 ...
	 * @param $html
	 */
	public static function showEntityResultHit( SpecialSearch $searchPage, EntityResult $result, array $terms,
		&$link, &$redirect, &$section, &$extract, &$score, &$size, &$date, &$related, &$html
	) {
		$extract = '';
	...
	public static function onShowSearchHitTitle(
		...
		if ( $result instanceof EntityResult ) {
			...
			self::getLink( $linkHandler, $result, $title, $titleSnippet, $attributes,
	 ...
	 * @param $titleSnippet
	 */
	public function showPlainSearchTitle( Title $title, &$titleSnippet ) {
		if ( $this->isTitleEntity( $title ) ) {
