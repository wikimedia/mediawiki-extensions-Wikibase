...
class LinkBeginHookHandler {
	...
	public function doOnLinkBegin( Title $target, &$html, array &$customAttribs, RequestContext $context ) {
		...
		if ( !$targetIsForeignEntityPage && !$this->entityNamespaceLookup->isEntityNamespace( $target->getNamespace() ) ) {
			return;
		}
	...
	private function isForeignEntityPage( LinkTarget $target ) {
		...
		return substr( $target->getText(), 0, strlen( 'Special:EntityPage/' ) ) === 'Special:EntityPage/';
	}
