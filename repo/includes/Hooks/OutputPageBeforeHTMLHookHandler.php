...
class OutputPageBeforeHTMLHookHandler {
	...
	private function getEntityViewPlaceholderExpander( OutputPage $out ) {
		...
		if ( $revisionId === null || Revision::newFromId( $revisionId )->isCurrent() ) {
			...
			$labelsProvider = $emptyEntity instanceof LabelsProvider ? new TermIndexLabelsProvider( $this->termIndex, $entityId ) : null;
			$descriptionsProvider = $emptyEntity instanceof DescriptionsProvider ? new TermIndexDescriptionsProvider( $this->termIndex, $entityId ) : null;
			$aliasesProvider = $emptyEntity instanceof AliasesProvider ? new TermIndexAliasesProvider( $this->termIndex, $entityId ) : null;
		} else {
