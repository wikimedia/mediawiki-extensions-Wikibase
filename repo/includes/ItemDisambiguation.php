...
class ItemDisambiguation {
	...
	public function getResultHtml( array $searchResult ) {
		...
		$result .= $this->getLabelHtml(
			...
			$searchResult[TermSearchInteractor::MATCHEDTERMS_KEY]
		);
	 ...
	 * @param Term[][] $matchedTerms
	 *
	 * @return string HTML
	 */
	private function getLabelHtml( $displayTerms, $matchedTerms ) {
		if( array_key_exists( TermIndexEntry::TYPE_LABEL, $displayTerms ) ) {
			$displayLabel = $displayTerms[TermIndexEntry::TYPE_LABEL];
		}
		// We might have multiple matches here so just take the first one
		$matchedTerm = $matchedTerms[TermIndexEntry::TYPE_LABEL][0];
	...
	private function getDescriptionHtml( $displayTerms, $entityId ) {
		if( array_key_exists( TermIndexEntry::TYPE_DESCRIPTION, $displayTerms ) ) {
