...
class SpecialSetSiteLink extends SpecialSetEntity {
	...
	protected function setValue( $entityContent, $language, $value ) {
		...
		if( $site === false ) {
			throw new MWException( $this->msg( 'wikibase-setentity-invalid-langcode', $language )->text() ); // should not happen (strike out?)
		}
		if ( $site->normalizePageName( $value ) === false && $value !== '' ) {
			throw new MWException( $this->msg( 'wikibase-error-ui-no-external-page' )->text() );
		}
