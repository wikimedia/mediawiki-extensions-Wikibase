...
class SpecialMergeItems extends SpecialWikibasePage {
	...
	public function execute( $subPage ) {
		...
		if ( $this->modifyEntity( $status ) ) {
			if ( !$status()->isGood() ) {
				...
				$fromEditEntity = new EditEntity( $this->fromEntityContent, $this->getUser(), false, $this->getContext() );
				...
				if ( !$fromStatus->isGood() ) {
					...
					$toEditEntity = new EditEntity( $this->toEntityContent, $this->getUser(), false, $this->getContext() );
	...
	protected function getSummary( $direction, $id ) {
		...
		$summary = new Summary( 'wbmergeitems', $direction, null, array( $entityIdFormatter->format( $id ) ) );
		return $summary;
	}
	...
	protected function getFormElements() {
		$fromId = $this->fromEntityContent ? $this->fromEntityContent->getTitle()->getText() : '';
		$toId = $this->toEntityContent ? $this->toEntityContent->getTitle()->getText() : '';
