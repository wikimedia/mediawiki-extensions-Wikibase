...
class SpecialItemByLabel extends SpecialItemResolver {
	...
	public function execute( $subPage ) {
		...
		$parts = ( $this->subPage === '' ) ? array() : explode( '/', $this->subPage, 3 );
		...
		$description = $request->getVal( 'description', isset( $parts[2] ) ? $parts[2] : '' );
		if ( $description === '' ) {
			$description = null;
		}
		$itemContents = array();
		...
		if ( isset( $language ) && isset( $label ) ) {
			...
			if ( count( $itemContents ) === 1 ) {
				wfDebug(">>>>>>>>>> count single: " .  count( $itemContents ));
				//wfDebug(print_r($itemContents));
				$this->displayItem( $itemContents[0] );
			}
			elseif ( count( $itemContents ) > 1 ) {
				wfDebug(">>>>>>>>>> count multiple: " .  count( $itemContents ));
		...
		if ( $itemContents === array() ) {
			$this->switchForm( $language, $label, $description );
		}
	}
	 ...
	 */
	protected function switchForm( $langCode, $label, $description ) {
		global $wgScript;

		// this makes the usecase become really weird
		$description = null;
		...
		if ( isset( $langCode ) || isset( $label ) ) {
			$this->getOutput()->addHTML(
				...
				. $this->msg( $languageName !== $langCode ? 'wikibase-itembylabel-nothing-found' : 'wikibase-itembylabel-invalid-langcode' )
					->params( $langCode, $label, $description )
					->parse()
		...
		$this->getOutput()->addHTML(
			...
			// this makes the usecase become really weird
			//. Xml::inputLabel( $this->msg( 'wikibase-itembylabel-lookup-description' )->text(), 'description', 'descriptionname', 72, $description ? $description : '' )
		...
		if ( isset( $langCode ) && isset( $label ) ) {
			$this->getOutput()->addHTML(
				...
				. $this->msg( 'wikibase-itembylabel-create' )
					->params( $langCode ? $langCode : '', $label ? $label : '', $description ? $description : '' )
					->parse()
				. Html::closeElement( 'div' )
			);
		}
