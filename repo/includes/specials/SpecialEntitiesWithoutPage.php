...
abstract class SpecialEntitiesWithoutPage extends SpecialWikibaseQueryPage {
	...
	public function execute( $subPage ) {
		...
		if ( $this->language !== '' && !in_array( $this->language, \Wikibase\Utils::getLanguageCodes() ) ) {
			$output->addWikiMsg( 'wikibase-entitieswithoutpage-invalid-language', $this->language );
			$this->language = '';
		}
		...
		if ( $this->type !== null && !in_array( $this->type, $possibleTypes ) ) {
			$output->addWikiMsg( 'wikibase-entitieswithoutpage-invalid-type', $this->type );
			$this->type = null;
		}
		$typeSelect = new XmlSelect( 'type', 'wb-entitieswithoutpage-type', $this->type );
		$typeSelect->addOption( $this->msg( 'wikibase-entitieswithoutpage-label-alltypes' )->text(), '' );
		...
		$output->addHTML(
			...
			Html::element(
				...
				$this->msg( 'wikibase-entitieswithoutpage-label-language' )->text()
			) . ' ' .
			Html::input(
				'language',
				$this->language,
			...
			Html::element(
				...
				$this->msg( 'wikibase-entitieswithoutpage-label-type' )->text()
			) . ' ' .
			$typeSelect->getHTML() . ' ' .
			Html::input(
				'submit',
				$this->msg( 'wikibase-entitieswithoutpage-submit' )->text(),
				'submit',
				array(
