...
class SpecialCreateItem extends SpecialWikibasePage {
	...
	public function execute( $subPage ) {
		...
		if ( ( isset( $label ) && $label != '' ) || ( isset( $description ) && $description != '' ) ) {
			...
			if ( !$status->isOK() ) {
				throw new \MWException( $this->msg( 'wikibase-createitem-error' )->text() );
			}
	...
	public function createItemForm( $label, $description ) {
		$this->getOutput()->addHTML(
				...
				. Xml::inputLabel( $this->msg( 'wikibase-createitem-label' )->text(), 'label', 'wb-createitem-label', 12, $label ? htmlspecialchars( $label ) : '' )
				. Xml::closeElement('br')
				. Xml::inputLabel( $this->msg( 'wikibase-createitem-description' )->text(), 'description', 'wb-createitem-description', 36, $description ? htmlspecialchars( $description ) : '' )
				. Xml::closeElement( 'br' )
