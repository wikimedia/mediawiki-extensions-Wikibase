...
class SpecialItemByTitle extends SpecialItemResolver {
	...
	public function execute( $subPage ) {
		...
		$siteId = htmlspecialchars( $request->getVal( 'site', isset( $parts[0] ) ? $parts[0] : '' ) );
		$page = htmlspecialchars( $request->getVal( 'page', isset( $parts[1] ) ? $parts[1] : '' ) );
		$itemContent = null;
	...
	protected function switchForm( $siteId, $page ) {
		...
		$this->getOutput()->addHTML(
			...
			. Xml::inputLabel( $this->msg( 'wikibase-itembytitle-lookup-site' )->text(), 'site', 'sitename', 12, $siteId ? $siteId : '' )
			. Xml::inputLabel( $this->msg( 'wikibase-itembytitle-lookup-page' )->text(), 'page', 'pagename', 36, $page ? $page : '' )
		...
		if ( $validSite && isset( $page ) ) {
			$this->getOutput()->addHTML(
				...
				. $this->msg( 'wikibase-itembytitle-create' )
					->params( $siteId ? $siteId : '', $page ? $page : '' )
					->parse()
				. Html::closeElement( 'div' )
			);
		}
