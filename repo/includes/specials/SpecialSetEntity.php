...
abstract class SpecialSetEntity extends SpecialWikibasePage {
	 ...
	 */
	public function __construct( $valueName, $title = "" ) {
		parent::__construct( $title == "" ? "Set" . $valueName : $title );
		...
		try {
			if ( $language !== null ) {
				if ( ( !Language::isValidBuiltInCode( $language ) or ( !in_array( $language, \Wikibase\Utils::getLanguageCodes() ) ) ) ) {
					throw new MWException( $this->msg( 'wikibase-setentity-invalid-langcode', $language )->text() );
				}
			}

			if ( $entityContent === null && $value !== null && $rawId !== null ) {
				throw new MWException( $this->msg( 'wikibase-setentity-invalid-id', $rawId )->text() );
			}
			...
			if ( $entityContent !== null && $language !== null && $this->getRequest()->wasPosted() ) {
				...
				if ( !$editEntity->isSuccess() ) {
					throw new MWException( $editEntity->getStatus()->getMessage() );
				}
				else {
