   ...
   /**
	...
	private function addExpandedValue( $target, $propertyValueQName, DataValue $value, array $props) {
		...
		foreach( $props as $prop => $type ) {
			...
			if ( $type == 'url' ) {
				...
				// TODO: temp fix
				$rdfValue = "\"{$this->fixDateTime($data)}\"^^xsd:dateTime";
			} else {
	...
	private function addStatementForString( $target, $propertyValueQName, $dataType,
		...
		if ( $dataType == 'commonsMedia' ) {
			...
			$rdfValue = "<" . str_replace(array(">", "\\"), array("%3E", "%5C"), $value->getValue()) . ">";
		} else {
	...
	/**
	 * Temporary fix - Blazegraph doesn't like dates starting with +
	 * This will be gone when we add custom date parser
	 * @param string $dt
	 */
	private function fixDateTime($dt) {
		if($dt[0] == "+") {
			return substr($dt, 1);
		}
		return $dt;
	...
	private function addStatementForTime( $target, $propertyValueQName, $dataType,
		...
		// TODO: we may need to support more precise date types, such as xsd:gYear
		$this->lines[$target][] = "$propertyValueQName \"{$this->fixDateTime($value->getTime())}\"^^xsd:dateTime";
