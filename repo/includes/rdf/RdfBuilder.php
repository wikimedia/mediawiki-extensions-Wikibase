...
class RdfBuilder {
	 ...
	 */
	public function __construct( SiteList $sites, $baseUri, $dataUri, EntityLookup $entityLookup, $dataFormat, EasyRdf_Graph $graph = null ) {
		if ( !$graph ) {
		...
		$this->produceWhat = $this->getProduce();
	 ...
	 * @return integer
	 */
	protected function getProduce() {
		switch($this->dataFormat) {
			case 'simple':
				return self::PRODUCE_BEST_STATEMENTS | self::PRODUCE_SITELINKS;
			case 'full':
				return self::PRODUCE_ALL;
			case 'dump':
				return self::PRODUCE_ALL_STATEMENTS | self::PRODUCE_BEST_STATEMENTS | self::PRODUCE_QUALIFIERS | self::PRODUCE_REFERENCES | self::PRODUCE_SITELINKS;
		}
		return self::PRODUCE_ALL_STATEMENTS | self::PRODUCE_QUALIFIERS | self::PRODUCE_REFERENCES | self::PRODUCE_SITELINKS;
	}
	 ...
	 * @return string
	 */
	private function getCommonsName( $file ) {
	...
	private function addStatement( EntityId $entityId, Statement $statement, $shortForm ) {
		...
		if ( $this->shouldProduce( self::PRODUCE_QUALIFIERS ) ) {
		...
		if ( $this->shouldProduce( self::PRODUCE_REFERENCES ) ) {
			$statementResource = $this->getStatementResource( $statement );
	 ...
	 *        	Target node to which we're attaching the snak
	 * @param SnakObject $snak
	 *        	Snak object
		...
		switch ( $typeId ) {
			...
			case 'string' :
				if ( $dataType == 'commonsMedia' ) {
					$target->addResource( $propertyValueQName, $this->getCommonsName( $value->getValue() ) );
				} elseif ( $dataType == 'url' ) {
	...
	public function addEntity( Entity $entity ) {
		...
		if ( $this->shouldProduce( self::PRODUCE_SITELINKS ) && $entity instanceof Item ) {
			$this->addSiteLinks( $entity );
		}

		if ( $this->shouldProduce( self::PRODUCE_ALL_STATEMENTS | self::PRODUCE_BEST_STATEMENTS ) && $entity instanceof EntityDocument ) {
