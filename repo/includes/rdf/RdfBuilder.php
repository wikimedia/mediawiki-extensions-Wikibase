...
class RdfBuilder {
	...
	public function __construct( SiteList $sites, $baseUri, $dataUri,
			EntityLookup $entityLookup, $flavor, RdfWriter $emitter ) {

		$this->documentWriter = $emitter;
	...
	private function addLabels( Entity $entity ) {
		...
		foreach ( $entity->getLabels() as $languageCode => $labelText ) {
			...
			//FIXME: RdfWriter needs to optimize for repeated about() calls for the same resource!
			$this->entityWriter->about( self::NS_ENTITY, $entityLName )
	...
	private function addStatement( EntityId $entityId, Statement $statement ) {
		...
		if ( $this->shouldProduce( RdfProducer::PRODUCE_REFERENCES ) ) {
			foreach ( $statement->getReferences() as $ref ) { //FIXME: split body into separate method
				...
				$this->statementWriter->about( self::NS_STATEMENT, $statementLName ) //FIXME: optimized for repeated about()
	 ...
	 *
	 * @param RdfWriter $emitter The emitter to receive the property value (must be primed to expect a predicate).
	 * @param Snak $snak Snak object
	 * @param string $propertyNamespace The property namespace for this snak
	 * @param bool $simpleValue
	 */
	private function addSnak( RdfWriter $emitter, Snak $snak, $propertyNamespace, $simpleValue = false ) {
		...
		switch ( $snak->getType() ) {
			...
			case 'somevalue' :
				...
				$emitter->say( $propertyNamespace, $propertyValueLName )->is( self::NS_ONTOLOGY, 'Somevalue' );
				break;
			case 'novalue' :
				$propertyValueLName = $this->getEntityLName( $propertyId );

				$emitter->say( $propertyNamespace, $propertyValueLName )->is( self::NS_ONTOLOGY, 'Novalue' );
				break;
			default:
	 ...
	 *
	 * @return string the id of the value node, for use with the '_' prefix.
	 */
	private function addExpandedValue( DataValue $value, array $props ) {
		$node = $this->valueWriter->blank(); //TODO: avoid bnode, use a hash based qname, like WDT does
		$this->valueWriter->about( '_', $node )->a( self::NS_ONTOLOGY, 'Value' );
	 ...
	 * @param bool $simpleValue
	 */
	private function addStatementValue( RdfWriter $emitter, PropertyId $propertyId,
		...
		if( !is_callable( array( $this, $typeId ) ) ) {
			...
			$this->$typeId( $emitter, $propertyNamespace, $propertyValueLName, $dataType, $value, $simpleValue );
		}
	 ...
	 * @param bool $simpleValue
	 */
	private function addStatementForWikibaseEntityid( RdfWriter $emitter, $propertyValueNamespace, $propertyValueLName, $dataType,
			EntityIdValue $value, $simpleValue = false ) {

		$entityId = $value->getValue()->getEntityId();
		$entityLName = $this->getEntityLName( $entityId );
		$emitter->say( $propertyValueNamespace, $propertyValueLName )->is( self::NS_ENTITY, $entityLName );
	 ...
	 * @param bool $simpleValue
	 */
	private function addStatementForString( RdfWriter $emitter, $propertyValueNamespace, $propertyValueLName, $dataType,
			StringValue $value, $simpleValue = false ) {
		if ( $dataType == 'commonsMedia' ) {
			$emitter->say( $propertyValueNamespace, $propertyValueLName )->is( $this->getCommonsURI( $value->getValue() ) );
		} elseif ( $dataType == 'url' ) {
			$emitter->say( $propertyValueNamespace, $propertyValueLName )->is( $value->getValue() );
		} else {
			$emitter->say( $propertyValueNamespace, $propertyValueLName )->text( $value->getValue() );
		}
	}
	 ...
	 * @param bool $simpleValue
	 */
	private function addStatementForMonolingualtext( RdfWriter $emitter, $propertyValueNamespace, $propertyValueLName, $dataType,
			MonolingualTextValue $value, $simpleValue = false ) {
		$emitter->say( $propertyValueNamespace, $propertyValueLName )->text( $value->getText(), $value->getLanguageCode() );
	}
	 ...
	 * @param bool $simpleValue
	 */
	private function addStatementForTime( RdfWriter $emitter, $propertyValueNamespace, $propertyValueLName, $dataType,
		...
		$emitter->say( $propertyValueNamespace, $propertyValueLName )->value( $timestamp, 'xsd', 'dateTime' );
...
// 							'after'=> 'dateTime',
			...
			$emitter->say( $propertyValueNamespace, $propertyValueLName."-value" )->is( '_', $valueLName );
		}
	}
	 ...
	 * @param bool $simpleValue
	 */
	private function addStatementForQuantity( RdfWriter $emitter, $propertyValueNamespace, $propertyValueLName, $dataType,
			QuantityValue $value, $simpleValue = false ) {
		$emitter->say( $propertyValueNamespace, $propertyValueLName )->value( $value->getAmount(), 'xsd', 'decimal' );
	 ...
	 * @param bool $simpleValue
	 */
	private function addStatementForGlobecoordinate( RdfWriter $emitter, $propertyValueNamespace, $propertyValueLName, $dataType,
			GlobeCoordinateValue $value, $simpleValue = false ) {

		$point = "Point({$value->getLatitude()} {$value->getLongitude()})";
		$emitter->say( $propertyValueNamespace, $propertyValueLName )->value( $point, self::NS_GEO, "wktLiteral" );
