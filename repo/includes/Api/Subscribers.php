...
class GetSubscribers extends ApiQueryBase {
	 ...
	 */
	public function __construct( ApiQuery $mainModule, $moduleName, $modulePrefix = 'wbs' ) {
	...
	private function formatResult( ResultWrapper $res, $limit, array $props ) {
		...
		foreach ( $res as $row ) {
			...
			if ( $entry ) {
				$entry['subscribers'][] = $this->addSubsciber( $row, isset( $props['url'] ) );
			} else {
	...
	private function buildEntry( &$entry, $row, $url ) {
		$entry = [ 'subscribers' => [ $this->addSubsciber( $row, $url ) ] ];
	 ...
	 * @return string[]
	 */
	private function addSubsciber( \stdClass $row, $url ) {
	...
	protected function getExamplesMessages() {
		return [
			"action=wbsubscribers&wbgsentities=Q42" =>
				"apihelp-query+wbsubscribers-example-1",
			"action=wbsubscribers&wbgsentities=Q42&wbgsprops=url" =>
