...
class EditEntity extends ModifyEntity {
	...
	public function __construct( ApiMain $mainModule, $moduleName, $modulePrefix = '' ) {
		...
		$this->changeOpsDeserializers = $wikibaseRepo->getChagneOpDeserializerCallbacks();
	}
	...
	private function getChangeOps( array $changeRequest, EntityDocument $entity ) {
		...
		$type = $entity->getType();
		if ( isset( $this->changeOpsDeserializers[$type] ) ) {
			$changeOp = call_user_func(
				[ $this->changeOpsDeserializers[$type], 'createEntityChangeOp' ],
				$changeRequest
			);
