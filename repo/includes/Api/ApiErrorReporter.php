...
class ApiErrorReporter {
	...
	public function dieStatus( StatusValue $status, $errorCode, $httpRespCode = 0, $extradata = array() ) {
		...
		$msg = $this->getMessageForCode( $errorCode, $extradata );

		if ( $msg ) {
			$this->addMessageToResult( $msg, $extradata );

			// NOTE: ApiUsageException::__construct takes a Status object, but has no room for
			// the error code.
			...
			throw new ApiUsageException( $this->apiModule, $status, $httpRespCode );
		}
	}
	...
	public function dieError( $description, $errorCode, $httpRespCode = 0, $extradata = array() ) {
		$message = $this->getMessageForCode( $errorCode, $extradata );
	 ...
	 * @return Message|null
	 */
	private function getMessageForCode( $errorCode, $extradata = [] ) {
		$messageKey = "wikibase-api-$errorCode";
		$message = wfMessage( $messageKey );

		return $message->exists() ? $message : null;
	}
