...
class LinkedDataHandler {
	...
	public function getRdfForItem( EntityContent $entity, $format ) {
		...
		foreach ( $entity->getEntity()->getClaims() as $claims ) {
			...
			foreach( $claimsByProperty as $claims ) {
				...
				foreach( $claims as $claim ) {
					if ( $claim->getMainSnak()->getType() === 'value' ) {
						...
						$statementUri = $entityUri . "/Statement" . strval( $counter );
		...
		// TODO: decide on $format which function to call
		return self::outputRdfXml( $this->graph );
	}

	private function outputRdfXml( ) {
		$data = $this->graph->serialise( 'rdfxml' );
		if (!is_scalar( $data )) {
