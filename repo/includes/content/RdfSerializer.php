...
class RdfSerializer {
	...
	protected function newRdfGraph() {
		...
		EasyRdf_Namespace::set( 'wikibase_ontology', self::ontologyBaseUri . '/ontology/' );

		//register namespaces
		EasyRdf_Namespace::set( 'wikibase_entity', $this->uriBase . '/entity/' );
		EasyRdf_Namespace::set( 'wikibase_data', $this->uriBase . '/data/' );
		EasyRdf_Namespace::set( 'wikibase_property', $this->uriBase . '/property/' );
		EasyRdf_Namespace::set( 'wikibase_value', $this->uriBase . '/value/' );
		EasyRdf_Namespace::set( 'wikibase_qualifier', $this->uriBase . '/qualifier/' );
		EasyRdf_Namespace::set( 'wikibase_statement', $this->uriBase . '/statement/' );
	...
	public function buildGraphForEntity( Entity $entity, \Revision $revision = null ) {
		...
		$entityUri = 'wikibase_id:' . ucfirst( $entity->getId() );
		...
		foreach( $claimsByProperty as $claims ) {
			...
			foreach( $claims as $claim ) {
				...
				if ( $snak instanceof PropertyValueSnak ) {
					...
					//FIXME: sane mechanism for extending this? Some kind of expert logic?

					$counter += 1;
				}

				//FIXME: handle NoValueSnak and SomeValueSnak!
			}
		}

		// TODO: sitelinks, use skos:isPrimarySubjectOf (not foaf:primaryTopic)
