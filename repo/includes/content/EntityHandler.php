...
abstract class EntityHandler extends ContentHandler {
	...
	public function makeEntityRedirectContent( EntityRedirect $redirect ) {
		...
		if ( !defined( 'WB_EXPERIMENTAL_FEATURES' ) || !WB_EXPERIMENTAL_FEATURES ) {
			...
			return $contentClass::newRedirect( $redirect, $title );
		} else {
			return null;
		}
	}
	...
	public function makeEntityContent( Entity $entity ) {
		...
		if ( ! $content->isValid() ) {
			throw new InvalidArgumentException( 'The provided Entity is incomplete (e.g. missing an EntityId)' );
		}
		...
		if ( !$entity ) {
			...
			if ( $redirect === null ) {
				throw new MWContentSerializationException(
					'The serialized data contains nither an Entity nor an EntityRedirect!'
				);
			}
