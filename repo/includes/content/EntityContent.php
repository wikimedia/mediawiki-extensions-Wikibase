...
abstract class EntityContent extends AbstractContent {
	...
	protected function addDataToParserOutput( ParserOutput $pout, EntityRevision $revision ) {
		...
		$this->applyEntityPageProperties( $pout, $revision->getEntity() );
	}
	 ...
	 * @param Entity $entity
	 */
	private function applyEntityPageProperties( ParserOutput $output, Entity $entity ) {
		$properties = $this->getEntityPageProperties( $entity );
	 ...
	 */
	protected function getEntityPageProperties( Entity $entity ) {
		$properties = array(
	 ...
	 * @return int
	 */
	protected function getEntityStatus( Entity $entity ) {
		if ( $entity->isEmpty() ) {
			return self::STATUS_EMPTY;
		} elseif ( !$entity->hasClaims() ) {
