...
abstract class EntityContent extends AbstractContent {
	 ...
	 *
	 * @todo FIXME: Handle redirects correctly, see bug 65585.
	 *
	...
	public function getDiff( EntityContent $toContent ) {
		...
		if ( $fromContent->isRedirect() ) {
			$fromContent = $this->getContentHandler()->makeEmptyContent();
		}

		if ( $toContent->isRedirect() ) {
			$toContent = $this->getContentHandler()->makeEmptyContent();
		}

		return $fromContent->getEntity()->getDiff( $toContent->getEntity() );
	}

	/**
	 * Returns a patched version of this EntityContent
	 *
	 * @todo FIXME: Handle redirects correctly, see bug 65585.
	 *
	 * @param Diff $patch
	 ...
	 */
	public function getPatched( Diff $patch ) {
		if ( $this->isRedirect() ) {
			throw new \LogicException( 'Can\'t yet patch a redirect!'  );
		}

		$patched = $this->copy();
		$patched->getEntity()->patch( $patch );
		return $patched;
	}
