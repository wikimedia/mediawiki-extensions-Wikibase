...
class SqlStore implements Store {
	...
	private function updateTermsTable( \DatabaseUpdater $updater, DatabaseBase $db ) {
		...
		// note: we just check the term_search index, but we use this info
		// to trigger a number of different updates.
		// todo: trigger on $db->fieldType( 'wb_terms', 'term_row_id' ) !== 'BIGINT'
		// instead, pending fieldType being implemented across database types.
