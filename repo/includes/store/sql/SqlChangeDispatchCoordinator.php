...
class SqlChangeDispatchCoordinator implements ChangeDispatchCoordinator {
	 ...
	 */
	private function getClientLB( $wiki ) {
		return wfGetLB( $wiki );
	}
	 ...
	 */
	private function getClientMaster( $wiki ) {
	 ...
	 */
	private function releaseClientMaster( $wiki, \DatabaseBase $db  ) {
	...
	public function lockClient( $siteID ) {
		...
		try {
			...
			if ( $lock === false ) {
				...
				$this->warn( "Warning: Failed to acquire lock on $wikiDB for site $siteID!" );
