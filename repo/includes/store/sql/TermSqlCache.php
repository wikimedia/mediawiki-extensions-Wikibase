...
class TermSqlCache implements TermCache {
	...
	protected function getEntityTerms( Entity $entity ) {
		...
		foreach ( $terms as &$term ) {
			$term['term_lowercase_text'] = strtolower( Utils::squashToNFC( $term['term_text'] ) );
		}
	...
	/**
	 * Rebuild the lowercase index field term_lowercase_text from the source term_text field.
	 *
	 * FIXME: for some unknown reason some rows are skipped in the rebuild
	 *
	 * @since 0.2
	 */
	public function rebuildLowercaseIndex() {
		$dbw = wfGetDB( DB_MASTER );
		...
		while ( $hasMoreResults ) {
			...
			$continuationTerm = $this->rebuildLowercaseIndexForTerms( $terms, $dbw, $limit );
	 ...
	 */
	protected function rebuildLowercaseIndexForTerms( Iterator $terms, DatabaseBase $dbw, $limit ) {
		$termNumber = 0;
		...
		foreach ( $terms as $term ) {
			$dbw->update(
				$this->tableName,
				array(
					'term_lowercase_text' => strtolower( Utils::squashToNFC( $term->term_text ) )
				),
				array(
