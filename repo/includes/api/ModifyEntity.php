	...
	protected function parseSitelinkBadges( array $badgesParams ) {
		...
		foreach ( $badgesParams as $badgeSerialization ) {
			...
			if ( is_null( $itemTitle ) ) {
				...
				$this->dieUsage( "Badges: no item found matching ID $badgeSrialization", 'no-such-entity-id' );
			}
			...
			if ( is_null( $itemContent ) ) {
				...
				wfProfileOut( __METHOD__ );
				$this->dieUsage( "Badges: can't access item content of " . $itemTitle->getPrefixedDBkey() . ", revision may have been deleted.", 'no-such-entity' );
			}
