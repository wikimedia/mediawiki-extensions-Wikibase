...
class ApiErrorReporter {
	...
	public function dieStatus( Status $status, $code, $httpStatusCode = 0, $extradata = array() ) {
		...
		//XXX: when to prefer $statusCode over $code?
	 ...
	 * @throws LogicException
	 */
	public function dieMessage( Message $message, $code, $httpStatusCode = 0, $extradata = array() ) {
	...
	protected function convertMessageToResult( Message $message ) {
		...
		$html = $this->forceMessageLanguage( $message, $this->language )->parse();
