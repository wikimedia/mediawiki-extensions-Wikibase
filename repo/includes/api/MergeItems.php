...
class MergeItems extends ApiWikibase {
	 ...
	 * @return array
	 */
	private function mergeAliases( $fromContent, $toContent ) {
		$changeOps = array();
	...
	private function validateItemContents( $fromContent, $toContent ) {
		if( $fromContent->getEntity()->getId() === $toContent->getEntity()->getId() ){
	...
	private function getMergeItemsChangeops( $fromContent, $toContent ) {
		...
		$changeOps = array_merge_recursive( $changeOps, $this->mergeAliases( $fromContent, $toContent ) );
