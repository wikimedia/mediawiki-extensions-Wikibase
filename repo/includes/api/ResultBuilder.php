...
class ResultBuilder {
	 ...
	 */
	public function __construct( $result, $serializerFactory = null ) {
	 ...
	 * @param array|string $path where the data is located
	 */
	public function addReference( Reference $reference, $path ) {
		$serializer = new ReferenceSerializer( $this->serializationOptions );
		$value = $serializer->getSerialized( $reference );
		$this->addValue( $path, $value, $reference->getHash(), 'reference' );
	}
