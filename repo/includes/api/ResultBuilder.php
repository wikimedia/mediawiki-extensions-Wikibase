...
class ResultBuilder {
	 ...
	 * @since 0.5
	 *
	 * @deprecated this is doing dumb shitm, please die...
	 *
	 ...
	 * @param string $tag tag name to use for elements of $values
	 */
	public function setListNew( $path, $name, array $values, $tag ) {
		$this->checkPathType( $path );
		Assert::parameterType( 'string', $name, '$name' );
		Assert::parameterType( 'string', $tag, '$tag' );
	...
	public function addAliasGroupList( AliasGroupList $aliasGroupList, $path ) {
		...
		$this->setListNew( $path, 'aliases', $values, 'language' );
	}
	...
	public function addSiteLinkList( SiteLinkList $siteLinkList, $path, $addUrl = false ) {
		...
		$this->setListNew( $path, 'sitelinks', $values, 'sitelink' );
	}
	...
	public function addStatements( array $statements, $path, $props = 'all' ) {
		...
		$this->setListNew( $path, 'claims', $values, 'property' );
	}
