...
class ResultBuilder {
	...
	public function addLabels( array $labels, $path, $name = 'labels', $tag = 'label' ) {
		$labelSerializer = new LabelSerializer( $this->getSerializationOptions() );
	...
	public function addDescriptions( array $descriptions, $path, $name = 'descriptions', $tag = 'description' ) {
		$descriptionSerializer = new DescriptionSerializer( $this->getSerializationOptions() );
	...
	public function addAliases( array $aliases, $path, $name = 'aliases', $tag = 'alias' ) {
		$aliasSerializer = new AliasSerializer( $this->getSerializationOptions() );
	...
	public function addSiteLinks( array $siteLinks, $path, $name = 'sitelinks', $tag = 'sitelink', $options ) {
		...
		$options->merge( $this->getSerializationOptions() );
		...
		if ( isset( $options ) ) {
			if ( in_array( EntitySerializer::SORT_ASC, $options ) ) {
				...
				$options->setOption( EntitySerializer::OPT_SORT_ORDER, EntitySerializer::SORT_DESC );
			}

			if ( in_array( 'url', $options ) ) {
				$options->addToOption( EntitySerializer::OPT_PARTS, "sitelinks/urls" );
			}
	...
	public function addClaims( array $claims, $path, $name = 'claims', $tag = 'claim' ) {
		$claimSerializer = new ClaimsSerializer( $this->getSerializationOptions() );
