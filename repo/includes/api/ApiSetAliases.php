...
class ApiSetAliases extends ApiModifyItem {
	...
	protected function autoComment( array $params, $plural = 1 ) {
		if ( isset( $params['set'] ) ) {
			if ( isset( $params['add'] ) ) {
				$comment = "set-add-language-aliases";
			}
			elseif ( isset( $params['remove'] ) ) {
				$comment = "set-remove-language-aliases";
			}
			else {
				$comment = "set-language-aliases";
			}
		}
		elseif ( isset( $params['add'] ) ) {
			$comment = "add-language-aliases";
		}
		elseif ( isset( $params['remove'] ) ) {
			$comment = "remove-language-aliases";
		}
		if ( isset($comment) ) {
	...
	protected function autoSummary( array $params ) {
		...
		if ( isset( $params['set'] ) ) {
			if ( isset( $params['add'] ) ) {
				$summary = self::pickValuesFromParams( $params, 'set', 'add' );
			}
			elseif ( isset( $params['remove'] ) ) {
				$summary = self::pickValuesFromParams( $params, 'set', 'remove' );
			}
			else {
				$summary = self::pickValuesFromParams( $params, 'set' );
			}
		}
		elseif ( isset( $params['add'] ) ) {
			$summary = self::pickValuesFromParams( $params, 'add' );
		}
		elseif ( isset( $params['remove'] ) ) {
			$summary = self::pickValuesFromParams( $params, 'remove' );
		}
