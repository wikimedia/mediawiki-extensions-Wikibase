	...
	protected function autoComment( array $params, $available=128 ) {
		...
		if ( 1<$count ) {
			$comment = "set-language-attributes:" . $params['language']
				. SUMMARY_GROUPING_STR
				. FormatSummary::pickValuesFromParams( $params, $available - strlen( "set-language-attributes:" . $params['language'] ), 'badge', 'label', 'description' );
		}
		elseif ( isset( $params['label'] ) ) {
			$comment = "set-language-label:" . $params['language']
				. SUMMARY_GROUPING_STR
				. FormatSummary::pickValuesFromParams( $params, $available - strlen( "set-language-label:" . $params['language'] ), 'label');
		}
		elseif ( isset( $params['description'] ) ) {
			$comment = "set-language-description:" . $params['language']
				. SUMMARY_GROUPING_STR
				. FormatSummary::pickValuesFromParams( $params, $available - strlen( "set-language-description:" . $params['language'] ), 'description' );
		}
		else {
			$comment = '';
		}
