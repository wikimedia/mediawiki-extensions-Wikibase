...
class ApiSearchEntities extends ApiBase {
	...
	*/
	public function execute() {
		...
		$hits = $this->searchEntities( $params['language'], $params['text'], $params['type'] );
		...
		foreach ( $hits as $hit ) {
			...
			if ( $entity->getLabel( $params['language'] ) !== false ) {
				...
				$pos = strpos( $entity->getLabel( $params['language'] ), $params['text'] );
				if ( $pos !== false ) {
					$score = strlen( $params['text'] ) / strlen( $entity->getLabel( $params['language'] ) );
				}
			}
			...
			if ( $entity->getAliases( $params['language'] ) !== array() ) {
				...
				foreach ( $entity->getAliases( $params['language'] ) as $alias ) {
					$pos = strpos(  $alias, $params['text'] );
					if ( $pos !== false ) {
						$aliasscore = strlen( $params['text'] ) / strlen( $alias );
						if ( $aliasscore > $score ) {
							$score = $aliasscore;
						}
					}
