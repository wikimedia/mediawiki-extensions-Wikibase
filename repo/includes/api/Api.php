...
abstract class Api extends \ApiBase {
	 ...
	 */
	public function checkPermissions( Item $item, User $user, array $params ) {
		if ( Settings::get( 'apiInDebug' ) && !Settings::get( 'apiDebugWithRights', false ) ) {
			return \Status::newGood();
		}

		$permissions = $this->getRequiredPermissions( $item, $params );
		$status = \Status::newGood();

		foreach ( $permissions as $perm ) {
			$permStatus = $item->checkPermission( $perm, $user, true );
