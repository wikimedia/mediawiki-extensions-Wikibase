...
class EditEntity extends ModifyEntity {
	...
	protected function getRequiredPermissions( EntityId $entityId = null ) {
		if ( $entityId === null ) {
			throw new InvalidArgumentException( 'Can not edit entity with no id' );
		}
		...
		if ( !$this->entityExists( $entityId ) ) {
			...
			if ( $entityId->getEntityType() !== 'item' ) {
				$permissions[] = $entityId->getEntityType() . '-create';
			}
		}
	 ...
	 * @param int $revId
	 */
	private function validateDataProperties( $data, Entity $entity, $revId = 0 ) {
	...
	private function checkEntityType( $data, Entity $entity ) {
		if ( isset( $data['type'] ) && $data['type'] !== $entity->getType() ) {
			$this->dieError(
	...
	private function validateMultilangArgs( $arg, $langCode ) {
		...
		if ( !empty( $this->validLanguageCodes )
			&& !array_key_exists( $arg['language'], $this->validLanguageCodes )
		) {
