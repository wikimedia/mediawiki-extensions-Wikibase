...
class EditEntity extends ModifyEntity {
	...
	protected function modifyEntity( EntityContent &$entityContent, array $params ) {
		...
		$currentRevision = $this->entityRevisionLookup->getEntityRevision( $entity->getId() );
		if ( $params['clear'] ) {
			// If we are not clearing the current revision then die!
			if( !$entityContent->getEntityRevision()->getRevision() === $currentRevision->getRevision() ) {
				wfProfileOut( __METHOD__ );
				$this->dieUsage(
					'Tried to clear entity using baserevid not equal to that of latest revision',
					'editconflict'
				);
			}
			$entity->clear();
