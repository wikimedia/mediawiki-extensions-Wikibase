		...
		if( array_keys( $claims ) !== range( 0, count( $claims ) - 1 ) ){
			foreach( $claims as $subClaims ){
				$changeOps = array_merge( $changeOps,
					$this->getRemoveClaimsChangeOps( $subClaims, $guidGenerator , $guidValidator, $guidParser ),
					$this->getModifyClaimsChangeOps( $subClaims, $guidGenerator , $guidValidator, $guidParser ) );
			}
		} else {
			$changeOps = array_merge( $changeOps,
				$this->getRemoveClaimsChangeOps( $claims, $guidGenerator , $guidValidator, $guidParser ),
