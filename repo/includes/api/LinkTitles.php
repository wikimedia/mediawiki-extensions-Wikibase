...
class LinkTitles extends ApiWikibase {
	...
	public function execute() {
		...
		if ( $params['fromsite'] === $params['tosite'] ) {
			...
			$this->dieUsage( 'The from site can not match the to site' , 'invalidparam' );
		}

		if ( !( strlen( $params['fromtitle'] ) > 0 && strlen( $params['totitle'] ) > 0 ) ) {
			wfProfileOut( __METHOD__ );
			$this->dieUsage( 'The from title can not match the to title' , 'invalidparam' );
		}
		...
		elseif ( $fromId === $toId ) {
			// noop
			wfProfileOut( __METHOD__ );
			$this->dieUsage( 'Common item detected', 'commonitem' );
		}
		else {
			// dissimilar items
			wfProfileOut( __METHOD__ );
			$this->dieUsage( 'No common item detected' , 'nocommonitem' );
		}
	...
	public function getPossibleErrors() {
		return array_merge( parent::getPossibleErrors(), array(
			...
			array( 'code' => 'commonitem', 'info' => $this->msg( 'wikibase-api-commonitem' )->text() ),
			array( 'code' => 'nocommonitem', 'info' => $this->msg( 'wikibase-api-nocommonitem' )->text() ),
		) );
	}
