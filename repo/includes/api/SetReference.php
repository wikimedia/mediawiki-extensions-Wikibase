...
class SetReference extends ApiWikibase {
	...
	protected function getSnaks( $rawSnaks ) {
		...
		if ( !is_array( $rawSnaks ) || !count( $rawSnaks ) ) {
			$this->dieUsage( 'No snaks or invalid JSON given', 'invalidjson' );
		}
		...
		try {
			foreach ( $rawSnaks as $byPropertySnaks ) {
				if ( !is_array( $byPropertySnaks ) ) {
					$this->dieUsage( 'Invalid snak JSON given', 'invalidjson' );
				}
			...
			$this->dieUsage( 'Invalid snak JSON given. IllegalValueException', 'invalidjson' );
		}
	...
	protected function updateReference( Entity $entity, $statementGuid, Snaks $snaks, $refHash = null ) {
		...
		if ( !$claims->hasClaimWithGuid( $statementGuid ) ) {
			$this->dieUsage( 'Could not find the statement' , 'nosuchstatement' );
		}
		...
		if ( $refHash !== null ) {
			if ( $references->hasReferenceHash( $refHash ) ) {
				...
				$this->dieUsage( 'The statement does not have any associated reference with the provided reference hash', 'nosuchreference' );
			}
		}
	...
	public function getPossibleErrors() {
		return array_merge( parent::getPossibleErrors(), array(
			...
			array( 'code' => 'nosuchstatement', 'info' => $this->msg( 'wikibase-api-nosuchstatement' )->text() ),
			array( 'code' => 'notstatement', 'info' => $this->msg( 'wikibase-api-notstatement' )->text() ),
			array( 'code' => 'nosuchreference', 'info' => $this->msg( 'wikibase-api-nosuchreference' )->text() ),
		) );
	}
