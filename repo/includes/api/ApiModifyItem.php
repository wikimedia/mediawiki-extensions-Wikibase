...
abstract class ApiModifyItem extends Api {
	...
	public function execute() {
		...
		if ( Settings::get( 'apiDeleteEmpty' ) && $itemContent->isEmpty() ) {
			...
			// This could include additional information
			$commentary = array();
			if ($comment) {
				$commentary[] = $comment;
			}
			if ($summary) {
				$commentary[] = $lang->truncate( $summary, SUMMARY_MAX_LENGTH - strlen( $comment ) );
			}
			// Do the actual save, or if it don't exist yet create it.
			$status = $itemContent->save( implode( ' ', $commentary ), $user, $this->flags  );
			$success = $status->isOK();
