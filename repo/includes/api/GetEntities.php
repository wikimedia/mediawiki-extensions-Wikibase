...
class GetEntities extends ApiWikibase {
	...
	public function execute() {
		...
		if ( !( isset( $params['ids'] ) XOR ( isset( $params['sites'] ) && isset( $params['titles'] ) ) ) ) {
			...
			$this->dieUsage( 'Either provide the item "ids" or pairs of "sites" and "titles" for corresponding pages', 'id-xor-wikititle' );
		}
		...
		if ( !isset( $params['ids'] ) ) {
			...
			if ( $numSites === 0 || $numTitles === 0 ) {
				wfProfileOut( __METHOD__ );
				$this->dieUsage( 'Either provide the item "id" or pairs of "site" and "title" for a corresponding page', 'id-xor-wikititle' );
			}
			else {
				$idxSites = 0;
				$idxTitles = 0;
