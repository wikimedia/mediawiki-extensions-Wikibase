...
class SearchEntities extends ApiBase {
	 ...
	 */
	private function getRankedMatches( $text, $entityType, $languageCode, $limit ) {
		...
		if ( $entityId !== null ) {
			$allSearchResults[$nextSearchKey] = array(
				...
				'matchedTerms' => array(),
				'displayTerms' => array(), //TODO inject label, desc and aliases here?
			);
		...
		foreach( array( false, true ) as $prefixSearch ) {
			...
			$searchResults = $this->searchEntities( $text, $entityType, $languageCode, $missing, $prefixSearch );
