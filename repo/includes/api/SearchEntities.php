...
class SearchEntities extends ApiBase {
	...
	private function getSearchEntries( array $params ) {
		...
		foreach ( $matches as $match ) {
			...
			if( array_key_exists( TermIndexSearchInteractor::MATCHEDTERMTYPE_KEY, $match) ) {
				$entry['match']['type'] = $match[TermIndexSearchInteractor::MATCHEDTERMTYPE_KEY];
			}
			if( array_key_exists( TermIndexSearchInteractor::MATCHEDTERM_KEY, $match) ) {
	 ...
	 *           Value: array( displayTerms => Term[]|Term[][], matchedTerm => Term, matchedTermType => string )
	...
	private function getRankedMatches( $text, $entityType, $languageCode, $limit, $strictLanguage ) {
		...
		foreach( array( false, true ) as $prefixSearch ) {
			...
			$extraSearchResults = $this->searchEntities(
				$text,
				$entityType,
				$languageCode,
				$missing,
			...
			$allSearchResults = $this->mergeSearchResults( $allSearchResults, $extraSearchResults, $limit );
		}
	...
	private function mergeSearchResults( $searchResults, $newSearchResults, $limit ) {
		...
		foreach( $newSearchResults as $searchResult ) {
