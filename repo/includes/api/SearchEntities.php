...
class SearchEntities extends ApiBase {
	...
	private function getSearchEntries( array $params ) {
		...
		foreach ( $matches as $match ) {
			...
			$entry['matched'] = $this->termsToArray( $match['matchedTerms'] );
			$entries[] = $entry;
		}
		return $entries;
	}
	 ...
	 *           Value: array( displayTerms => Term[]|Term[][], matchedTerms => Term[][] )
	...
	private function getRankedMatches( $text, $entityType, $languageCode, $limit, $strictLanguage ) {
		...
		if ( $entityId !== null ) {
			$allSearchResults[$nextSearchKey] = array(
				...
				TermSearchInteractor::MATCHEDTERMS_KEY => array(),
		...
		foreach( array( false, true ) as $prefixSearch ) {
			...
			foreach( $searchResults as $searchResult ) {
				...
				$entityId = $searchResult[TermSearchInteractor::ENTITYID_KEY];
				$entityIdString = $entityId->getSerialization();
				if( !array_key_exists( $entityIdString, $entityIdMap ) ) {
					$allSearchResults[$nextSearchKey] = array(
						TermSearchInteractor::ENTITYID_KEY => $searchResult[TermSearchInteractor::ENTITYID_KEY],
						TermSearchInteractor::DISPLAYTERMS_KEY => $searchResult[TermSearchInteractor::DISPLAYTERMS_KEY],
					);
					$entityIdMap[$entityIdString] = $nextSearchKey;
					$nextSearchKey++;
				}
				// Add all matched terms into the result
				$allSearchResults[$entityIdMap[$entityIdString]] = $this->mergeSearchResultMatchedTerms(
					$allSearchResults[$entityIdMap[$entityIdString]],
					$searchResult
				);
			}
		}
	...
	/**
	 * @param array $targetResult
	 * @param array $sourceResult
	 *
	 * @return array $targetResult with $sourceResult merged into it
	 */
	private function mergeSearchResultMatchedTerms( array $targetResult, array $sourceResult ) {
		$matchedTerms = array();
		if( isset( $targetResult[TermSearchInteractor::MATCHEDTERMS_KEY] ) ) {
			$matchedTerms = $targetResult[TermSearchInteractor::MATCHEDTERMS_KEY];
		}
		/** @var Term|Term[] $termData */
		foreach( $sourceResult[TermSearchInteractor::MATCHEDTERMS_KEY] as $termType => $termData ) {
			if( !array_key_exists( $termType, $matchedTerms ) ) {
				$matchedTerms[$termType] = $termData;
			} else {
				foreach( $termData as $newTerm ) {
					if( !in_array( $newTerm, $matchedTerms[$termType] ) ) {
						$matchedTerms[$termType][] = $newTerm;
					}
				}
			}
		}
		$targetResult[TermSearchInteractor::MATCHEDTERMS_KEY] = $matchedTerms;
		return $targetResult;
	}
