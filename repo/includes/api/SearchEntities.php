...
class SearchEntities extends ApiBase {
	...
	private function getRankedMatches( $text, $entityType, $languageCode, $limit, $strictLanguage ) {
		...
		if ( $entityId !== null ) {
			...
			$matchedEntityIdSerializations[] = $entityId->getSerialization();
		}
		...
		foreach( array( false, true ) as $prefixSearch ) {
			...
			foreach( $searchResults as $searchResult ) {
				...
				if( !in_array( $entityIdString, $matchedEntityIdSerializations ) ) {
					$allSearchResults[] = $searchResult;
					$matchedEntityIdSerializations[] = $entityIdString;
					$missing = $limit - count( $allSearchResults );
					if ( $missing <= 0 ) {
						continue;
					}
