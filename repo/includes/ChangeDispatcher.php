...
class ChangeDispatcher {
	...
	public function dispatchTo( $wikiState ) {
		...
		$this->coordinator->releaseClient( $continueAfter, $wikiState );
	...
	public function getPendingChanges( $siteID, $wikiDB, $after ) {
		...
		//FIXME: getHitRatio is not defined in ChunkAccess!
		$this->trace( "Got " . count( $batch ) . " pending changes. "
			. sprintf( "Cache hit rate is %2d%%", $this->chunkedChangesAccess->getHitRatio() * 100 ) );
	 ...
	 */
	private function filterChanges( $siteID, $wikiDB, $changes, $limit ) {
