...
class ChangeOpAliases extends ChangeOpBase {
	 ...
	 */
	public function __construct( $language, array $aliases, $action, TermChangeValidationHelper $validationHelper  ) {
	...
	protected function validateChange( Entity $entity ) {
		...
		if ( $this->action === 'set' || $this->action === 'add' ) {
			...
			foreach ( $this->aliases as $alias ) {
				$this->validationHelper->validateAlias( $this->language, $alias );
			}
		}
	}
}
