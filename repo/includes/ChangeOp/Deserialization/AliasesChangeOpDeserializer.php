	...
	private function getIndexedAliases( array $aliases ) {
		...
		foreach ( $aliases as $langCode => $serialization ) {
			if ( !is_string( $langCode ) ) {
				$indexedAliases[] = ( array_values( $serialization ) === $serialization ) ? $serialization : array( $serialization );
			} else {
				$indexedAliases[$langCode] = ( array_values( $serialization ) === $serialization ) ? $serialization : array( $serialization );
			}
		}
	...
	private function getIndexedAliasesChangeOps( array $indexedAliases ) {
		...
		foreach ( $indexedAliases as $langCode => $serializations ) {
			...
			foreach ( $serializations as $serialization ) {
				...
				$alias = array( $this->stringNormalizer->trimToNFC( $serialization['value'] ) );
