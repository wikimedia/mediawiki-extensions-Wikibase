...
class EntityDataRequestHandler {
	...
	public function handleRequest( $doc, WebRequest $request, OutputPage $output ) {
		...
		if ( $id === null || $id === '' ) {
			...
			throw new HttpError( 400, new Message( 'wikibase-entitydata-bad-id', $id ) );
		}
	...
	public function getCanonicalFormat( $format ) {
		...
		if ( $canonicalFormat === null ) {
			$msg = new Message( 'wikibase-entitydata-unsupported-format', $format );
	...
	public function httpContentNegotiation( WebRequest $request, OutputPage $output, EntityId $id, $revision = 0 ) {
		...
		if ( $format === null ) {
			...
			$msg = new Message( 'wikibase-entitydata-not-acceptable', $mimeTypes );
	...
	private function getEntityRevision( EntityId $id, $revision ) {
		...
		try {
			...
			if ( $entityRevision === null ) {
				...
				$msg = new Message( 'wikibase-entitydata-not-found', $prefixedId );
			...
			if ( is_string( $revision ) ) {
				...
				$msg = new Message( 'wikibase-entitydata-bad-revision', $prefixedId, $revision );
			...
			$msg = new Message( 'wikibase-entitydata-storage-error', $prefixedId, $revision );
