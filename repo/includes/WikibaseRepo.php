...
use Wikibase\Lib\EntitySource;
use Wikibase\Lib\EntitySourceDefinitions;
...
class WikibaseRepo {
	...
	private static function getEntitySourceDefinitionsFromSettings( SettingsArray $settings ) {
		...
		$source = new EntitySource( 'local', array_keys( $localEntityNamespaces ), $localDatabaseName, $localEntityNamespaces );

		return new EntitySourceDefinitions( [ $source ] );
	}
	...
	private function getXYZServices() {
		...
		return new MultipleEntitySourceServices( $this->entitySourceDefinitions, [] );
	}
