...
class RdfVocabulary {
	...
	/**
	 * @var string[] Associative array mapping repository names to names specific to the particular repository
	 * (ie. containing repository suffix).
	 */
	public $entityNamespaceNames = [];

	/**
	 * @var string[] Associative array mapping repository names to maps, each mapping the "general" property
	 ...
	 * @var MapCacheLRU
	 */
	private $entityLocalNameCache;

	/**
	 * @var MapCacheLRU
	 */
	private $entityRepositoryNameCache;
	 ...
	 * @param int $maxNameCacheKeys Maximum number of entries in entity repository and local
	 *            name caches (defaults to 1000).
	 */
	public function __construct(
		...
		$maxNameCacheKeys = 1000 // TODO: what would be a reasonable default?
	) {
		Assert::parameter(
		...
		$this->entityLocalNameCache = new MapCacheLRU( $maxNameCacheKeys );
		$this->entityRepositoryNameCache = new MapCacheLRU( $maxNameCacheKeys );
	}
	...
	public function getEntityLName( EntityId $entityId ) {
		...
		if ( !$this->entityLocalNameCache->has( $id ) ) {
			...
			$localName = str_replace( ':', '.', $localIdPart );
			$this->entityLocalNameCache->set( $id, $localName );
		}

		return $this->entityLocalNameCache->get( $id );
	}
	...
	public function getEntityRepositoryName( EntityId $entityId ) {
		...
		if ( !$this->entityRepositoryNameCache->has( $id ) ) {
			$this->entityRepositoryNameCache->set( $id, $entityId->getRepositoryName() );
		}

		return $this->entityRepositoryNameCache->get( $id );
	}
