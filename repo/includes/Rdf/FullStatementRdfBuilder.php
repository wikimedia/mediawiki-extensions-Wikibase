...
class FullStatementRdfBuilder implements EntityRdfBuilder {
	...
	private function addStatement( EntityId $entityId, Statement $statement, $isBest ) {
		...
		if ( $this->produceQualifiers ) {
			...
			foreach ( $statement->getQualifiers() as $q ) {
				$this->snakBuilder->addSnak( $this->statementWriter, $q, $this->vocabulary->getSuffixedNamespaceName( RdfVocabulary::NSP_QUALIFIER, $q->getPropertyId()) );
			}
		}
		...
		if ( $this->produceReferences ) {
			...
			foreach ( $statement->getReferences() as $reference ) { //FIXME: split body into separate method
				...
				foreach ( $reference->getSnaks() as $refSnak ) {
					$this->snakBuilder->addSnak( $this->referenceWriter, $refSnak, $this->vocabulary->getSuffixedNamespaceName( RdfVocabulary::NSP_REFERENCE, $refSnak->getPropertyId() ) );
				}
			}
		}
	}
	...
	private function addMainSnak( EntityId $entityId, $statementLName, Statement $statement, $isBest ) {
		...
		$this->statementWriter->about( $this->vocabulary->getSuffixedNamespaceName( RdfVocabulary::NS_ENTITY, $entityId ), $entityLName )
			->say( $this->vocabulary->getSuffixedNamespaceName( RdfVocabulary::NSP_CLAIM, $snak->getPropertyId() ), $propertyLName )
		...
		$this->snakBuilder->addSnak(
			...
			$snak,
			$this->vocabulary->getSuffixedNamespaceName( RdfVocabulary::NSP_CLAIM_STATEMENT, $snak->getPropertyId() )
		);
	}
