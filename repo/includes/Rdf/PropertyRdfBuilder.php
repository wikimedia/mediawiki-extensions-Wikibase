...
class PropertyRdfBuilder implements EntityRdfBuilder {
	...
	private function getPropertyRdfType( Property $property ) {
		...
		return in_array( $property->getDataTypeId(), $propertyTypesToBeObjects )
	...
	private function getNormalizedPropertyRdfType( Property $property ) {
		$propertyType = $property->getDataTypeId();
		$propertyTypesToBeNormalized = [
			'quantity',
			'external-id',
		];
		$nonObjectPropertyTypesToBeNormalizedAsObjects = [
			'external-id',
		];

		if ( in_array( $propertyType, $propertyTypesToBeNormalized ) ) {
			return $this->getPropertyRdfType( $property ) === self::OBJECT_PROPERTY
				|| in_array( $propertyType, $nonObjectPropertyTypesToBeNormalizedAsObjects )
					? self::OBJECT_PROPERTY
					: self::DATATYPE_PROPERTY;
		}
