	...
	public function addSiteLinks( Item $item ) {
		...
		foreach ( $item->getSiteLinkList() as $siteLink ) {
			...
			$this->writer->about( $url )
				...
				->say( RdfVocabulary::NS_SCHEMA_ORG, 'about' )->is( $this->vocabulary->getSuffixedNamespaceName( RdfVocabulary::NS_ENTITY, $item->getId() ), $entityLName )
			...
			foreach ( $siteLink->getBadges() as $badge ) {
				$this->writer
					->say( RdfVocabulary::NS_ONTOLOGY, 'badge' )
						->is( $this->vocabulary->getSuffixedNamespaceName( RdfVocabulary::NS_ENTITY, $badge ), $this->vocabulary->getEntityLName( $badge ) );
			}
