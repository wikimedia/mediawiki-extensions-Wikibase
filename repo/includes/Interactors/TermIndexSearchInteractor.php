...
class TermIndexSearchInteractor implements TermSearchInteractor {
	...
	public function searchForEntities( $text, $languageCode, $entityType, array $termTypes ) {
		$matchedTermIndexEntries = $this->getMatchingTerms( $text, $languageCode, $entityType, $termTypes );
	 ...
	 */
	private function getMatchingTerms( $text, $languageCode, $entityType, array $termTypes ) {
	...
	private function getSearchResults( array $termIndexEntries, array $aliasTermMap ) {
		...
		foreach ( $termIndexEntries as $termIndexEntry ) {
			...
			if( !array_key_exists( $entityIdString, $searchResults ) ) {
				...
				$searchResults[$entityIdString] = $this->mergeSearchResults(
	 ...
	 */
	private function mergeSearchResults( array $targetResult, array $sourceResult ) {
		$targetMatchedTerms = $targetResult[TermSearchInteractor::MATCHEDTERMS_KEY];
		foreach( $sourceResult[TermSearchInteractor::MATCHEDTERMS_KEY] as $termType => $terms ) {
			if( !array_key_exists( $termType, $targetMatchedTerms ) ) {
				$targetMatchedTerms[$termType] = $terms;
			} else {
				foreach( $terms as $term ) {
					if( !in_array( $term, $targetMatchedTerms[$termType] ) ) {
						$targetMatchedTerms[$termType][] = $term;
					}
				}
			}
		}
		$targetResult[TermSearchInteractor::MATCHEDTERMS_KEY] = $targetMatchedTerms;
		return $targetResult;
	}
	...
	private function convertToSearchResult( TermIndexEntry $termIndexEntry, array $aliasTermMap ) {
		...
		return array(
			...
			TermSearchInteractor::MATCHEDTERMS_KEY => array( $termIndexEntry->getType() => array( $termIndexEntry->getTerm() ) ),
