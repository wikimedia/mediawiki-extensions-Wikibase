...
class TermIndexSearchInteractor implements TermSearchInteractor {
	...
	private function mergeSearchResults( array $targetResult, array $sourceResult ) {
		...
		$matchedTermsArrayKeys = array_keys( $sourceResult[TermSearchInteractor::MATCHEDTERMS_KEY] );
		$newMatchedTermType = $matchedTermsArrayKeys[0];
		if( $newMatchedTermType == TermIndexEntry::TYPE_ALIAS ) {
			$newMatchedTerm = $sourceResult[TermSearchInteractor::MATCHEDTERMS_KEY][$newMatchedTermType][0];
		} else {
			$newMatchedTerm = $sourceResult[TermSearchInteractor::MATCHEDTERMS_KEY][$newMatchedTermType];
		}

		// Otherwise we must merge our matches into existing results
		if( !$this->matchedTermIsAlreadyInSearchResult( $newMatchedTerm, $newMatchedTermType, $targetResult ) ) {
			$targetResult = $this->addMatchedTermToSearchResult(
				$newMatchedTerm,
				$newMatchedTermType,
				$targetResult
			);
