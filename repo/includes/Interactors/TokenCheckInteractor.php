...
class TokenCheckInteractor {
	...
	/**
	 * Check the token send via the given web request
	 *
	 ...
	 * "missingtoken" if no token was send,
	 * "mustposttoken" if the token was not send via POST,
	...
	public function checkRequestToken( WebRequest $request, $tokenParam, $salt = null ) {
		...
		if ( !$request->wasPosted() ) {
			throw new TokenCheckException( 'Tokens must be send via a POST request', 'mustposttoken' );
		}
