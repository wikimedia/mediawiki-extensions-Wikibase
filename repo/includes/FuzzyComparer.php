...
final class FuzzyComparer {
	...
	public function compile() {
		...
		foreach ( $strings as $string ) {
			$signatures[$string] = self::makeSignature( $string );
		}
	 ...
	 */
	public static function makeSignature( $string ) {
	 ...
	 */
	public function getOverallScore( $testString ) {
		...
		foreach ( $this->strings as $string => $signature ) {
			if ( $string !== $testString ) {
				$score = $this->getSingleScore( $testString, $string );
				if ( $score > $highScore ) {
					$highScore = $score;
				}
			}
	 ...
	 * @since 0.3
	 *
	 * @param string $testString a string to compare against one of the other strings
	 * @param string $cmpString a string from the set of other strings
	 *
	 * @return integer the score
	 */
	public function getSingleScore( $testString, $cmpString ) {
		$diff = 0;
		for ( $i = 0; $i <= 0x0ff; $i++) {
