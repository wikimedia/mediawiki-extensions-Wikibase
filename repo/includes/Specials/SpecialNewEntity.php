...
abstract class SpecialNewEntity extends SpecialWikibaseRepoPage {
	...
	protected function prepareArguments() {
		$this->label = $this->stringNormalizer->trimWhitespace( $this->getRequest()->getVal(
			'label',
			isset( $this->parts[0] ) ? $this->parts[0] : ''
		) );
		$this->description = $this->stringNormalizer->trimWhitespace( $this->getRequest()->getVal(
			'description',
			isset( $this->parts[1] ) ? $this->parts[1] : ''
		) );
		$aliases = $this->stringNormalizer->trimWhitespace( $this->getRequest()->getVal( 'aliases' ) );
