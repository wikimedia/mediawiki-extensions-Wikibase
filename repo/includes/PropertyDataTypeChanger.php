...
class PropertyDataTypeChanger {
	 ...
	 */
	public function __construct( EntityRevisionLookup $entityRevisionLookup, EntityStore $entityStore, DataTypeFactory $dataTypeFactory ) {
	...
	private function assertDataTypesCompatible( $oldTypeId, $newTypeId ) {
		...
		if ( $oldType->getDataValueType() !== $newType->getDataValueType() ) {
			throw new InvalidArgumentException( "New and old DataType need to have the same data value type." );
		}
	}
