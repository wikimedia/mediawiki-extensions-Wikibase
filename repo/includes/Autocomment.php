...
final class Autocomment {
	...
	public static function formatAutoSummary( array $parts, $lang = false ) {
		...
		elseif ( $lang instanceof \Language) {
			...
			$composite = $wgContLang->commaList( $parts );
			return array( count( $parts ), $composite, $wgContLang );
		}
	}
	...
	public static function formatTotalSummary( $comment, $summary, $lang = false, $length = SUMMARY_MAX_LENGTH ) {
		...
		$comment = \Sanitizer::escapeHtmlAllowEntities( Utils::squashToNFC( $comment ) );
		$summary = \Sanitizer::escapeHtmlAllowEntities( Utils::squashToNFC( $summary ) );
		$mergedString = '';
		if ( $comment !== '' ) {
