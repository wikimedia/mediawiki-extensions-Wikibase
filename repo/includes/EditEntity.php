...
class EditEntity {
	...
	public function attemptSave( EntityDocument $newEntity, $summary, $flags, $token, $watch = null ) {
		...
		//      if any new revisions were created between then and now (a "late" edit conflict).
		...
		$this->applyPreSaveChecks( $newEntity ); // modifies $this->status
	...
	private function applyPreSaveChecks( EntityDocument $newEntity ) {
		if ( $this->hasEditConflict() ) {
			if ( !$this->fixEditConflict( $newEntity ) ) {
