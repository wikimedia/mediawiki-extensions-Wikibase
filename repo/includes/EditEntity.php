...
class EditEntity {
	...
	/**
	 * Returns the current revision of the entity.
	 *
	...
	public function fixEditConflict() {
		...
		//XXX: can $current or $base be null?!
	...
	public function attemptSave( $summary, $flags, $token, $watch = null ) {
		...
		try {
			...
			$editStatus = Status::newGood( $entityRevision );
	...
	protected function applyPreSaveChecks() {
		...
		if ( !$this->preSaveChecks ) {
			...
			$this->preSaveChecks = new PreSaveChecks();
		}

		$this->preSaveChecks->applyPreSaveChecks( $entity, $entityDiff );
		return $this->status;
	}
