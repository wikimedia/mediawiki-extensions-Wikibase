...
abstract class EntityView extends \ContextSource {
	...
	public function getHtml( EntityContent $entity, Language $lang = null, $editable = true ) {
		...
		$toc = array();
		$this->addSectionToTOC( $entity, $lang, $toc );
	...
	/**
	 * Adds sections to the table of contents.
	 *
	 * @since 0.4
	 *
	 * @param EntityContent $entity the entity to render
	 ...
	 */
	abstract public function addSectionToTOC( EntityContent $entity, Language $lang = null, array &$toc );
	...
	public function getHtmlForTOC( EntityContent $entity, Language $lang = null, array &$toc ) {
		...
		$count = 1;
		$entries = array_map(
			function( $section ) use ( &$count, $prefixedId ) {
				return $section === false
					...
					: wfTemplate( 'wb-entity-toc-entry',
						...
						$section,
						wfMessage( 'wikibase-toc-section-' . $section )->escaped()
					);
			},
			$toc
		);
	...
	public function getInnerHtml( EntityContent $entity, Language $lang = null, $editable = true, array &$toc ) {
		...
		if ( class_exists( 'Babel' ) && !$this->getUser()->isAnon() ) {
			$termsHtml = $this->getHtmlForLanguageTerms( $entity, $lang, $editable );
			if ( $termsHtml !== '' ) {
				array_unshift( $toc, 'other' );
			}
		}

		if ( count( $toc ) > 1 ) {
			$html .= $this->getHtmlForTOC( $entity, $lang, $toc );
		}
		$html .= $termsHtml;
