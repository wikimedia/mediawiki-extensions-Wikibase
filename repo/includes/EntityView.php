...
abstract class EntityView extends ContextSource {
	...
	public function getHtmlForClaims( Entity $entity, $editable = true ) {
		...
		foreach( $claimsByProperty as $claims ) {
			...
			$propertyLabel = ( isset( $propertyInfo[$key] )
				&& !empty( $propertyInfo[$key]['labels'] ) )
				? reset( $propertyInfo[$key]['labels'] )
				: $key;
	...
	protected function getPropertyInfo( Entity $entity, $languageCode ) {
		...
		$this->entityInfoBuilder->addTerms( $entities, array( 'label', 'description' ), array( $languageCode ) );
