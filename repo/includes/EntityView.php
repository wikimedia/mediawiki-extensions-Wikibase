...
abstract class EntityView extends \ContextSource {
	...
	public function getHtmlForClaims( EntityContent $entity, Language $lang = null, $editable = true ) {
		...
		foreach( $claimsByProperty as $claims ) {
			...
			$claimHtmlGenerator = new ClaimHtmlGenerator( new ValueFormatterFactory( $GLOBALS['wgValueFormatters'] ) );

			$i = 0;
			foreach( $claims as $claim ) {
				$propertyHtml .= $claimHtmlGenerator->getHtmlForClaim( $entity, $claim, $lang, $editable, $htmlForEditSection );
			}
