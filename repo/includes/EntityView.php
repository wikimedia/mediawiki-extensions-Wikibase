...
abstract class EntityView extends \ContextSource {
	...
	public function __construct( ValueFormatterFactory $valueFormatters, IContextSource $context = null ) {
		...
		$this->languageFallbackChain = $this->languageFallbackChainFactory->newFromContext( $this->getContext() );
	}
	...
	public function registerJsConfigVars( OutputPage $out, EntityContent $entityContent, $langCode, $editableView = false  ) {
		...
		$out->addJsConfigVars( 'wbContextLanguage',
			$this->languageFallbackChainSerializer->serialize( $this->languageFallbackChain )
		);
		...
		// requires a huge amount of memory due to Language object initialization without those performance patches.
	 ...
	 * @param null|LanguageFallbackChain $languageFallbackChain
	 *                   Set it to include labels to display for the given language fallback chain too.
	 * @return array
	 */
	protected static function getBasicEntityInfo(
		EntityLookup $entityLoader, array $entityIds, $langCode, $languageFallbackChain = null
	) {
		wfProfileIn( __METHOD__ );
