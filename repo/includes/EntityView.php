...
abstract class EntityView extends \ContextSource {
	 ...
	 * @since 0.1
	 *
	 * @param EntityRevision $entity the entity to render
	 * @param \Language|null $lang the language to use for rendering. if not given, the local context will be used.
	 * @param bool $editable whether editing is allowed (enabled edit links)
	 * @return string
	 */
	public function getHtml( EntityRevision $entity, Language $lang = null, $editable = true ) {
	...
	public function getInnerHtml( EntityRevision $entity, Language $lang, $editable = true ) {
		...
		if ( $entity->getEntity()->getType() === 'item' ) {
			$claims = $this->getHtmlForClaims( $entity->getEntity(), $lang, $editable );
		}

		$languageTerms = $this->getHtmlForLanguageTerms( $entity->getEntity(), $lang, $editable );

		$html = wfTemplate( 'wb-entity-content',
			$this->getHtmlForLabel( $entity->getEntity(), $lang, $editable ),
			$this->getHtmlForDescription( $entity->getEntity(), $lang, $editable ),
			$this->getHtmlForAliases( $entity->getEntity(), $lang, $editable ),
			$languageTerms,
			$claims
		);
	 ...
	 * @since 0.1
	 *
	 * @param EntityRevision      $entity the entity to analyze/render
	 ...
	 * @return ParserOutput
	 */
	public function getParserOutput( EntityRevision $entity, ParserOptions $options = null, $generateHtml = true ) {
		wfProfileIn( __METHOD__ );
	 ...
	 * @since 0.1
	 *
	 * @param EntityRevision       $entity the entity to output
	 ...
	 */
	public function render( EntityRevision $entity, OutputPage $out = null, ParserOptions $options = null, ParserOutput $pout = null ) {
		wfProfileIn( __METHOD__ );
