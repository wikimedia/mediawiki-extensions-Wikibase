...
class FingerprintView {
	...
	public function getHtml( Fingerprint $fingerprint, EntityId $entityId = null, $editable = true ) {
		...
		$html .= $this->getHtmlForLabel( $fingerprint->getLabels(), $entityId, $editable );
		$html .= $this->getHtmlForDescription( $fingerprint->getDescriptions(), $entityId, $editable );
		$html .= wfTemplate( 'wb-entity-header-separator' );
		$html .= $this->getHtmlForAliases( $fingerprint->getAliasGroups(), $entityId, $editable );
	...
	private function getAliasListHTML( array $aliases ) {
		...
		foreach ( $aliases as $alias ) {
			$html .= wfTemplate( 'wb-alias', htmlspecialchars( $alias ) );
		}
		return wfTemplate( 'wb-aliases', $html );
	}
