...
/**
 * Detector for description constraint violations.
 *
 ...
 */
class DescriptionConstraintDetector extends MultiLangConstraintDetector {
	...
	public function checkConstraints( Entity $entity, Status $status, Diff $diff = null, array $limits = null ) {
		...
		if ( wfRunHooks( 'WikibaseCheckConstraintsForDescription',
			array( &$foundSets, $entity->getDescriptions(), $limits, $status ) ) ) {

			// default constraints in addition to the ones checked inside the hook
			$foundSets[] = static::findLengthConstraintViolations( $entity->getDescriptions(), $limits['length'], $status );
		}

		$failedLangs = $this->inDiff( $foundSets, $diff === null ? null : $diff->getDescriptionsDiff() );
