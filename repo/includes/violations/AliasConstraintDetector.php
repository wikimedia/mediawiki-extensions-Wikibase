...
/**
 * Detector for alias constraint violations.
 *
 ...
 */
class AliasConstraintDetector extends MultiLangConstraintDetector {
	...
	public function checkConstraints( Entity $entity, Status $status, Diff $diff = null, array $limits = null ) {
		...
		if ( wfRunHooks( 'WikibaseCheckConstraintsForAliases',
			array( &$foundSets, $entity->getAllAliases(), $limits, $status ) ) ) {

			// default constraints in addition to the ones checked inside the hook
			$foundSets[] = static::findLengthConstraintViolations( $entity->getAllAliases(), $limits['length'], $status );
		}

		$failedLangs = $this->inDiff( $foundSets, $diff === null ? null : $diff->getAliasesDiff() );
