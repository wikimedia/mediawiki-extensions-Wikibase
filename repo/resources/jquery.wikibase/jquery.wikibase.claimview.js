		...
		'listviewcreate snaklistviewstartediting': function( event, toolbarController ) {
			...
			if(
				...
				.on( 'addtoolbaradd.addtoolbar', function( e ) {
					listview.enterNewItem().done( function( $snaklistview ) {
						var snaklistview = listview.listItemAdapter().liInstance( $snaklistview );
						snaklistview.enterNewItem().done( function( $snakview ) {
							var snaklistviewListview = snaklistview.$listview.data( 'listview' ),
								lia = snaklistviewListview.listItemAdapter();
							lia.liInstance( $snakview ).focus();
						} );
					} );
				} );
								...
								if( movetoolbar ) {
									var btnUp = movetoolbar.getButton( 'up' ),
										...
										nextListItemIsPending = hasNextListItem && (
											nextSnaklist === null
											|| claimview._initialQualifiers.indexOf( nextSnaklist.toArray()[0] ) === -1
										),
