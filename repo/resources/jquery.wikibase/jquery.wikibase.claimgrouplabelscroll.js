...
( function( $ ) {
	...
	$.widget( 'wikibase.' + WIDGET_NAME, {
		...
		update: function() {
			...
			var $visibleClaims =
					findFirstVisibleMainSnakElementsWithinClaimList( this.element ).closest( '.wb-claim' );

			for( var i = 0; i < $visibleClaims.length; i++ ) {
				var $visibleClaim = $visibleClaims.eq( i ),
					$claimGroup = $visibleClaim.closest( '.wb-claimlistview' ),
	...
	function findFirstVisibleMainSnakElement( $searchRange ) {
		...
		var $mainSnaks = $searchRange.find( '.wb-claim-mainsnak .wb-snak-value-container' );
