...
wb.ui.Base.prototype = {
	...
	init: function( subject ) {
		...
		this._subject = arguments[0] = $( subject ); // make sure we have a jQuery object, not a plain DOM node
		var result = this._init.apply( this, arguments ); // should be overwritten rather than init()

		// add UI class(es) but after _init() in case subject changes
		this._subject.addClass( this.UI_CLASS + ' ' + this.SECONDARY_UI_CLASSES );
		return result;
	},
