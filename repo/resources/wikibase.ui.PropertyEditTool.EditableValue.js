...
window.wikibase.ui.PropertyEditTool.EditableValue.prototype = {
	...
	remove: function() {
		...
		if( this.isPending() ) {
			...
			var promise = this.__isRemoving_deferred.promise();
			promise.apiAction = this.API_ACTION.REMOVE;
	...
	save: function() {
		...
		if( ! this.isValid() ) { // remove instead! Save equals remove in this case!
			...
			promise.apiAction = this.API_ACTION.SAVE_TO_REMOVE;
		} else {
			...
			promise.apiAction = this.API_ACTION.SAVE;
		}
		return promise;
	},
	...
	stopEditing: function( save ) {
		...
		promise.apiAction = this.API_ACTION.NONE;

		if( !this.isInEditMode() ) {
			return promise;
		} else if ( this.__isStopEditing_deferred &&
			!( this.__isStopEditing_deferred.isResolved() || this.__isStopEditing_deferred.isRejected() ) ) {
			promise = this.__isStopEditing_deferred.promise();
			promise.isOngoingStopEditing = true;
			return promise;
		}
		...
		// store deferred so we can return it when this is called again while still running
		this.__isStopEditing_deferred = promise
		.done(
			$.proxy( function() {
