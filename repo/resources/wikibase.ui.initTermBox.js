...
( function( $, mw, wb ) {
	...
	mw.hook( 'wikibase.domready' ).add( function() {
		var termsValueTools = [],
			ulsIsDefined = mw.uls !== undefined && $.uls.data !== undefined;

		if( $( 'tr.wb-terms-label, tr.wb-terms-description' ).length === 0 && ulsIsDefined ) {
	...
	function renderTermBox( title, entity, languageCodes ) {
		...
		for( var i = 0; i < languageCodes.length; i++ ) {
			...
			$tbody.append( mw.template( 'wb-term',
				...
				( labels[languageCode] !== undefined ) ? labels[languageCode] : '',
				( descriptions[languageCode] !== undefined ) ? descriptions[languageCode] : '',
				'',
				'',
				'',
				'',
