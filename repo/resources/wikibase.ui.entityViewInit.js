		...
		$( wb ).on( 'startItemPageEditMode', function( event, origin, options ) {
			...
			if( mw.config.exists( 'wbCopyrightWarning' ) ) {
				...
				var tooltip = new wb.ui.Tooltip(
					toolbar.$btnSave.data( 'wbbutton' ).getTooltipParent(), // adjust tooltip to save button
					{},
					$message,
				...
				var messageAnchor = $( '<span/>' ).wblabel();
				messageAnchor.data( 'wblabel' ).setTooltip( tooltip );

				$hideMessage.on( 'click', function( event ) {
					event.preventDefault();
					messageAnchor.data( 'wblabel' ).removeTooltip();
