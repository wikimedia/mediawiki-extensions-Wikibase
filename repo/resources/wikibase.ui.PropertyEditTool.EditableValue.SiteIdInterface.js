...
/**
 * Serves the input interface to write a site code or to selectone. This will also validate whether the site code is
...
wb.ui.PropertyEditTool.EditableValue.SiteIdInterface = wb.utilities.inherit( $PARENT, {
	...
	init: function( subject ) {
		...
		this._initSiteList();
		this._subject = null; // necessary so original init() won't destroy this

		// NOTE: this isn't too nice since we don't call the parent init() which would override the list we create
		//       with _initSiteList() which we also can't call afterwards since the original init() already requires
		//       it when normalizing the initial value.
		wb.ui.PropertyEditTool.EditableValue.Interface.prototype.init.call( this, subject );
	},
