<?php

declare( strict_types = 1 );

namespace Wikibase\Repo\Tests\Unit\ServiceWiring;

use Wikibase\Lib\LanguageNameLookupFactory;
use Wikibase\Lib\Store\EntityTitleLookup;
use Wikibase\Repo\ItemDisambiguationFactory;
use Wikibase\Repo\Tests\Unit\ServiceWiringTestCase;

/**
 * @coversNothing
 *
 * @group Wikibase
 *
 * @license GPL-2.0-or-later
 */
class ItemDisambiguationFactoryTest extends ServiceWiringTestCase {

	public function testConstruction(): void {
		$this->mockService( 'WikibaseRepo.EntityTitleLookup',
			$this->createStub( EntityTitleLookup::class ) );
		$this->mockService( 'WikibaseRepo.LanguageNameLookupFactory',
			$this->createStub( LanguageNameLookupFactory::class ) );

		$this->assertInstanceOf(
			ItemDisambiguationFactory::class,
			$this->getService( 'WikibaseRepo.ItemDisambiguationFactory' )
		);
	}

}
