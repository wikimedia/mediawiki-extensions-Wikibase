...
class RdfBuilderTest extends \MediaWikiTestCase {
	...
	public static function getTestGraphs() {
		...
		$graphs['terms'] = self::makeEntityGraph(
			...
			array(
				'rdf:type' => $builder->getEntityTypeQName( Item::ENTITY_TYPE ),
				'foaf:primaryTopicOf' => array(
					$builder->getEntityQName( RdfBuilder::NS_DATA, $entities['terms']->getId() ),
					"//en.wikipedia.org/wiki/Berlin", //FIXME: resolve protocol relative
					"//ru.wikipedia.org/wiki/%D0%91%D0%B5%D1%80%D0%BB%D0%B8%D0%BD", //FIXME: resolve protocol relative
				),
				'rdfs:label' => array(
