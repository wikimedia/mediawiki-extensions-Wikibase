...
class MediaWikiConfigModuleTest extends PHPUnit_Framework_TestCase {
	...
	public function testGetScript_returnsJavaScript() {
		...
		$expected = 'mediaWiki.config.set( "configValueKey", "configValue" );';
		$this->assertSame( $expected, $this->newInstance()->getScript( $context ) );
	}
	...
	private function newInstance() {
		return new MediaWikiConfigModule( [ 'getconfigvalueprovider' => function () {
			$provider = $this->getMockBuilder( MediaWikiConfigValueProvider::class )
				->disableOriginalConstructor()
				->getMock();

			$provider->expects( $this->any() )
				->method( 'getKey' )
				->will( $this->returnValue( 'configValueKey' ) );
