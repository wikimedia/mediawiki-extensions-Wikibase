...
class SearchEntitiesIntegrationTest extends PHPUnit_Framework_TestCase {
	...
	public function testElasticSearchIntegration( $query, array $expectedIds ) {
		...
		$entitySearchElastic = new MockEntitySearchElastic(
...
/**
 * Mock class that unpacks query that search class returns
 * and extracts ID clause from it.
 */
class MockEntitySearchElastic extends EntitySearchElastic {
	public function getRankedSearchResults( $text, $languageCode, $entityType, $limit, $strictLanguage ) {
		$result = parent::getRankedSearchResults( $text, $languageCode, $entityType, $limit,
				$strictLanguage );
