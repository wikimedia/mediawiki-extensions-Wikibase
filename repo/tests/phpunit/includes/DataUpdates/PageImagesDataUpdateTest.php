...
class PageImagesDataUpdateTest extends PHPUnit_Framework_TestCase {
	 ...
	 */
	private function getDataUpdate( array $propertyIds ) {
	...
	public function testConstructor( $propertyIds ) {
		$dataUpdate = $this->getDataUpdate( $propertyIds );
		$this->assertInstanceOf( 'Wikibase\Repo\DataUpdates\PageImagesDataUpdate', $dataUpdate );
	}
	...
	public function testGetBestImageFileName(
		...
		$dataUpdate = $this->getDataUpdate( $propertyIds );
		$this->assertSame( $expected, $dataUpdate->getBestImageFileName( $statements ) );
	}
