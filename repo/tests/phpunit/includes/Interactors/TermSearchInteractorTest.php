...
class TermSearchInteractorTest extends PHPUnit_Framework_TestCase {
	...
	private function getMockTermIndex() {
		return new MockTermIndex(
			array(
				//Q111
				$this->getWbTerm( 'Foo', 'en', WbTerm::TYPE_LABEL, new ItemId( 'Q111' ) ),
				$this->getWbTerm( 'Foo german decription', 'de', WbTerm::TYPE_DESCRIPTION, new ItemId( 'Q111' ) ),
				$this->getWbTerm( 'Foooooo', 'en', WbTerm::TYPE_ALIAS, new ItemId( 'Q111' ) ),
				//Q222
				$this->getWbTerm( 'Bar', 'en', WbTerm::TYPE_LABEL, new ItemId( 'Q222' ) ),
				$this->getWbTerm( 'BarGerman', 'de', WbTerm::TYPE_LABEL, new ItemId( 'Q222' ) ),
				$this->getWbTerm( 'BarGerman description', 'de', WbTerm::TYPE_DESCRIPTION, new ItemId( 'Q222' ) ),
				$this->getWbTerm( 'Barrrrr', 'en', WbTerm::TYPE_ALIAS, new ItemId( 'Q222' ) ),
				//Q333
				$this->getWbTerm( 'Food', 'en', WbTerm::TYPE_LABEL, new ItemId( 'Q333' ) ),
				$this->getWbTerm( 'Bar snacks', 'en', WbTerm::TYPE_ALIAS, new ItemId( 'Q333' ) ),
				//P11
				$this->getWbTerm( 'Foo', 'en', WbTerm::TYPE_LABEL, new PropertyId( 'P11' ) ),
				//P22
				$this->getWbTerm( 'Lama', 'en', WbTerm::TYPE_LABEL, new PropertyId( 'P22' ) ),
				$this->getWbTerm( 'Laama', 'en', WbTerm::TYPE_ALIAS, new PropertyId( 'P22' ) ),
				$this->getWbTerm( 'Lamama', 'en', WbTerm::TYPE_ALIAS, new PropertyId( 'P22' ) ),
			)
		);
	}
	 ...
	 * @returns WbTerm
	 */
	private function getWbTerm( $text, $languageCode, $termType, EntityId $entityId ) {
		return new WbTerm( array(
			'termText' => $text,
	...
	private function getMockLabelDescriptionLookup() {
		...
		$mock->expects( $this->any() )
			...
			->will( $this->returnCallback( function( EntityId $entityId ) use ( $testCase ) {
				return $testCase->getDisplayTerm( $entityId, WbTerm::TYPE_LABEL );
			}
			) );
		$mock->expects( $this->any() )
			->method( 'getDescription' )
			->will( $this->returnCallback( function( EntityId $entityId ) use ( $testCase ) {
				return $testCase->getDisplayTerm( $entityId, WbTerm::TYPE_DESCRIPTION );
			}
			) );
		return $mock;
	}
	...
	public function provideSearchForTermsTest() {
		...
		return array(
			...
			'Foo, Item, EN, Label Only' => array(
				$this->getTermSearchInteractor( false, false, 5000 ),
				array( 'Foo',  array( 'en' ), 'item', array( WbTerm::TYPE_LABEL ) ),
				array(
			...
			'Foo, Item, EN, Label & Alias, Prefix' => array(
				...
				array( 'Foo',  array( 'en' ), 'item', array( WbTerm::TYPE_LABEL, WbTerm::TYPE_ALIAS ) ),
				array(
					array( 'entityId' => new ItemId( 'Q111' ), 'term' => new Term( 'en', 'Foo' ), ),
			...
			'La, Property, EN, Aliases, Prefix' => array(
				$this->getTermSearchInteractor( false, true, 5000 ),
				array( 'La',  array( 'en' ), 'property', array( WbTerm::TYPE_ALIAS ) ),
				array(
	...
	public function testSearchForTerms_returnsExpectedResults( $interactor, $params, $expectedTermsDetails ) {
		...
		foreach ( $results as $key => $result ) {
			...
			$expectedDisplayTerms = array(
				WbTerm::TYPE_LABEL => $this->getDisplayTerm( $expectedEntityId, WbTerm::TYPE_LABEL ),
				WbTerm::TYPE_DESCRIPTION => $this->getDisplayTerm( $expectedEntityId, WbTerm::TYPE_DESCRIPTION ),
			);
