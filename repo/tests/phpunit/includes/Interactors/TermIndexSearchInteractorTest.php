...
class TermIndexSearchInteractorTest extends PHPUnit_Framework_TestCase {
	...
	public function provideSearchForEntitiesTest() {
		...
		return array(
			...
			'Q111&Q333 Foo en Label match prefix search' => array(
				...
				array(
					array( 'entityId' => new ItemId( 'Q111' ), 'terms' => array(
						'label' => array( new Term( 'en', 'Foo' ) ),
					) ),
					array( 'entityId' => new ItemId( 'Q333' ), 'terms' => array(
						'label' => array( new Term( 'en', 'Food is great' ) ),
					) ),
				)
			),
			...
			'Q111 Foo en all term types match case insensitive' => array(
				...
				array(
					array( 'entityId' => new ItemId( 'Q111' ), 'terms' => array(
						'label' => array( new Term( 'en', 'Foo' ) ),
						'description' => array( new Term( 'en', 'Foo' ) ),
			...
			'P22&P44 La en-ca with fallback all terms' => array(
				...
				array(
					array( 'entityId' => new PropertyId( 'P22' ), 'terms' => array(
						'label' => array( new Term( 'en-ca', 'Lama' ) ),
						'description' => array( new Term( 'en', 'La-description' ) ),
					) ),
					array( 'entityId' => new PropertyId( 'P44' ), 'terms' => array(
						'label' => array( new Term( 'en', 'Lama' ) ),
					) ),
				)
			),
		);
	...
	public function testSearchForEntities_returnsExpectedResults( $interactor, $params, $expectedTermsDetails ) {
		...
		foreach ( $results as $key => $result ) {
			...
			/** @var Term[][] $resultMatchedTerms */
			$resultMatchedTerms = $result['matchedTerms'];
			/** @var Term[][] $expectedTerms */
			$expectedTerms = $expectedTermDetails['terms'];
			foreach( $expectedTerms as $expectedKey => $expectedTermOrArray ) {
				$this->assertCount( count( $expectedTermOrArray ), $resultMatchedTerms[$expectedKey] );
				foreach( $expectedTermOrArray as $term ) {
					$this->assertTrue( in_array( $term, $resultMatchedTerms[$expectedKey] ) );
				}
			}
