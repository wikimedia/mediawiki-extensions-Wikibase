...
class TermIndexSearchInteractorTest extends PHPUnit_Framework_TestCase {
	...
	public function provideSearchForEntitiesTest() {
		...
		return array(
			...
			'Q111 Foo en Label match exactly' => array(
				...
				array(
					array( 'entityId' => new ItemId( 'Q111' ), 'terms' => array(
						'label' => new Term( 'en', 'Foo' ),
					) ),
				)
			),
			'Q111&Q333 Foo en Label match prefix search' => array(
				...
				array(
					...
					array( 'entityId' => new ItemId( 'Q333' ), 'terms' => array(
						'label' => new Term( 'en', 'Food is great' ),
					) ),
				)
			),
			...
			'Q111 Foo en all term types match case insensitive' => array(
				...
				array(
					array( 'entityId' => new ItemId( 'Q111' ), 'terms' => array(
						'label' => new Term( 'en', 'Foo' ),
						'description' => new Term( 'en', 'Foo' ),
						'alias' => array( new Term( 'en', 'Foo' ), new Term( 'en', 'FOO' ) ),
					) ),
				)
			),
			...
			'Q555 Ta en-ca with fallback aliases only' => array(
				...
				array(
					array( 'entityId' => new ItemId( 'Q555' ), 'terms' => array(
						'alias' => array( new Term( 'en-ca', 'TAAA' ), new Term( 'en-ca', 'Taa' ), new Term( 'en', 'Ta' ), new Term( 'en', 'Taa' ) ),
					) ),
				)
			),
			'P22&P44 La en-ca with fallback all terms' => array(
				...
				array(
					array( 'entityId' => new PropertyId( 'P22' ), 'terms' => array(
						'label' => new Term( 'en-ca', 'Lama' ),
						'description' => new Term( 'en', 'La-description' ),
					) ),
					array( 'entityId' => new PropertyId( 'P44' ), 'terms' => array(
						'label' => new Term( 'en', 'Lama' ),
					) ),
				)
			),
		);
	...
	public function testSearchForEntities_returnsExpectedResults( $interactor, $params, $expectedTermsDetails ) {
		...
		foreach ( $results as $key => $result ) {
			/** @var Term[]|array[] $resultTerms */
			$resultTerms = $result['matchedTerms'];
			...
			/** @var Term[]|array[] $expectedTerms */
			$expectedTerms = $expectedTermDetails['terms'];
			foreach( $expectedTerms as $expectedKey => $expectedTermOrArray ) {
				if( is_array( $expectedTermOrArray ) ) {
					$this->assertCount( count( $expectedTermOrArray ), $resultTerms[$expectedKey] );
					foreach( $expectedTermOrArray as $term ) {
						$this->assertTrue( in_array( $term, $resultTerms[$expectedKey] ) );
					}
				} else {
					$this->assertEquals( $expectedTermOrArray, $resultTerms[$expectedKey] );
				}
			}
