...
class SqlChangeDispatchCoordinatorTest extends \MediaWikiTestCase {
	...
	private function fetchChangesDispatchRows( $where = '' ) {
		$dbw = wfGetDB( DB_MASTER );
		$res = $dbw->select( 'wb_changes_dispatch', '*', $where, __METHOD__ );
	...
	public function testSelectClient( $chdRows, $expected ) {
		...
		if ( $expected !== null ) {
			...
			$expectedRow = array_merge( $expected, array( 'chd_disabled' => '0' ) );
			$this->assertEquals( $expectedRow, array_shift( $rows ) );
		}
	}
