...
class ActionTestCase extends MediaWikiTestCase {
	...
	protected static function makeTestItemData() {
		...
		$item = $item->copy();
		$item->setDescription( 'de', 'Hautstadt von Deutschland' );
		$items['Berlin'][] = $item;
	...
	public static function initTestItems() {
		...
		foreach ( $itemData as $handle => $revisions ) {
			...
			foreach ( $revisions as $item ) {
				...
				self::$testItems[$handle] = $content->getItem();
			}
		}
	}
	 ...
	 */
	public function getTestItem( $handle ) {
		self::initTestItems();
	 ...
	 * @throws \Exception if the handle is not known
	 */
	public function getTestItemPage( $handle ) {
		$item = $this->getTestItem( $handle );
		$content = ItemContent::newFromItem( $item );
