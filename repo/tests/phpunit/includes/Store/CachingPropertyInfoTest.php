...
class CachingPropertyInfoTest extends \MediaWikiTestCase {
	...
	public function testStoringChangedPropertyIsPersistedAndRetrievable() {
		...
		// But a new lookup with no class cache shows the new data
		// TODO fix this?
		/**
		 * This currently fails, as the cache->set that is done by setPropertyInfo is ignored then the lookup uses getWithSetCallback
		 * due to the process cache held within WANCache.
		 * Perhaps we should turn off the process cache for WANCache?
		 */
		$this->assertCount( 1, $lookup->getAllPropertyInfo() );
