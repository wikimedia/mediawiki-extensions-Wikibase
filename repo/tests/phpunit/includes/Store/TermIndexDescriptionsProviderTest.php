...
/**
 *
 * @covers Wikibase\Repo\Store\TermIndexDescriptionsProvider
 *
 * @group Wikibase
 * @group WikibaseRepo
 * @group WikibaseStore
 ...
 */
class TermIndexDescriptionsProviderTest extends PHPUnit_Framework_TestCase {
	...
	public function instanceProvider() {
		return [
			...
			[ new ItemId( 'Q333' ), new TermList( [] ) ],
			[ new ItemId( 'Q555' ), new TermList( [] ) ],
			[ new PropertyId( 'P22' ), new TermList( [ new Term( 'en', 'La-description' ) ] ) ],
			[ new PropertyId( 'P44' ), new TermList( [ new Term( 'de', 'Lama-de-desc' ) ] ) ],
	 ...
	 */
	public function testDescriptionsProvider( EntityId $entityId, TermList $expected ) {
		$labelsProvider = new TermIndexDescriptionsProvider( $this->getMockTermIndex(), $entityId );
		$this->assertTrue( $expected->equals( $labelsProvider->getDescriptions() ) );
	}
