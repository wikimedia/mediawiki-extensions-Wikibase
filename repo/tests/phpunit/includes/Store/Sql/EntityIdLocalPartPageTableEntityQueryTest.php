...
class EntityIdLocalPartPageTableEntityQueryTest extends TestCase {
	...
	public function testSelectRowsSimple_noSlottedEntities() {
		...
		$database->makeList( [ 'page_title' => 'Q1', 'page_namespace' => 0 ], LIST_AND )->will(
				function () { return 'Q1-condition'; }
			);
		$database->makeList( [ "Q1-condition" ], LIST_OR )->will(
				function () { return 'combined-condition'; }
			);
		...
		$database->select(
			[ "extraTable" ],
			[ "extraField", "page_title", "page_title" ],
			"combined-condition",
			EntityIdLocalPartPageTableEntityQuery::class . "::selectRows",
			[],
			[ 0 => "extraJoin" ]
		)->willReturn(
	...
	public function testSelectRowsCombination() {
		...
		$database->makeList(
				...
				LIST_AND
			)->will( function () { return 'ooo123-condition'; } );
		...
		$database->select(
			...
			EntityIdLocalPartPageTableEntityQuery::class . "::selectRows",
			[],
			[ 0 => "extraJoin", "slots" => [ "INNER JOIN", "page_latest=slot_revision_id" ] ]
		)->willReturn(
			// A Traversable object
				new ArrayObject(
					[
