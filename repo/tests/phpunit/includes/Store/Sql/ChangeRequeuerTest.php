...
class ChangeRequeuerTest extends MediaWikiTestCase {
	...
	public function testGetChangeRowIdsInRange( $from, $to, $expected ) {
		...
		$db->expects( $this->exactly( 2 ) )
			->method( 'timestamp' )
			->willReturnCallback( function ( $value ) { return "T{$value}T"; } );
		$db->expects( $this->exactly( 2 ) )
			->method( 'addQuotes' )
			->willReturnCallback( function ( $value ) { return "Q{$value}Q"; } );
	...
	public function testBatchRequeueRows() {
		...
		$requeuer = new ChangeRequeuer( $lb );
		$requeuer->batchRequeueRows( $ids, $batchSize );
	}
