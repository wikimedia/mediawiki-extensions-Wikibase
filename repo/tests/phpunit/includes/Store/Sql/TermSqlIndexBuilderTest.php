...
class TermSqlIndexBuilderTest extends \MediaWikiTestCase {
	...
	public function testGivenEntityTypeRebuildOnlyRebuildsTermsOfEntitiesOfGivenType() {
		$item = $this->getItems()[0];
		$property = $this->getProperty();

		$this->saveEntities( [ $item, $property ] );

		$this->assertTermIndexRowsHaveFullEntityId( $item->getId()->getSerialization(), 5 );
		$this->assertTermIndexRowsHaveFullEntityId( $property->getId()->getSerialization(), 1 );

		$this->clearFullEntityIdColumn();

		$this->assertTermIndexRowsHaveFullEntityId( $item->getId()->getSerialization(), 0 );
		$this->assertTermIndexRowsHaveFullEntityId( $property->getId()->getSerialization(), 0 );

		$termFullEntityIdBuilder = $this->getBuilder();
		$termFullEntityIdBuilder->setEntityType( $item->getType() );
	...
	private function getProperty() {
		$property = new Property( new PropertyId( 'P222' ), null, 'string' );
