...
use Wikibase\Lib\Store\Sql\EntityIdLocalPartPageTableEntityQueryHelper;
...
class WikiPageEntityMetaDataLookupTest extends MediaWikiTestCase {
	...
	private function getWikiPageEntityMetaDataLookup( $nsLookup = null, $slotRoleStore = null ) {
		...
		return new WikiPageEntityMetaDataLookup(
			$nsLookup,
			new EntityIdLocalPartPageTableEntityQueryHelper(
				$nsLookup,
				$slotRoleStore
			)
		);
	...
	public function testGetWhereHasConditionsPerEntityId() {
		...
		$roleStore = $this->prophesize( NameTableStore::class );
		$roleStore->getId( 'second' )->willReturn( 2 );
		$roleStore = $roleStore->reveal();

		$metaDataLookup = $this->getWikiPageEntityMetaDataLookup( $namespaceLookup, $roleStore );
