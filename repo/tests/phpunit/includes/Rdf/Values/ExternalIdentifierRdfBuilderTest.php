...
class ExternalIdentifierRdfBuilderTest extends \PHPUnit_Framework_TestCase {
	...
	public function testAddValue() {
		...
		$vocab = WikibaseRepo::getDefaultInstance()->getRdfVocabulary();

		$builder = new ExternalIdentifierRdfBuilder( $uriPatternProvider, $vocab );

		$writer = new NTriplesRdfWriter();
		$writer->prefix( 'www', "http://www/" );
		$writer->prefix( 'wdt', "http://acme/prop/" );
		$writer->prefix( 'wdtrc', "http://acme/prop-normalized/" );
		...
		$expected = [
			'<http://www/Q1> <http://acme/prop-normalized/P1> <http://xyzzy.com/vocab/AB%26123> .',
			'<http://www/Q1> <http://acme/prop/P1> "AB&123" .',
