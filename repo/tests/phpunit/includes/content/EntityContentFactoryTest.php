...
class EntityContentFactoryTest extends \MediaWikiTestCase {
	...
	public function titleProvider() {
		...
		$argLists[] = array(
			Title::newFromText( "Nyan", NS_SPECIAL ),
			null
		);

		$argLists[] = array(
			Title::newFromText( "Poland", NS_MAIN ),
			null
		);

		$argLists[] = array(
			Title::newFromText( "Q1337", NS_USER ),
			null
		);
	 ...
	 */
	public function testGetIdForTitle( Title $title,  $expected ) {
		$factory = $this->newFactory();

		$actual = $factory->getIdForTitle( $title );
