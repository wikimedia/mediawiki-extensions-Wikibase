...
abstract class EntityContentTest extends \MediaWikiTestCase {
	...
	public function testSaveFlags() {
		...
		$status = $entityContent->save( 'create item' );
		$this->assertTrue( $status->isOK(), 'save failed', 'try to save without flags, save failed' );
	}
	...
	public function testRepeatedSave() {
		...
		$status = $entityContent->save( 'create item', null, EDIT_NEW );
		$this->assertTrue( $status->isOK(), "save failed", 'create, save failed, status ok' );
		$this->assertTrue( $status->isGood(), $status->getMessage(), 'create, status is good' );
		...
		$this->assertTrue( $status->isOK(), "save failed", 'change, status ok' );
		$this->assertTrue( $status->isGood(), $status->getMessage(), 'change, status good' );
		...
		$this->assertTrue( $status->isOK(), "save failed", 'change again, save failed, status ok' );
		$this->assertTrue( $status->isGood(), $status->getMessage(), 'change again, status good' );
		$this->assertNotEquals( $prev_id, $entityContent->getWikiPage()->getLatest(), "revision ID should change on edit" );

		// save unchanged
		$prev_id = $entityContent->getWikiPage()->getLatest();
		$status = $entityContent->save( 'save unmodified', null, EDIT_UPDATE );
		$this->assertTrue( $status->isOK(), "save failed", 'save unchanged, save failed, status ok' );
