...
class BotEditTest extends WikibaseApiTestCase {
	...
	private function assertResultValue( $expected, $key, $change ) {
		if ( array_key_exists( $key, $expected ) ){
			if ( $expected[$key] === true ) {
				$this->assertParamTrue( $key, $change );
			} else {
				$this->assertParamFalse( $key, $change );
			}
		}
	}

	private function assertParamTrue( $key, $change ) {
	...
	private function assertParamFalse( $key, $change ) {
