...
class ApiConventionsTest extends WikibaseApiTestCase {
	...
	*/
	...
	private function assertGetFinalPossibleErrors ( $moduleClass, $module ) {
		...
		$this->assertInternalType( 'array', $errArray, $moduleClass .' is not an array for the method ' .$method ); //method korrigieren
		$this->assertNotEmpty( $errArray, 'the Module ' .$moduleClass .' does not have the method ' .$method );
		foreach ( $errArray as $subArr ) {
			$this->assertInternalType( 'array', $subArr, $moduleClass .'is not an array for the method ' .$method );
			$this->assertNotEmpty( $subArr, 'the arry for the module ' .$moduleClass .' is empty' );

			foreach ( $subArr as $key => $value ) {
				//SearchEntities has only an empty array
				//AvailableBadges doesn't have the function implemented
				if ( $moduleClass != 'Wikibase\Api\SearchEntities' && $moduleClass != 'Wikibase\Api\AvailableBadges' ) {
					$bool = strcmp( $key, 'code' ) === 0 || strcmp( $key, 'info' ) === 0;
					$this->assertTrue( $bool, 'the module ' .$moduleClass .' has the key ' .$key .' instead of code or info' );
				}
			}
			//same propblem as above- missing implementation of possibleErrors
			if ( $moduleClass != 'Wikibase\Api\SearchEntities' && $moduleClass != 'Wikibase\Api\AvailableBadges' ) {
				$this->assertNotEquals( '&lt;wikibase-api-' .$subArr['code'] .'&gt;', $subArr['info'], 'error for ' .$moduleClass );
			}
		}
	}
