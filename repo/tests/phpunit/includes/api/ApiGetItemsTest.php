...
class ApiGetItemTest extends ApiModifyItemBase {
	...
	function testGetItemById( $handle ) {
		...
		$this->assertEquals( 1, count( $res['items'] ), "requestiong a single item should return exactly one item entry" );
	}
	...
	public function testGetItemsByBadTitle( ) {
		...
		$this->assertEquals( 1, count( $keys ), "requestiong a single item should return exactly one item entry" );
