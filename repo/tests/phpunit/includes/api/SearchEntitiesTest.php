...
class SearchEntitiesTest extends PHPUnit_Framework_TestCase {
	 ...
	 */
	private function getMockTitle( $exists ) {
	...
	private function getMockSearchInteractor( $params, $returnResults = array() ) {
		...
		$mock->expects( $this->atLeastOnce() )
			...
			->with(
				...
				$this->equalTo( array( TermIndexEntry::TYPE_LABEL, TermIndexEntry::TYPE_DESCRIPTION ) )
			)
	...
	private function getMockTermIndex() {
		return new MockTermIndex(
			array(
				// Only have a single alias
				$this->getTermIndexEntry( 'Foooooo', 'en-ca', TermIndexEntry::TYPE_ALIAS, new ItemId( 'Q111' ) ),
			)
		);
	}
	...
	public function provideTestSearchEntities() {
		$multipleInteractorReturnValues = array(
			array(
				...
				'matchedTerms' => array(
					TermIndexEntry::TYPE_LABEL => array(
						...
						new Term( 'en-gb', 'FoooooSecondMatch' )
					),
					TermIndexEntry::TYPE_ALIAS => array( new Term( 'en-gb', 'FoooooMatchedAlias' ) ),
				),
				'displayTerms' => array(
			...
			array(
				...
				'matchedTerms' => array(
					TermIndexEntry::TYPE_LABEL => array( new Term( 'de', 'AMatchedTerm' ) ),
				),
				'displayTerms' => array(
					TermIndexEntry::TYPE_LABEL => new Term( 'fr', 'ADisplayLabel' ),
					TermIndexEntry::TYPE_ALIAS => array(
						new Term( 'de', 'Alias1' ),
						new Term( 'de', 'Alias2' ),
					),
				),
		...
		$q222Result = array(
			...
			'matched' => array(
				TermIndexEntry::TYPE_LABEL => array( 'Fooooo', 'FoooooSecondMatch' ),
				TermIndexEntry::TYPE_ALIAS => array( 'FoooooMatchedAlias' ),
			),
		);
		$q333Result = array(
			'id' => 'Q333',
			...
			TermIndexEntry::TYPE_ALIAS => array( 'Alias1', 'Alias2' ),
			'matched' => array(
				TermIndexEntry::TYPE_LABEL => array( 'AMatchedTerm' ),
			),
		);
		return array(
