...
class SetQualifierTest extends \ApiTestCase {
	...
	protected function claimProvider() {
		...
		$statement = new \Wikibase\StatementObject( $mainSnak );
	...
	public function testRequests() {
		foreach ( $this->claimProvider() as $claim ) {
			...
			foreach ( $this->newQualifierProvider() as $qualifier ) {
				$this->makeAddRequest( $claim->getGuid(), $qualifier );
			}
		}
	}
	...
	protected function makeAddRequest( $statementGuid, Snak $qualifier ) {
		...
		if ( $qualifier instanceof \Wikibase\PropertyValueSnak ) {
			$params['value'] = \FormatJson::encode( $qualifier->getDataValue()->getArrayValue() );
		}
