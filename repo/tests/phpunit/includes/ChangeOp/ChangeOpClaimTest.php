...
class ChangeOpClaimTest extends \PHPUnit_Framework_TestCase {
	...
	public function invalidIndexProvider() {
		return array(
			array( 'foo' ),
			array( array() ),
			array( $this->helper->makeStatement( 'P7' ) ),
		);
	}
	...
	public function testConstructionWithInvalidIndex( $invalidIndex ) {
		...
		new ChangeOpClaim(
			$this->helper->makeStatement( 'P7' ),
			$this->helper->getGuidGenerator(),
			$this->helper->getMockGuidValidator(),
			$this->helper->getMockGuidParser( $itemId ),
	...
	public function provideInvalidApply() {
		...
		$args['wrong entity'] = array ( $wrongItem, new ChangeOpClaim(
			...
			$this->helper->getGuidGenerator(),
			$this->helper->getMockGuidValidator(),
			$this->helper->getMockGuidParser( $item->getId() ),
			$this->helper->getMockSnakValidator()
		) );
	...
	public function testApplyInvalid( EntityId $entityId, Claim $claim ) {
		...
		$changeOpClaim = new ChangeOpClaim(
			...
			new ClaimGuidGenerator(),
			$this->helper->getMockGuidValidator(),
			$this->helper->getMockGuidParser( $entityId ),
			$this->helper->getMockSnakValidator(),
			0
		);
