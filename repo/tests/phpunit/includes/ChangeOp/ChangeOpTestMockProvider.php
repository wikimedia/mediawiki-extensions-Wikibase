...
class ChangeOpTestMockProvider {
	 ...
	 */
	/*public function getMockTitleLookup() {
	 ...
	 * @return SiteStore
	 */
	/*public function getMockSiteStore( array $sites = null ) {

		if ( $sites === null ) {
			$sites = array(
				'enwiki' => 'http://en.wikipedia.org/wiki/$1',
				'dewiki' => 'http://de.wikipedia.org/wiki/$1',
			);
		}
		...
		$mock->expects( PHPUnit_Framework_TestCase::any() )
			->method( 'getSite' )
			->will( PHPUnit_Framework_TestCase::returnCallback(
				function ( $siteId ) use ( $sites ) {
					if ( !isset( $sites[$siteId] ) ) {
						return null;
					}

					$url = $sites[$siteId];
					$group = preg_replace( '!^https?://\w{1,5}\.|\.(org|com|net|de)/.*$!', '', $url );

					$site = new Site();
					$site->setGlobalId( $siteId );
					$site->setLinkPath( $url );
					$site->setGroup( $group );
