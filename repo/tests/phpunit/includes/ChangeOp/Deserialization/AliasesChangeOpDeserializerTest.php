...
class AliasesChangeOpDeserializerTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGivenChangeRequestWithAdd_createsChangeOpsWithChangeOpAddingAliases() {
		$changeOpAddingAliases = $this->getMock( ChangeOp::class );
		$changeOpAddingAliases->wasApplied = false;
		$changeOpAddingAliases->method( 'validate' )->willReturn( true );
		$changeOpAddingAliases->method( 'apply' )
			->willReturnCallback(
				function () use ( $changeOpAddingAliases ) {
					$changeOpAddingAliases->wasApplied = true;
				}
			);
		$factory = $this->getFingerPrintChangeOpFactory();
		$factory->method( 'newAddAliasesOp' )->willReturn( $changeOpAddingAliases );
		...
		self::assertTrue( $changeOpAddingAliases->wasApplied );
	}
