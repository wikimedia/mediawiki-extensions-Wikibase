...
use Wikibase\Repo\ChangeOp\Deserialization\SiteLinkBadgeLookup;
...
class SiteLinksChangeOpDeserializerTest extends \PHPUnit_Framework_TestCase {
	...
	private function newSiteLinkBadgeLookup() {
		$lookup = $this->getMockBuilder( SiteLinkBadgeLookup::class )
	...
	private function newSiteLinksChangeOpDeserializer() {
		return new SiteLinksChangeOpDeserializer(
			...
			$this->newSiteLinkBadgeLookup(),
			new SiteLinkChangeOpFactory( array_flip( [ self::BADGE_ITEM_ID ] ) ),
	...
	public function testGivenInvalidSitelink_createEntityChangeOpThrowsException() {
		$validator = $this->getMock( SiteLinkChangeOpSerializationValidator::class );
	...
	public function testGivenInvalidBadgesData_createEntityChangeOpThrowsException() {
		$badgeLookup = $this->getMockBuilder( SiteLinkBadgeLookup::class )
			->disableOriginalConstructor()
			->getMock();

		$badgeLookup->method( 'parseSiteLinkBadgesSerialization' )
			->will( $this->throwException(
	...
	private function getItemWithSiteLinkwithBadges() {
		...
		$item->setSiteLinkList( new SiteLinkList( [ new SiteLink( self::SITE_ID, self::SITE_LINK_TITLE, [ new ItemId( self::BADGE_ITEM_ID ) ] ) ] ) );
	...
	public function testGivenSiteAndBadgesAndSiteLinkForSiteExistsWithBadges_changeOpSetsNewSetOfBadges() {
		...
		$deserializer = new SiteLinksChangeOpDeserializer(
			new SiteLinkChangeOpSerializationValidator(),
			$this->newSiteLinkBadgeLookup(),
			new SiteLinkChangeOpFactory( array_flip( [ self::BADGE_ITEM_ID, $newBadgeId ] ) ),
			$this->newSiteLinkTargetProvider(),
			new StringNormalizer(),
			[ 'testwikis' ]
		);
