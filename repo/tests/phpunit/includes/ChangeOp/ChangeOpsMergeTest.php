...
use Wikibase\Repo\Tests\AnItem;
...
class ChangeOpsMergeTest extends MediaWikiTestCase {
	...
	private function newItemWithId( $idString ) {
		return AnItem::create()->withId( $idString )->build();
	}
	...
	public function provideData() {
		$testCases = array();
		$emptyItem = AnItem::create()->build();

		$itemWithEnLabel = AnItem::create()
			->withLabel( 'en', 'foo' )
			->build();
		...
		$itemWithEnBarLabel = AnItem::havingLabel( 'en', 'bar' )
			->build();

		$itemWithLabelAndAlias = AnItem::havingLabel( 'en', 'bar' )
			->withAliases( 'en', [ 'foo' ] )
			->build();
		...
		$itemWithDescription = AnItem::create()
		...
		$itemWithBarDescription = AnItem::create()
		...
		$itemWithFooBarAliases = AnItem::create()
		...
		$itemWithFooBarBazAliases = AnItem::create()
		...
		$itemWithStatement = AnItem::create()
		...
		$itemWithQualifiedStatement = AnItem::create()
		...
		$bigItemBuilder = AnItem::havingId( 'Q111' )
			->withLabel( 'en', 'foo' )
			->withLabel( 'pt', 'ptfoo' )
			->withDescription( 'en', 'foo' )
		...
		$smallerMergedItem = AnItem::create()
			->withId( 'Q222' )
			->withDescription( 'pl', 'pldesc' )
			->withSiteLink( 'nlwiki', 'bar' )
			->build();

		$bigMergedItem = AnItem::create()
			->withId( 'Q111' )
			->withLabel( 'en', 'toLabel' )
			->withLabel( 'pt', 'ptfoo' )
			->withDescription( 'en', 'foo' )
	...
	public function testExceptionThrownWhenFromHasLink() {
		...
		$to = AnItem::create()->withId( 'Q222' )->build();
	...
	public function testExceptionThrownWhenToHasLink() {
		$from = AnItem::create()->withId( 'Q111' )->build();
