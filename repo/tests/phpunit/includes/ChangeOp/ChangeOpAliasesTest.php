...
class ChangeOpAliasesTest extends \PHPUnit_Framework_TestCase {
	...
	public function validateProvider() {
		...
		return array(
			'set invalid alias' => array( new ChangeOpAliases( 'fr', array( 'INVALID' ), 'set', $validatorFactory ) ),
			'add invalid alias' => array( new ChangeOpAliases( 'fr', array( 'INVALID' ), 'add', $validatorFactory ) ),
			'set invalid language' => array( new ChangeOpAliases( 'INVALID', array( 'valid' ), 'set', $validatorFactory ) ),
			'add invalid language' => array( new ChangeOpAliases( 'INVALID', array( 'valid' ), 'add', $validatorFactory ) ),
			'remove invalid language' => array ( new ChangeOpAliases( 'INVALID', array( 'valid' ), 'remove', $validatorFactory ) ),
		);
	}
	...
	public function testValidate( ChangeOpAliases $changeOpAliases ) {
		...
		$entity->getFingerprint()->setAliasGroup( new AliasGroup( 'zh', array( 'clone sucks' ) ) );

		$oldAliases = $entity->getFingerprint()->getAliasGroup( 'zh' )->getAliases();

		$result = $changeOpAliases->validate( $entity );
		$this->assertFalse( $result->isValid() );

		// Aliases should not have changed during validation
		$this->assertEquals( $oldAliases, $entity->getFingerprint()->getAliasGroup( 'zh' )->getAliases(), 'Aliases modified by validation!' );
	}
	...
	public function testValidateLeavesEntityUntouched() {
		...
		$changeOpAliases = new ChangeOpAliases( 'de', array( 'clone sucks' ), 'set', $validatorFactory );
		$changeOpAliases->validate( $entity );
		var_dump($entity);die();
		$this->assertEquals( Item::newEmpty(), $entity );
	}
