...
class FingerprintViewTest extends \MediaWikiLangTestCase {
	...
	public function testGetHtmlEditable( Fingerprint $fingerprint, EntityId $entityId, $languageCode ) {
		...
		if ( $fingerprint->getLabels()->hasTermForLanguage( $languageCode ) ) {
		...
		if ( $fingerprint->getDescriptions()->hasTermForLanguage( $languageCode ) ) {
		...
		if ( $fingerprint->getAliasGroups()->hasGroupForLanguage( $languageCode ) ) {
