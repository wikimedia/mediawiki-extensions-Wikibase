...
class TestDispatchCoordinator extends Maintenance {
	...
	private function createFakeTables() {
		...
		foreach ( $tables as $table ) {
			if ( $dbw->tableExists( self::TABLE_PREFIX . $table ) ) {
				$dbw->dropTable( self::TABLE_PREFIX . $table );
			}
			$dbw->duplicateTableStructure( $table, self::TABLE_PREFIX . $table );
		}
	...
	private function dropFakeTables() {
		$dbw = wfGetDB( DB_MASTER );
		$dbw->dropTable( self::TABLE_PREFIX . 'wb_changes_dispatch' );
		$dbw->dropTable( self::TABLE_PREFIX . 'wb_changes' );
	}
