...
const hydratedReferencesComponent = require( '../../../resources/wikibase.wbui2025/components/hydratedReferences.vue' );
const indicatorsComponent = require( '../../../resources/wikibase.wbui2025/components/indicators.vue' );
const { mount } = require( '@vue/test-utils' );
const { createTestingPinia } = require( '@pinia/testing' );

describe( 'wikibase.wbui2025.hydratedReferences', () => {
	it( 'defines component', async () => {
		expect( typeof hydratedReferencesComponent ).toBe( 'object' );
		expect( hydratedReferencesComponent )
			.toHaveProperty( 'name', 'WikibaseWbui2025HydratedReferences' );
	} );
	...
	describe( 'the mounted component', () => {
		...
		let wrapper;
		beforeEach( async () => {
			wrapper = await mount( hydratedReferencesComponent, {
				props: {
					references: [
						{
