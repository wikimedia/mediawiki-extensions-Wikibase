...
describe "Check edit-conflicts" do
  ...
  context "check behaviour on edit conflicts (descriptions)" do
    xit "should login as user 1, change description and save revid" do
      visit_page(RepoLoginPage) do |page|
    ...
    xit "should login as user 2 (anon), change description" do
      visit_page(RepoLoginPage) do |page|
        page.logout_user
      end
      on_page(ItemPage) do |page|
    ...
    xit "should login as user 1 again, inject old revid & complain about edit conflict when changing description" do
      visit_page(RepoLoginPage) do |page|
        page.login_with(WIKI_ORDINARY_USERNAME, WIKI_ORDINARY_PASSWORD)
      end
      on_page(ItemPage) do |page|
    ...
    xit "should be possible to add sitelink" do
      on_page(ItemPage) do |page|
    ...
    xit "should be possible to change label" do
      on_page(ItemPage) do |page|
    ...
    xit "should be possible to add aliases" do
      on_page(ItemPage) do |page|
  ...
  context "check normal behaviour" do
    xit "should be possible to edit description again after a reload" do
      on_page(ItemPage) do |page|
        page.navigate_to_item
        page.wait_for_entity_to_load
        page.entityDescriptionSpan.should == description_user2
  ...
  context "check behaviour on edit conflicts (claims)", :experimental => true do
    ...
    it "should login as user 1, change claim and save revid" do
      ...
      on_page(ItemPage) do |page|
        ...
        first_claim_guid = @browser.execute_script("return $('.wb-claimview').first().data('claimview').value().getGuid()");
