...
describe( 'Property prefix search', () => {
	...
	const property1EnLabel = englishTermMatchingTwoProperties + ' some suffix';

	before( async () => {
		property1 = await createProperty( {
			data_type: 'string',
			labels: { en: property1EnLabel, de: 'de-label-' + utils.uniq() },
	...
	describe( '200 success response', () => {
		it( 'finds properties matching the search term', async () => {
			...
			assert.deepEqual( property1Result, {
				id: property1.id,
				'display-label': { language, value: property1EnLabel },
				description: { language, value: property1.descriptions.en },
				match: { type: 'label', language, text: property1EnLabel }
			} );
		...
		it( 'allows users with apihighlimits to use a higher limit', async () => {
			const response = await newSearchRequest( 'en', englishTermMatchingTwoProperties )
