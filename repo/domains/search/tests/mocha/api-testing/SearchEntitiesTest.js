...
async function getConceptUri( entityId ) {
...
describe( 'wbsearchentities', () => {
	...
	before( async () => {
		...
		testPropertyConceptUri = await getConceptUri( testPropertyId );
	} );
	...
	describe( 'property search', () => {
		...
		it( 'response contains the expected fields and result shape', async () => {
			...
			assert.equal( response.searchinfo.search, testPropertyId );
			assert.equal( response.success, 1 );
		...
		it( 'finds property by English label', async () => {
			...
			assert.equal( result.display.label.value, PROP_EN_LABEL );
			assert.equal( result.display.label.language, 'en' );
		} );
