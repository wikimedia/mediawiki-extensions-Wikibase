...
class RebuildTermStore extends Maintenance {
	...
	public function __construct() {
		...
		$this->addOption(
			'entity-type', 'The type of entity to rebuild terms for. Either "item" or "property"',
			true,
			true
		);
	...
	public function execute() {
		...
		$rebuilder = new TermStoreRebuilder(
			new InMemoryPropertyTermStore(), // TODO
			new InMemoryItemTermStore(), // TODO
			$this->newEntityIdPager(),
			$this->getReporter(),
			$this->getErrorReporter(),
			MediaWikiServices::getInstance()->getDBLoadBalancerFactory(),
			$this->wikibaseRepo->getEntityRevisionLookup( Store::LOOKUP_CACHING_RETRIEVE_ONLY ),
