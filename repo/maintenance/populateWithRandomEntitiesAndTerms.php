...
class PopulateWithRandomEntitiesAndTerms extends Maintenance
{
	...
	public function execute() {
		if ( WikibaseRepo::getDefaultInstance()
			...
			$this->output( "This script is not enabled! To enable it, set '\enablePopulateWithRandomEntitiesAndTermsScript => true' in config/Wikibase.default.php.\n" );
			exit;
		}
	...
	private function saveEntity( EntityDocument $entity, User $user, MediawikiEditEntityFactory $editEntityFactory ) {
	...
	private function addLabelsToEntity( EntityDocument $entity, array $languages, Generator $textGenerator ) {
	...
	private function addDescriptionsToEntity( EntityDocument $entity, array $languages, Generator $textGenerator ) {
		foreach ( $languages as $language ) {
			$termText = $textGenerator->current();
			$textGenerator->next();
	...
	private function addAliasesToEntity( EntityDocument $entity, array $languages, Generator $textGenerator ) {
		foreach ( $languages as $language ) {
			$termText = $textGenerator->current();
			$textGenerator->next();
