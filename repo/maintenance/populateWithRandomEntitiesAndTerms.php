...
class PopulateWithRandomEntitiesAndTerms extends Maintenance
{
	...
	private function generateEntities( $entityType, $textGenerator, $nrOfEntities, $languages ) {
		$entityClass = $entityType === 'item' ? Item::class : Property::class;

		for ( $i = 0; $i < $nrOfEntities; $i++ ) {
			$entity = new $entityClass( null, new Fingerprint() );
			...
			$this->addLabelsToEntity( $entity, $languages, $labelTextGeenerator );
			...
			// save entity
			$this->output( print_r( $entity, true )  );
		}
	}
	...
	private function addLabelsToEntity( $entity, $languages, $textGenerator ) {
		foreach ( $languages as $language ) {
			...
			$entity->getFignerprint()->setLabel( $langauge, $termText );
		}
	}
	...
	private function addDescriptionsToEntity( $entity, $languages, $textGenerator ) {
		foreach ( $languages as $language ) {
			...
			$entity->getFignerprint()->setDescription( $langauge, $termText );
		}
	}
	...
	private function addAliasesToEntity( $entity, $languages, $textGenerator ) {
		foreach ( $languages as $language ) {
			...
			$entity->getFignerprint()->setAliasGroup( $langauge, [ $termText ] );
		}
	}
