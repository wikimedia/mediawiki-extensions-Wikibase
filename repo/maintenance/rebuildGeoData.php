...
class RebuildGeoData extends Maintenance {
	 ...
	 */
	private function updateSearch( LinksUpdate $linksUpdate ) {
		// @fixme: this gets ParserOutput from ParserCache so would not have
		// coordinates for cached pages and not work correctly for this.
		// we could separately run forceSearchIndex.php on the item namespace
		// or ideally forceSearchIndex.php can work based on a list of page ids
		// to obtain non-cached ParserOutput or even better, make use of the
		// Content and ParserOutput objects that we already have here.
		// \CirrusSearch\Hooks::onLinksUpdateCompleted( $linksUpdate );
	}
