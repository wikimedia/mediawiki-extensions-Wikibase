...
class ClearTermSqlIndexSearchFields extends Maintenance {
	...
	public function __construct() {
		...
		$this->addOption(
			'clear-term-weight',
			'Whether to clear the term_weight column or not. ' .
			...
			'Defaults to true; set to false to reduce UPDATE load.',
			false,
			true
		);
	}
	...
	private function getTermSqlIndexSearchFieldsClearer() {
		...
		switch ( $this->getOption( 'clear-term-weight', 'true' ) ) {
			case 'true':
				$clearTermWeight = true;
				break;
			case 'false':
				$clearTermWeight = false;
				break;
			default:
				$this->fatalError(
					'--clear-term-weight option must be "true" or "false"'
				);
		}
