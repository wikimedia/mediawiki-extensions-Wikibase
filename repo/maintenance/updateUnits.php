...
QUERY;
		$baseUnitsData = $client->query( $baseQuery );
		$baseUnits = [];
		// arrange better lookup
		foreach ( $baseUnitsData as $base ) {
			$item = substr( $base['item'], $baseLen );
			$baseUnits[$item] = $base;
		}
		...
		$usageQuery = <<<UQUERY
SELECT ?unit (COUNT(?unit) as ?c) WHERE {
...
# Exclude currencies
  ...
  LIMIT 200
UQUERY;
		$unitUsage = $client->getIDs( $usageQuery, "unit" );
...
QUERY;
		...
		foreach ( $baseUnits as $base => $baseData ) {
			$convertUnits[$base] = [
				'factor' => "1",
				'unit' => $base,
				'label' => $baseData['itemLabel'],
				'siLabel' => $baseData['itemLabel']
			];
		}
