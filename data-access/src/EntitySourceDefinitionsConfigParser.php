		...
		foreach ( $sourceConfig as $sourceName => $sourceData ) {
			...
			foreach ( $sourceData['entityNamespaces'] as $entityType => $namespaceSlot ) {
				if ( !is_string( $entityType ) ) {
					throw new InvalidArgumentException( 'Entity namespace definition of entity source "' . $sourceName . '" should be indexed by strings' );
				}
				if ( !is_string( $namespaceSlot ) && !is_int( $namespaceSlot ) ) {
					throw new InvalidArgumentException( 'Entity namespaces of entity source "' . $sourceName . '" should be either a string or an integer' );
				}
			}
