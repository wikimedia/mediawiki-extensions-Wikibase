...
class RepositoryServiceContainerFactory {
	...
	public function __construct(
		...
		foreach ( $wiringFiles as $wiringFile ) {
			if (!file_exists($wiringFile)) {
				throw new \InvalidArgumentException( "File `$wiringFile` does not exist" );
			}
		}
		...
		$this->wiringFiles = array_merge(
			[ __DIR__ . '/RepositoryServiceWiring.php' ],
			$wiringFiles
		);
		$this->client = $client;
	}
