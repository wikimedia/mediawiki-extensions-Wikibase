...
final class ClientHooks {
	...
	public static function onSpecialMovepageAfterMove( \MovePageForm $movePage, \Title &$oldTitle, \Title &$newTitle ) {
		...
		if ( $itemId !== false ) {
			$itemByTitle = 'Special:ItemByTitle/' . $globalId . '/' . wfUrlencode( $oldTitle->getDBkey() );
	...
	public static function onSkinTemplateOutputPageBeforeExec( \Skin &$skin, \QuickTemplate &$template ) {
		...
		if ( in_array( $title->getNamespace(), Settings::get( 'namespaces' ) ) && $title->exists() ) {
			...
			if ( $itemId ) {
				...
				$template->data['language_urls'][] = array(
					'href' => ClientUtils::repoArticleUrl( "Special:ItemByTitle/$siteId/" . wfUrlencode( $titleText ) ),
