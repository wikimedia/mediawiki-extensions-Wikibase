...
final class ClientHooks {
	...
	public static function onSkinBuildSidebar( Skin $skin, &$bar ) {
		...
		$sitesStore = SiteSQLStore::newInstance();
		...
		if ( count( $siteIdsToOutput ) === 0 ) {
			$otherProjectsSitesProvider = new OtherProjectsSitesProvider( $sitesStore, $settings->getSetting( 'siteLinkGroups' ) );
			$currentSite = $sitesStore->getSite( $settings->getSetting( 'siteGlobalID' ) );
	 ...
	 * @param Skin $skin
	 */
	public static function onSidebarBeforeOutput( array &$sidebar, Skin $skin ) {
		$settings = WikibaseClient::getDefaultInstance()->getSettings();
		if (
			$settings->getSetting( 'otherProjectsLinksBeta' ) &&
			!$settings->getSetting( 'otherProjectsLinksByDefault' ) &&
	...
	public static function onGetBetaFeaturePreferences( User $user, array &$betaPreferences ) {
		...
		$betaPreferences['wikibase-otherprojects'] = array(
			...
			'screenshot' => array(
				'ltr' => $wgExtensionAssetsPath . '/Wikibase/client/resources/images/wb-otherprojects-beta-ltr.png',
				'rtl' => $wgExtensionAssetsPath . '/Wikibase/client/resources/images/wb-otherprojects-beta-rtl.png'
			),
