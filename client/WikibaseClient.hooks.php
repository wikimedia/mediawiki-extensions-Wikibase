		...
		if ( $rev !== false ) {
			...
			$params = array(
				...
				'rc-external-data' => array(
					'rc_curid' => $rev->rev_page,
					'rc_this_oldid' => $rev->rev_id,
					'rc_last_oldid' => $rev->rev_parent_id,
					'rc_user' => $rev->rev_user,
					'rc_user_text' => $rev->rev_user_text,
				)
			);
			...
			$rc = \RecentChange::newLogEntry( $fields['time'], $title, $user, '',
