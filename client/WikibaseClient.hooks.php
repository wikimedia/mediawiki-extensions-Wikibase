...
final class ClientHooks {
	...
	public static function onSkinTemplateGetLanguageLink( &$languageLink, Title $languageLinkTitle, Title $title ) {
		...
		$linkBadges = $languageLinkBadgeHandler->getBadges( $title, $languageLinkTitle );
		foreach ( $linkBadges as $badge ) {
		...
		if ( is_array( $settings->get( 'displayBadges' ) ) ) {
			foreach ( $settings->get( 'displayBadges' ) as $badge => $icon ) {
				if ( in_array( $badge, $linkBadges ) ) {
					$title = $languageLinkBadgeHandler->getTitle( $badge, $wgLang->getCode() );
					if ( $title ) {
