...
class PropertyParserFunction {
	...
	public function __construct( \Title $title, $siteId,
		WikiPageEntityLookup $entityLookup, SiteLinkLookup $siteLinkLookup ) {
	...
	protected function setEntityId( \Title $title, $siteId ) {
		...
		$this->entityId = new EntityId( Item::ENTITY_TYPE, $itemId );
	...
	public static function render( \Parser $parser, $propertyLabel ) {
		...
		$instance = new self( $parser->getTitle(), Settings::get( 'siteGlobalID' ), $entityLookup, $siteLinkLookup );
