...
class BulkSubscriptionUpdater {
	...
	private function slurpSubscriptions( ResultWrapper $res, &$continuation = array() ) {
		...
		if ( isset( $row ) ) {
			$continuation = array( $row->eu_entity_id );
		}
	...
	public function purgeSubscriptions() {
		while ( $this->runDeletionBatch() ) {
			$this->progressReporter->reportMessage( 'Purging subscription table: '
				. "deleted subscriptions for {$this->subscriberId}." );
		};
	}
	...
	private function runDeletionBatch() {
		...
		if ( !$dbw->selectRow( 'wb_changes_subscription', 'cs_row_id', $conditions ) ) {
			// HACK: Since $dbw->affectedRows() doesn't seem to work for deletions, we check to see
			// whether there is anything left to delete.
			return false;
		}
		...
		$dbw->delete(
			...
			__METHOD__,
			array(
				'LIMIT' => $this->batchSize, //FIXME: LIMIT doesn't work here!
			)
		);
