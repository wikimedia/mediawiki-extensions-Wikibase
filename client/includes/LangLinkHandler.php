	...
	protected function getSiteByNavigationId( $id, $group ) {
		...
		if ( $this->sitesByNavigationId === null ) {
			...
			foreach ( $this->sites->getSites() as $site ) {
				...
				if ( !isset( $this->sitesByNavigationId[$site->getSiteGroup()] ) ) {
					$this->sitesByNavigationId[$site->getSiteGroup()] = array();
				}

				foreach ( $ids as $navId ) {
					$this->sitesByNavigationId[$site->getSiteGroup()][$navId] = $site;
				}
			}
		}
