...
class LanguageLinkBadgeHandler {
	...
	public function getSiteLink( Title $title, $siteId ) {
		$siteLink = new SiteLink( $this->siteId, $title->getText() );
	...
	public function getBadges( Title $title, Title $languageLinkTitle ) {
		$interwiki = Interwiki::fetch( $languageLinkTitle->getInterwiki() );
		if ( !$interwiki ) {
			return array();
		}

		$siteLink = $this->getSiteLink( $title, $interwiki->getWikiID() );
		if ( !$siteLink ) {
			return array();
		}
