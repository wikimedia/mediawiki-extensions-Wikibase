...
class SidebarHookHandlers {
	...
	public static function newFromGlobalState() {
		...
		return new SidebarHookHandlers(
			$namespaceChecker,
			$badgeDisplay,
			function() use ( $wikibaseClient ) {
				return $wikibaseClient->getOtherProjectsSidebarGenerator();
			},
	...
	public function __construct(
		...
		\Closure $otherProjectsSidebarGenerator,
	...
	public function doSidebarBeforeOutput( Skin $skin, array &$sidebar ) {
		...
		if ( $this->otherProjectsLinksDefault || $betaFeatureEnabled ) {
			...
			if ( $otherProjectsSidebar === null ) {
				// @todo do lazy initialization in a nicer way
				$otherProjectsSidebarGenerator = call_user_func( $this->otherProjectsSidebarGenerator );
