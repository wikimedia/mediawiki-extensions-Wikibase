...
class ParserOutputHooks {
	...
	public function doParserAfterParse( Parser &$parser, &$text, StripState $stripState ) {
		if ( !$this->isWikibaseEnabled( $parser->getTitle()->getNamespace() ) ) {
			// shorten out
			return true;
		}
		...
		$useRepoLinks = $this->langLinkHandler->useRepoLinks( $parser->getTitle(), $parser->getOutput() );

		try {
			if ( $useRepoLinks ) {
				// add links
				$this->langLinkHandler->addLinksFromRepository( $parser->getTitle(), $parser->getOutput() );
			}

			$this->langLinkHandler->updateItemIdProperty( $parser->getTitle(), $parser->getOutput() );
		} catch ( \Exception $e ) {
