...
class LanguageLinkBadgeDisplay {
	...
	public function assignBadges( Title $title, Title $languageLinkTitle, array &$languageLink ) {
		...
		foreach ( $siteLink->getBadges() as $badgeItemId ) {
			$idSerialization = $badgeItemId->getSerialization();
			$badge = Sanitizer::escapeClass( $idSerialization );
			if ( !isset( $languageLink['class'] ) ) {
				$languageLink['class'] = "badge-$badge";
			} else {
				$languageLink['class'] .= " badge-$badge";
			}
			$linkBadges[] = $idSerialization;
		}

		// use the order of the config
		$displayBadges = array_intersect( $this->displayBadges, $linkBadges );
