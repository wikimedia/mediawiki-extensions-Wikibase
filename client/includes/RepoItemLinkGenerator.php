...
class RepoItemLinkGenerator {
	...
	public function __construct( NamespaceChecker $namespaceChecker,
		RepoLinker $repoLinker, $enableSiteLinkWidget ) {
	...
	public function getLink( \Title $title, $action, $isAnon, $noExternalLangLinks, $prefixedId ) {
		...
		if ( $this->canHaveLink( $title, $action, $noExternalLangLinks ) ) {
			$entityId = $this->getEntityId( $prefixedId );
	 ...
	 */
	protected function getEntityId( $prefixedId ) {
		if ( $prefixedId !== null ) {
			return EntityId::newFromPrefixedId( $prefixedId );
		}
