...
class RepoItemLinkGenerator {
	 ...
	 * @return string|null
	 */
	public function getLink( Title $title, $action, $noExternalLangLinks, $prefixedId ) {
		$entityId = null;
		if ( is_string( $prefixedId ) ) {
			$entityId = $this->entityIdParser->parse( $prefixedId );
		}

		if ( $entityId && $this->hasLangLinks ) {
	...
	private function getEditLinksLink( EntityId $entityId ) {
		$link = array(
			'href' => $this->getEntityLink( $entityId ),
	 ...
	 *
	 * @param Title $title
	 * @param EntityId|null $entityId Entity id we're linked to or null
	 *
	 * @return string
	 */
	 ...
	 * @return string
	 */
	private function getEntityLink( EntityId $entityId ) {
