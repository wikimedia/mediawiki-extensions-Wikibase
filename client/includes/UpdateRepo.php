...
abstract class UpdateRepo {
	 ...
	 * @return bool
	 */
	public function injectJob( $jobQueueGroup ) {
		wfProfileIn( __METHOD__ );

		if ( !$jobQueueGroup ) {
			wfProfileOut( __METHOD__ );
			throw new \MWException( "Invalid job queue group given" );
		}
		...
		if ( !$job ) {
			wfProfileOut( __METHOD__ );
			throw new \MWException( 'Tried to inject an invalid job' );
		}
		...
		if ( !$ok ) {
			// We don't throw an exception here as this could eg. happen if
			// the repo DB is in read only mode or the configuration is flawed.
			wfLogWarning( "Failed to push to job queue" );
			wfProfileOut( __METHOD__ );
			return false;
		}
