...
class ApiPropsEntityUsage extends ApiQueryBase {
	...
	public function execute() {
		...
		$repoUrl = $settings->getSetting( 'repoUrl' );
		$repoArticlePath = $settings->getSetting( 'repoArticlePath' );
		foreach ( $res as $row ) {
			...
			if ( isset( $prop['url'] ) ) {
				$entityUrl = urlencode( $row->eu_entity_id );
				// ewww
				$repoArticlePath = str_replace( '$1', $entityUrl, $repoArticlePath );
