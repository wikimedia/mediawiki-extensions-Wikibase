...
class SiteLinkCommentCreator {
	 ...
	 */
	public function getEditComment( Diff $siteLinkDiff = null, $action, Title $title ) {
	...
	private function getSiteLinkAddRemoveParams( DiffOp $diffOp, $action, $siteId, Title $title ) {
		...
		if ( in_array( $action, array( 'remove', 'restore' ) ) ) {
			...
			if ( $title->getFullText() === $diffOp->getOldValue() ) {
				$params['message'] = 'wikibase-comment-unlink';
			} elseif ( $title->getFullText() === $diffOp->getNewValue() ) {
				$params['message'] = 'wikibase-comment-linked';
			} else {
