...
class InfoActionHookHandler {
	...
	private function formatEntityUsage( IContextSource $context, array $usage ) {
		...
		foreach ( $usage as $key => $entityUsage ) {
			$entityId = (string)$entityUsage->getEntityId();
			$entities[$entityId] = $entityUsage->getEntityId();
			if ( !isset( $formattedUsage[$entityId] ) ) {
				$formattedUsage[$entityId] = [];
			}
			$formattedUsage[$entityId][] = $entityUsage->getAspect();
		}
		$output = '';

		foreach ( $formattedUsage as $entityId => $aspects ) {
