...
class SpecialUnconnectedPages extends SpecialWikibaseQueryPage {
	...
	protected function getResult( $offset = 0, $limit = 0 ) {
		...
		if ( $this->startTitle !== null ) {
			if ( $this->getNamespaceChecker()->isWikibaseEnabled( $this->startTitle->getNamespace() ) === false ) {
				// TODO: Better error handling
				// XXX: There is a bug here, with no namespaces enabled the code does not enter here, which it should
				return array();
			}
			...
			$namespaces = $this->getNamespaceChecker()->getValidNamespaces();
			if ( empty( $namespaces ) ) {
				// TODO: Better error handling
				return array();
			}
