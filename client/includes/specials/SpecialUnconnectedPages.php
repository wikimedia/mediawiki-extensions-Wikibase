...
class SpecialUnconnectedPages extends SpecialWikibaseQueryPage {
	 ...
	 * @return string[]
	 */
	public function buildConditionals( Title $title = null, $dbr, $checker ) {
		if ( $title !== null ) {
			if ( $checker->isWikibaseEnabled( $title->getNamespace() ) === false ) {
				return array();
			}
			$conds[] = 'page_title >= ' . $dbr->addQuotes( $title->getText() );
			$conds[] = 'page_namespace = ' . $title->getNamespace();
		}
		else {
			$namespaces = $checker->getValidNamespaces();
			if ( empty( $namespaces ) ) {
				return array();
			}
			$conds[] = 'page_namespace IN (' . implode(',', $namespaces ) . ')';
		}
		return $conds;
	}
	...
	protected function getResult( $offset = 0, $limit = 0 ) {
		...
		$conds = $this->buildConditionals( $this->startTitle, $dbr, $this->getNamespaceChecker() );
		...
		$rows = $dbr->select(
			...
			array(
				...
				//'USE INDEX' => array( 'page_props' => 'pp_propname' )
			),
			array(
