...
class UsageAspectTransformer {
	...
	private function getFilteredAspects( array $aspectKeys, array $relevant ) {
		...
		$aspectMap = array_flip( $directMatches );

		// Matches 'L.xx' in $aspects to 'L' in $relevant.
		$this->intersectAspectsIntoMap( $aspectKeys, $relevant, $aspectMap );

		// Matches 'L.xx' in $relevant to 'L' in $aspects.
		$this->intersectAspectsIntoMap( $relevant, $aspectKeys, $aspectMap );

		ksort( $aspectMap );
		return array_keys( $aspectMap );
	}

	/**
	 * @param string[] $aspectKeys Array of aspect keys, with modifiers applied.
	 * @param string[] $relevant Array of aspects (without modifier).
	 * @param array &$aspectMap Map of aspect keys (with modifiers), the value being meaningless.
	 */
	private function intersectAspectsIntoMap( array $aspectKeys, array $relevant, array &$aspectMap ) {
