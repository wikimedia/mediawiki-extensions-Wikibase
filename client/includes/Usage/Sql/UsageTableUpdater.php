...
class UsageTableUpdater {
	 ...
	 */
	public function addUsageForPage( $pageId, array $usages, $touched ) {
		if ( empty( $usages ) ) {
			return 0;
		}
	 ...
	 *
	 */
	public function queryUsagesForPage( $pageId, $timeOp, $timestamp ) {
		if ( !is_int( $pageId ) ) {
	...
	public function pruneStaleUsages( $pageId, $lastUpdatedBefore ) {
		...
		$old = $this->queryUsagesForPage( $pageId, '<', $lastUpdatedBefore );
