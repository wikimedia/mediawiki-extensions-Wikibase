...
class SqlUsageTracker implements UsageTracker, UsageLookup {
	...
	public function trackUsedEntities( $pageId, array $usages, $touched ) {
		...
		try {
			...
			$oldUsages = $tableUpdater->queryUsagesForPage( $pageId, '>=', '00000000000000' );
			...
			$tableUpdater->addUsageForPage( $pageId, $added, $touched );
	...
	public function getUsagesForPage( $pageId, $touchedSince = '00000000000000' ) {
		...
		$tableUpdater = $this->newTableUpdater( $db );
		$usages = $tableUpdater->queryUsagesForPage( $pageId, '>=', $touchedSince );
	...
	public function getPagesUsing( array $entityIds, array $aspects = array() ) {
		...
		$idStrings = $this->getEntityIdStrings( $entityIds );
		$where = array(
			'eu_entity_id' => $idStrings,
			'eu_touched IS NULL OR ( eu_touched >= page_touched )' // ignore stale entries //FIXME TEST ME
		);
		...
		$res = $db->select(
			...
			array(),
			array(
				'page' => array( 'JOIN', 'page_id=eu_page_id' )
			)
		);
