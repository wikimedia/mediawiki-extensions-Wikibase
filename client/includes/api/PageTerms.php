...
class PageTerms extends ApiQueryBase {
	...
	public function execute() {
		...
		$pagesToEntityIds = $this->getEntityIdsForTitles( $titles, $params['continue'] );
	 ...
	 * @return array
	 */
	private function getEntityIdsForTitles( array $titles, $continue = null ) {
		$this->addTables( 'page_props' );
		$this->addFields( array( 'pp_page', 'pp_value' ) );
		$this->addWhereFld( 'pp_page', array_keys( $titles ) );

		if ( $continue ) {
			$this->addWhere( 'pp_page >=' . intval( $continue ) );
		}

		$this->addWhereFld( 'pp_propname', 'wikibase_item' );
