...
class PageTerms extends ApiQueryBase {
	...
	private function addTermsToResult( array $pagesToEntityIds, array $termGroups ) {
		...
		foreach ( $pagesToEntityIds as $currentPage => $entityId ) {
			if ( !isset( $termGroups[$currentPage] ) ) {
				//FIXME: This should not be a warning, as it can legitly happen when there are no
				//       terms for a page. This is a temporary hack to find an issue with Jenkins.
				wfWarn( __METHOD__ . ': No terms found for page ID ' . $currentPage );
				continue;
			}
	 ...
	 */
	private function addPageTerms( ApiResult $result, $pageId, $terms ) {
		if ( !is_array( $terms ) ) {
			throw new InvalidArgumentException( '$terms must be an array, got ' . gettype( $terms ) );
		}
