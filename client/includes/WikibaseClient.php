...
final class WikibaseClient {
	...
	private static function getRepositoryDefinitionsFromSettings( SettingsArray $settings ) {
		...
		// use its values.
		$repoSettingsArray = [];
		if ( $settings->hasSetting( 'foreignRepositories' ) ) {
			$repoSettingsArray = $settings->getSetting( 'foreignRepositories' );
		} elseif( $settings->hasSetting( 'repositories' ) ) {
			$repoSettingsArray = $settings->getSetting( 'repositories' );
		}
