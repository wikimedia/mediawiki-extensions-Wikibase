...
class AffectedPagesFinder {
	...
	private function getAffectedPages( EntityChange $change ) {
		...
		$relevantAspects = array( 'X', $changedAspects ); // X implies all!
		...
		if ( $change instanceof ItemChange && in_array( EntityUsage::TITLE_USAGE, $changedAspects ) ) {
			...
			//TODO: Inject $usagesFromDiff "in the fly" while streaming other usages.
			$this->mergeUsagesInto( $usagesFromDiff, $usages );
		}
