...
class WBCLangLinkHandler {
	...
	protected static function getLinks( $db_title ) {
		...
		if( isset( $source['api'] ) ) {
			return self::getLinksFromApi( $db_title, $source['api'] );
		} elseif( isset( $source['dir'] ) ) {
			return self::getLinksFromFile( $db_title, $source['dir'] );
		} else {
			return array();
		}
	}
	 ...
	 * @return Array of links, empty array for no links, false for failure.
	 */
	protected static function getLinksFromApi( $db_title, $api ) {
		global $wgLanguageCode;

		$url = $api .
			"?action=wbgetitem&format=php&site=" . $wgLanguageCode . "&title=" . urlencode( $db_title );
		...
		$links = $api_response['page']['sitelinks'];
	 ...
	 * @return Array of links, empty array for no links, false for failure.
	 */
	protected static function getLinksFromFile( $db_title, $dir ) {
		$file = "$dir/$db_title.json";
		if( file_exists( $file ) ) {
