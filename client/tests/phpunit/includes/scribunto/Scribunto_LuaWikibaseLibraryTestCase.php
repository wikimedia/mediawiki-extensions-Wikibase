...
class Scribunto_LuaWikibaseLibraryTestCase extends \Scribunto_LuaEngineTestBase {
	...
	public static function suite( $className ) {
		// If WikibaseClient can't find a Repo store it will throw an exception
		// therefor this tests wont work, even though we Mock away the real store...
		if (
			...
			$suite->addTest(
				new Scribunto_LuaEngineTestSkip(
					$className,
					"Skipping Lua tests because WikibaseClient doesn't have a local site link table."
				)
			);
			return $suite;
		}
