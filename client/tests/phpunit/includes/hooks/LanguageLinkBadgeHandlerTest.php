...
class LanguageLinkBadgeHandlerTest extends \MediaWikiTestCase {
	...
	private function getLanguageLinkBadgeHandler() {
		...
		$siteLinkLookup->expects( $this->any() )
			...
			->with( $this->logicalOr(
				...
				new SiteLink( 'dewiki', 'Wikipedia:Wikidata' ),
			...
			->will( $this->returnCallback( function( $link ) {
				if ( $link->getPageName() == 'Georg Friedrich Haendel'
					|| $link->getPageName() == 'Wikipedia:Wikidata'
				) {
	...
	public function getSiteLinkProvider() {
		...
		return array(
			...
			array( $siteLinkEnwiki, Title::newFromText( 'Wikipedia:Wikidata' ), 'enwiki', 'passing enwiki for non-main namespace title' ),
	...
	public function getBadgesProvider() {
		return array(
			...
			array( array( 'Q43', 'Q148' ), Title::newFromText( 'Wikipedia:Wikidata' ), Title::newFromText( 'en:George Frideric Handel' ), 'passing enwiki for non-main namespace title' ),
