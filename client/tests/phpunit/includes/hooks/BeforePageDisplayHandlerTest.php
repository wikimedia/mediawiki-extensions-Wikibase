...
class BeforePageDisplayHandlerTest extends \PHPUnit_Framework_TestCase {
	...
	public function testHandle_WikibaseForNamespace( $expectedJsModules, $expectedCssModules,
		...
		$modifiedOutput = $handler->addModules( $output, $skin, $actionName );

		$this->assertEquals( $expectedJsModules, $modifiedOutput->getModules(), 'js modules' );
		$this->assertEquals( $expectedCssModules, $modifiedOutput->getModuleStyles(), 'css modules' );
	}
