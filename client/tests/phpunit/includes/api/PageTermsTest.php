...
class PageTermsTest extends \MediaWikiTestCase {
	...
	private function newEntityId( $pageId ) {
		if ( $pageId > 1000 ) {
	...
	private function getEntityIdLookup() {
		$self = $this; // no access to $this in closures in PHP 5.3

		$idLookup = $this->getMock( 'Wikibase\Client\Store\EntityIdLookup' );
		$idLookup->expects( $this->any() )
			->method( 'getEntityIds' )
			->will( $this->returnCallback( function( array $titles ) use ( $self ) {
				$entityIds = array();
				...
				foreach ( $titles as $title ) {
					...
					$entityIds[$pid] = $self->newEntityId( $pid );
				}
