...
class UsageUpdaterTest extends \PHPUnit_Framework_TestCase {
	...
	private function getUsageTracker( array $oldUsage = null ) {
		...
		$mock = $this->getMock( 'Wikibase\Usage\UsageTracker' );

		$expects = $oldUsage === null ? $this->never() : $this->once();
	...
	private function getUsageLookup( array $unusedEntities = null ) {
		$mock = $this->getMock( 'Wikibase\Usage\UsageLookup' );

		$expects = $unusedEntities === null ? $this->never() : $this->once();
		$mock->expects( $expects )
	...
	private function getSubscriptionManager( $wiki, $subscribe, $unsubscribe ) {
		$mock = $this->getMock( 'Wikibase\Subscription\SubscriptionManager' );

		$empty = empty( $subscribe ) && empty( $unsubscribe );

		$expects = $empty ? $this->never() : $this->once();
		$mock->expects( $expects )
			->method( 'subscribe' )
