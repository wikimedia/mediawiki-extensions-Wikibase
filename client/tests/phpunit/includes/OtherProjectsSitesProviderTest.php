...
class OtherProjectsSitesProviderTest extends \MediaWikiTestCase {
	 ...
	 */
	public function testOtherProjectSites( SiteStore $siteStore, array $supportedSites, Site $inputSite, SiteList $result ) {
		$otherProjectsSitesProvider = new OtherProjectsSitesProvider( $siteStore, $supportedSites );

		$this->assertEquals(
			$result,
			$otherProjectsSitesProvider->getOtherProjectsSites( $inputSite )
		);
	}
