...
class RepoItemLinkGeneratorTest extends \MediaWikiTestCase {
	...
	public function getLinksProvider() {
		...
		$data[] = array( $editLinksLink, array( $title, 'view', false, null, $prefixedId ) );
		$data[] = array( $addLinksLink, array( $title, 'view', false, null, null ) );
		$data[] = array( null, array( $nonExistingTitle, 'view', false, null, null ) );
		$data[] = array( null, array( $title, 'view', true, null, null ) );
		$data[] = array( null, array( $title, 'history', false, null, $prefixedId ) );
		$data[] = array( $editLinksLink, array( $title, 'view', true, null, $prefixedId ) );
		$data[] = array( null, array( $title, 'view', false, array( '*' ), $prefixedId ) );
	 ...
	 */
	public function testGetLinks( $expected, $params ) {
		$repoLinker = $this->getRepoLinker();
		$namespaceChecker = $this->getNamespaceChecker();

		$repoItemLinkGenerator = new RepoItemLinkGenerator( $namespaceChecker, $repoLinker, true );

		$link = $repoItemLinkGenerator->getLink( $params[0], $params[1], $params[2], $params[3], $params[4] );
