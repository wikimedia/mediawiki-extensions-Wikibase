...
class ChangeHandlerTest extends \MediaWikiTestCase {
	...
	private function newChangeHandler( PageUpdater $updater = null, array $entities = array() ) {
		...
		$mangler = $this->getMock( 'Wikibase\Client\Changes\ChangeListMangler' );
		$mangler->expects( $this->any() )
			->method( 'mangleChanges' )
	...
	public function testHandleChanges() {
		...
		$changeHandler = $this->getMockBuilder( 'Wikibase\Client\Changes\ChangeHandler' )
			->disableOriginalConstructor()->setMethods( array( 'handleChange' ) )->getMock();
