...
class OtherProjectsSidebarGeneratorTest extends \MediaWikiTestCase {
	 ...
	 * @dataProvider projectLinkSidebarProvider
	 */
	public function testBuildProjectLinkSidebarForItem( array $siteIdsToOutput, array $result ) {
		$otherProjectSidebarGenerator = new OtherProjectsSidebarGenerator(
			'enwiki',
			$this->getSiteLinkLookup(),
			$this->getSiteStore(),
		...
		$this->assertEquals(
			$result,
			$otherProjectSidebarGenerator->buildProjectLinkSidebarForItem( ItemId::unserialize( 'Q42' ) )
		);
	}
