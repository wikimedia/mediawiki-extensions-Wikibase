...
class PageSplitTesterTest extends \PHPUnit\Framework\TestCase {
	...
	public function testScenarioSplit50() {
		...
		for ( $i = 0; $i < $iterations; ++$i ) {
			$random = wfRandom();
			$sampled += $subject->isSampled( $random ) ? 1 : 0;
			$buckets[ $subject->getBucket( $random ) ]++;
		}

		$this->assertGreaterThan( 0.49, $sampled / $iterations );
		$this->assertLessThan( 0.51, $sampled / $iterations );

		$this->assertGreaterThan( 0.32, $buckets['a'] / $iterations );
		$this->assertLessThan( 0.34, $buckets['a'] / $iterations );
		$this->assertGreaterThan( 0.32, $buckets['b'] / $iterations );
		$this->assertLessThan( 0.34, $buckets['b'] / $iterations );
		$this->assertGreaterThan( 0.32, $buckets['c'] / $iterations );
		$this->assertLessThan( 0.34, $buckets['c'] / $iterations );
	}
