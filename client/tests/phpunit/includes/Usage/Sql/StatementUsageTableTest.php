...
use Wikibase\Client\Usage\StatementUsage;
use Wikibase\Client\Usage\PageEntityDataUsages;
use Wikibase\Client\Usage\Sql\StatementUsageTable;
use Wikibase\DataModel\Entity\ItemId;
use Wikibase\DataModel\Entity\ItemIdParser;

/**
 * @covers Wikibase\Client\Usage\Sql\StatementUsageTable
 *
 * @group Wikibase
 ...
 * @author Marius Hoch
 */
class StatementUsageTableTest extends \MediaWikiTestCase {

	protected function setUp() {
		parent::setUp();

		$this->tablesUsed[] = StatementUsageTable::DEFAULT_TABLE_NAME;
	}
	...
	private function getEntityUsageTable( $batchSize = 1000 ) {
		return new StatementUsageTable( new ItemIdParser(), wfGetDB( DB_MASTER ), $batchSize );
	}
	...
	public function testAddUsages() {
		...
		$q4 = new ItemId( 'Q4' );
		$q5 = new ItemId( 'Q5' );
		//make new property ids
		//pass booleans

		$usagesT1 = array(
			new StatementUsage( $q3, S ),
			new StatementUsage( $q3, StatementUsage::LABEL_USAGE ),
			new StatementUsage( $q4, StatementUsage::LABEL_USAGE ),
		);

		$usagesT2 = array(
			new StatementUsage( $q3, StatementUsage::SITELINK_USAGE ),
			new StatementUsage( $q4, StatementUsage::LABEL_USAGE ),
			new StatementUsage( $q5, StatementUsage::ALL_USAGE ),
		);
	...
	private function getUsageStrings( array $usages ) {
		$strings = array_map( function ( EntityDataUsage $usage ) {
