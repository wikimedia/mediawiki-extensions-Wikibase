...
class SpecialUnconnectedPagesTest extends SpecialPageTestBase {
	...
	// TODO: Results during test indicates a default enabled, and then disable with $exclude
	public function provideNamespaceChecker() {
		return array(
			array( 0, true ),  // #0
			array( 1, true ),  // #1 - undefined
			array( 2, false ), // #2
			array( 3, true ),  // #3 - undefined
			array( 4, false ), // #4
			array( 5, true ),  // #5 - undefined
			array( 6, true ),  // #6 - undefined
			array( 7, true ),  // #7 - undefined
		);
	}
	...
	public function testBuildConditionals( $text, $expected ) {
		...
		$this->assertEquals( $expected, $page->buildConditionals( $title, $dbr, $checker ) );
	}

	public function provideBuildConditionals() {
		return array(
			array( 'foo', array( "page_title >= 'Foo'", "page_namespace = 0" ) ),
			array( ':foo', array( "page_title >= 'Foo'", "page_namespace = 0" ) ),
			array( 'en:foo', array( "page_title >= 'foo'", "page_namespace = 0" ) ),
			array( 'user:foo', array() ),
			array( 'user talk:foo', array( "page_title >= 'Foo'", "page_namespace = 3" ) ),
		);
	}
