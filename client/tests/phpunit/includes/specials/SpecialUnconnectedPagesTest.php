...
class SpecialUnconnectedPagesTest extends SpecialPageTestBase {
	 ...
	 */
	public function testExecute( $subPage, $explanation ) {
		...
		$this->assertTrue( true, $explanation );
	}

	// TODO: Results during test indicates a default enabled, and then disable with $exclude
	public function provideSubPage() {
		return array(
			array( '', 'Calling execute without any subpage value' ), // #0
			array( 'Main Page', 'Calling execute with Main Page as the subPage' ), // #1
		);
	}
	...
	public function provideBuildConditionals() {
		return array(
			...
			array( ':foo', array( "page_title >= 'Foo'", "page_namespace = 0", 'page_namespace IN (0)' ) ),
			array( 'en:foo', array( "page_title >= 'foo'", "page_namespace = 0", 'page_namespace IN (0)' ) ),
