...
/**
 * This widget can take a couple of arguments to make it work on pages and
 * sites other than the current one. All these options default to global state.
 *
 ...
 *
 * @option langLinkSiteGroup {string} Group of sites we allow the user to
 *         link the given page with
 */
$.widget( 'wikibase.linkitem', {
	...
	options: {
		pageTitle:
			( new mw.Title( mw.config.get( 'wgTitle' ), mw.config.get( 'wgNamespaceNumber' ) ) ).getPrefixedText(),
	...
	_createDialog: function() {
		this.$dialog = $( '<div>' )
			...
			.on( 'dialogclose', $.proxy( this.destroy, this ) )
			.append(
				$( '<p>' )
	...
	_getSiteLinkForm: function() {
		...
		.append( this._getSiteInput() )
		.append( $( '<br />' ) )
		.append( this._getPageInput() );
	},

	/**
	 * Get an input + label for selecting client sites
	 *
	 * @return {jQuery}
	 */
	_getSiteInput: function() {
		return $( '<label>' )
	...
	/**
	 * Get an input + label for selecting pages on a client site
	 *
	 * @return {jQuery}
	 */
	_getPageInput: function() {
		return $( '<label>' )
	...
	_onSecondStep: function() {
		...
		this.pageConnector = new wb.PageConnector(
		...
		this.pageConnector.getNewlyLinkedPages()
