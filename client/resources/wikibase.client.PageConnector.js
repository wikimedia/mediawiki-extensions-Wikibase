...
$.extend( wb.PageConnector.prototype, {
	...
	_setSiteLink: function( entity, siteId, pageName ) {
		return this.repoApi.setSitelink(
			entity.id,
			entity.lastrevid,
			siteId,
			pageName
	...
	_onFirstPageNoItem: function() {
		...
		.done( function( data ) {
			...
			if ( data.entities['-1'] ) {
				...
				self._setSiteLink( entity, self.firstSiteId, self.firstPageName )
				.done( deferred.resolve )
				.fail( deferred.reject );
			}
		} );
	...
	_linkOrMergeSecondPage: function( entity ) {
		...
		.done( function( data ) {
			...
			if ( data.entities['-1'] ) {
				...
				self._setSiteLink( entity, self.secondSiteId, self.secondPageName )
				.done( deferred.resolve )
				.fail( deferred.reject );
			} else {
